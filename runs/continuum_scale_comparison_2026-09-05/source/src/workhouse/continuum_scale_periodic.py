"""Exact finite periodic link-Hodge, Green-kernel and fast-field controls.

The all-size harmonic theorem is analytic. These controls retain interfaces,
harmonic cochains and a nonlocal projected-box witness on periods three and
four; they do not identify a nonlinear quantum or OS fast complement.
"""
...
def validate_sites(n, sites):
    if isinstance(n, bool) or not isinstance(n, int) or n < 3:
        raise ValueError("Use an integer period n>=3")
    if len(sites) != n**3 or set(sites) != set(itertools.product(range(n), repeat=3)):
        raise ValueError("Sites must enumerate the complete periodic cube exactly once")
...
def box_data(n, side, sites):
    validate_sites(n, sites)
    if isinstance(side, bool) or not isinstance(side, int) or side < 2 or n % side:
...
def periodic_green(n, sites):
    ...
    cosines = (
        [s.Integer(1), -s.Rational(1, 2), -s.Rational(1, 2)]
        if n == 3
    ...
    kernel = {
        x: sum(
            cosines[sum(a * b for a, b in zip(k, x, strict=True)) % n] / eigenvalues[k]
            for k in sites
            if k != (0, 0, 0)
        )
        / n**3
    ...
    return s.Matrix(
        len(sites),
        len(sites),
        lambda i, j: kernel[tuple((a - b) % n for a, b in zip(sites[i], sites[j], strict=True))],
    )


@lru_cache(maxsize=1)
def _cached_periodic_controls():
    if not __debug__:
    ...
    for n, side in [(3, 3), (4, 2)]:
        ...
        assert (actual_difference.T * actual_difference)[0] <= (raw_difference.T * raw_difference)[
            0
        ]
    ...
    return {
        "scope": __doc__,
        "periodic_controls": rows,
        "d1_d0_and_d2_d1_zero": True,
        "exact_componentwise_Hodge": True,
        "positive_interface_decomposition": True,
        "short_period_requires_separate_convention": rejected_short_period,
    }


def exact_periodic_controls():
    """Return an independent copy of the cached exact finite geometry report."""
    return copy.deepcopy(_cached_periodic_controls())
