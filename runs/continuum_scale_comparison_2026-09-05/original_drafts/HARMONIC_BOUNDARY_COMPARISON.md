...
5 September 2026. Outputs-only continuation of the physical scale-block
package. The main new result is an all-size coercivity and spectral-complement
...
identified with an OS-history block map. Nonlinear errors uniform in block
size and lattice dimension three remain separate obligations.
...
The root collaborator's exact frequency-dependent elimination retains a
memory term. Its static graph trial has potential K0 and norm M. If mu_j
...
source normalization. The present planar face-coordinate identity is not
silently extended to a three-dimensional curl with Bianchi constraints,
nor to a nonlinear OS-history complement.
