...
5 September 2026. Outputs-only analytic derivation, after the sealed physical
block package. The target is to replace a special two-square calculation by
a theorem for actual coupled Wilson cells, and identify the precise growing-
block constants needed for a scale comparison. No canonical record is changed.

## 1. Existing inputs, provenance and the new statement

The current graph was queried with `workhouse why G19` and `workhouse search
'curl'`. G19 already has the live actual-history/complement comparison route
and the dead raw averaging/diffusion shortcut. The generated graph was still
being rebuilt when the new two-square result ID was queried; its frozen
canonical proof, rather than that temporary absence, is the mathematical input.
...
A cheapest substantive next calculation is therefore a local gauge/Hodge
