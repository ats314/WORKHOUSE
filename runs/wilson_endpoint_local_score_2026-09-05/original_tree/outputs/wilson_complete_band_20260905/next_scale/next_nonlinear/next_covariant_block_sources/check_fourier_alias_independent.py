"""Finite exact controls for the independently audited alias mechanism."""
...
def controls() -> dict:
    ...
    for length in range(1, 13):
        ...
        fourth = Counter(
            s - t + v - w
            for s, t, v, w in itertools.product(range(length), repeat=4)
        )
        second_alias = {
            q // length: Fraction(count, length)
            for q, count in second.items()
            if q % length == 0
        }
        fourth_alias = {
...
def main() -> None:
    if not __debug__:
        raise RuntimeError("Optimized mode is not an acceptance mode")
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError(args.output)
    paths = [Path(__file__), Path(__file__).with_name("INDEPENDENT_FOURIER_ALIAS_AUDIT.md")]
    payload = {
        "schema": "workhouse-independent-fourier-alias/v1",
        "controls": controls(),
    ...
    with args.output.open("x", encoding="utf-8", newline="\n") as output:
        ...
        output.write("\n")
    print(json.dumps({"passed": True, "moment_cases": 12, "alias_fibers": 8}))
