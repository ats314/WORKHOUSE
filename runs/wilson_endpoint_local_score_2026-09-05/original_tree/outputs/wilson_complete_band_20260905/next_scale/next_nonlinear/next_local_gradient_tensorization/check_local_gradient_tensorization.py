"""Exact support-wise gradient energy and global bad-event controls."""
...
def main():
    ...
    proof = source.with_name("LOCAL_GRADIENT_EXCITATION_SUPPORT.md")
    report["source_sha256"] = {
        path.name: hashlib.sha256(path.read_bytes()).hexdigest() for path in (source, proof)
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    with args.output.open("x", encoding="utf-8", newline="\n") as stream:
        json.dump(report, stream, indent=2, sort_keys=True)
        stream.write("\n")
    print("PASS: complete tensor gradient matrices, centering negative and any-bad counterexample")
