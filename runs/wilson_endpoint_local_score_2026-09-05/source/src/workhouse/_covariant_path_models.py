"""Preserved exact covariant path and full Fourier controls.

Source: next_covariant_block_sources/check_covariant_path_sources.py
Original SHA256: e18341b684ff0b46edae3235ea113fcda7457f013a942be8433819bbc8dd871e
Finite n=4,L=2 controls; all-size theorem remains analytic.
"""
...
def geometry_controls() -> dict:
    ...
    rows = [[(j, r[row, j]) for j in range(r.cols) if r[row, j] != 0] for row in range(r.rows)]
    plane_wave_checks = 0
    for frequency in itertools.product(range(n), repeat=3):
        plane = {x: s.I ** (sum(frequency[j] * x[j] for j in range(3)) % 4) for x in sites}
