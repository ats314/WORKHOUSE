...
def partitions(sites):
    ...
    def visit(labels, maximum):
        if len(labels) == len(sites):
            yield tuple(tuple(site for site, label in zip(sites, labels, strict=True) if label == j)
...
def mobius_cumulants(family):
    ...
    for sites in sorted(family, key=lambda s: (len(s), s)):
        ...
        for partition in partitions(sites):
            k = len(partition)
            value += (-1) ** (k - 1) * factorial(k - 1) * tensor_entries(
                partition, [family[block] for block in partition], sites
            )
        answer[sites] = value
    return answer
...
def rooted_cumulants(family):
    ...
    G_X=sum_{B contains min(X)} C_B tensor G_{X\B}; isolate B=X.
    ...
    for sites in sorted(family, key=lambda s: (len(s), s)):
        ...
        for tail in subsets(sites[1:]):
            ...
            value -= tensor_permuted((block, complement), (answer[block], family[complement]), sites)
        answer[sites] = value
    return answer
...
def components(sites, edges):
    ...
    while remaining:
        part = {min(remaining)}
        while True:
            enlarged = part | {site for edge in edges if set(edge) <= set(sites) and part.intersection(edge)
                               for site in edge}
            if enlarged == part:
                break
            part = enlarged
...
def edge_projection():
    return sp.Matrix([[0, 0, 0, 0], [0, sp.Rational(1, 2), sp.Rational(1, 2), 0],
                      [0, sp.Rational(1, 2), sp.Rational(1, 2), 0], [0, 0, 0, 0]])


def ordered_transfer_family(ambient, edges, u=sp.Rational(1, 10), d=sp.Rational(1, 4)):
    if not isinstance(u, (int, sp.Rational)) or not isinstance(d, (int, sp.Rational)):
    ...
    for sites in subsets(ambient):
        ...
        for edge in edges:
            if set(edge) <= set(sites):
                ...
                embedded = tensor_entries((edge, outside), (local, sp.eye(2 ** len(outside))), sites)
        ...
        positive_certificates[sites] = {"det_B": str(determinant), "smallest_D_entry": str(min(free.diagonal()))}
...
def check_family(name, ambient, edges):
    ...
    activities = {sites: (sp.zeros(2) if len(sites) == 1 else cumulant)
                  for sites, cumulant in closed.items()}
    disconnected = []
    reconstruction = 0
    for sites, transfer in family.items():
        ...
        assert transfer == tensor_permuted(component_list, [family[c] for c in component_list], sites)
    ...
    if ((0, 1) in edges) and ((1, 2) in edges):
        left = tensor_entries(((0, 1), (2, 3)), (activities[(0, 1)], sp.eye(4)), ambient)
        right = tensor_entries(((1, 2), (0, 3)), (activities[(1, 2)], sp.eye(4)), ambient)
        commutator = left * right - right * left
        overlap = sp.trace(commutator.T * commutator)
        assert overlap > 0
        assert activities[ambient] != sp.zeros(2 ** len(ambient))
    return {
        "name": name, "ambient": ambient, "ordered_edges": edges,
        "subsystems": len(family), "reconstruction_count": reconstruction,
        "vacuum_anchoring_count": len(activities), "disconnected_cancellations": disconnected,
        ...
        "positivity_congruences": [{"sites": sites, **values} for sites, values in positivity.items()],
        "activities": [{"sites": sites, "matrix": [[str(v) for v in activity.row(i)] for i in range(activity.rows)]}
...
def missing_factorization_control():
    ambient = (0, 1)
    family = {sites: sp.diag(*(sp.Rational(1, 1 + sum(word)) for word in itertools.product((0, 1), repeat=len(sites))))
              for sites in subsets(ambient)}
    ...
    return {"family": "G_X=(I+sum_i q_i)^(-1)", "positive_and_vacuum_fixing": True,
            "component_factorization_fails": True, "disconnected_activity_11_entry": "1/12",
            "scope": "Two disconnected binary sites: vacuum fixing alone does not imply connected support."}


def exact_controls():
    return {
        "models": [check_family("ordered_chain", (0, 1, 2, 3), ((0, 1), (1, 2), (2, 3))),
                   check_family("disjoint_edges", (0, 1, 2, 3), ((0, 1), (2, 3)))],
        "missing_factorization": missing_factorization_control(),
        "scope": "Finite exact extraction algebra only; no uniform Wilson activity or marked norm bound.",
    }
...
if __name__ == "__main__":
    ...
    output = Path(__file__).with_name("independent_draft_certificate.json")
    if output.exists():
        raise FileExistsError(output)
    data = exact_controls()
    output.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps({"models": [{k: v for k, v in model.items() if k not in ("activities", "positivity_congruences")}
                                for model in data["models"]], "negative": data["missing_factorization"]}, indent=2))
