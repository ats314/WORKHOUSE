...
tried. Their default machine tier is T3; mathematical status and analytic
validity remain separate, as in CLAUDE.md.
...
class Notes:
    ...
    def unreviewed(self, archive_id: str) -> list[dict[str, Any]]:
        # Reviews concern exact content, not its later archive location. Reuse
        # the original scoped verdict without creating a new review or treating
        # a merely pinned/indexed document as mathematically reviewed.
        seen = self.reviewed_digests()
...
def format_status(notes: Notes) -> str:
    ...
    for archive in notes.archives:
        ...
        if rows is None:
            ...
            add("")
            continue
        manifest_digests = {row["digest"] for row in rows}
        reviewed = notes.reviewed_digests(aid) & manifest_digests
        reused = (notes.reviewed_digests() & manifest_digests) - reviewed
        counts: dict[str, int] = {}
        reused_counts: dict[tuple[str, str], int] = {}
        for review in notes.reviews:
            if review.get("archive") == aid:
                counts[review["verdict"]] = counts.get(review["verdict"], 0) + 1
            elif review["digest"] in reused:
                key = (review["archive"], review["verdict"])
                reused_counts[key] = reused_counts.get(key, 0) + 1
        pending = notes.unreviewed(aid)
        bearing = sum(1 for r in pending if r.get("coefficients"))
        add(
            f"  {len(rows)} unique documents, {len(reviewed)} own reviews, "
            f"{len(reused)} reused scoped reviews, {len(pending)} pending"
        )
        for verdict in sorted(VERDICTS):
            if counts.get(verdict):
                add(f"    {counts[verdict]:>5}  {verdict}")
        for (source_archive, verdict), count in sorted(reused_counts.items()):
            add(f"    {count:>5}  reused {verdict} from {source_archive}")
        if reused:
            add("  Original verdicts and reasons remain in ledger/notes.yaml under those archives.")
        if bearing:
...
def format_queue(notes: Notes, limit: int = 20) -> str:
    ...
    if not items:
        return "review queue is empty — every inventoried content hash has a scoped review"
