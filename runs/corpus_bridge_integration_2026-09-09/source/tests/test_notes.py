...
def test_identical_content_reuses_original_scoped_review_across_archives(registry):
    digest = _digest_of(registry, "hs_note")
    original = {
        "digest": digest,
        "archive": "TEST_ARCHIVE",
        "verdict": "set-aside",
        "reason": "The conditional statement is useful; its physical hypothesis is unproved.",
    }
    registry.reviews.append(original)
    registry.archives.append(
        {"id": "LATER_ARCHIVE", "description": "relocated copies", "source": "fixture"}
    )
    registry.manifests["LATER_ARCHIVE"] = [
        {"digest": digest, "paths": ["renamed.md"]},
        {"digest": "ab" * 32, "paths": ["hs_note.md"], "pinned_as": "paper/indexed.md"},
    ]

    # A different name with the same bytes inherits the original scoped review;
    # the same name with changed bytes and a pin still needs review.
    pending = registry.unreviewed("LATER_ARCHIVE")
    assert [row["digest"] for row in pending] == ["ab" * 32]
    assert registry.reviewed_digests("LATER_ARCHIVE") == set()
    assert registry.reviews == [original]
    assert all(row["digest"] != digest for _, row in notes.queue(registry))

    status = notes.format_status(registry)
    later = status.split("LATER_ARCHIVE:", 1)[1]
    assert "2 unique documents, 0 own reviews, 1 reused scoped reviews, 1 pending" in later
    assert "reused set-aside from TEST_ARCHIVE" in later
    assert "Original verdicts and reasons remain" in later
    assert registry.reviews[0]["reason"] == original["reason"]


def test_multiple_scoped_reviews_of_one_hash_do_not_double_count_reused_content(registry):
    digest = _digest_of(registry, "hs_note")
    registry.reviews.extend(
        [
            {
                "digest": digest,
                "archive": "TEST_ARCHIVE",
                "verdict": "extract",
                "reason": "Only the named finite identity.",
            },
            {
                "digest": digest,
                "archive": "COPY_ARCHIVE",
                "verdict": "duplicate",
                "reason": "Exact copy of the previously reviewed bytes.",
            },
        ]
    )
    registry.archives.append(
        {"id": "LATER_ARCHIVE", "description": "another copy", "source": "fixture"}
    )
    registry.manifests["LATER_ARCHIVE"] = [{"digest": digest, "paths": ["copy.md"]}]
    status = notes.format_status(registry).split("LATER_ARCHIVE:", 1)[1]
    assert "1 unique documents, 0 own reviews, 1 reused scoped reviews, 0 pending" in status
    assert "reused extract from TEST_ARCHIVE" in status
    assert "reused duplicate from COPY_ARCHIVE" in status
    assert registry.unreviewed("LATER_ARCHIVE") == []
