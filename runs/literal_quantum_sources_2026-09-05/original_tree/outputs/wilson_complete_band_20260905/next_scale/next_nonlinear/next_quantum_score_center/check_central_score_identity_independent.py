...
def controls():
    ...
    zero(dprime-(constant-aa**2-bb**2-2*cot*(aa-bb)), 'actual Wilson Riccati difference')
    ...
    zero(laplacian_class(2*sp.cos(theta))+r(3,4)*2*sp.cos(theta), 'normalized fundamental Casimir')
    omega = 2+sp.cos(theta)  # smooth positive on the whole SU(2) group
    reflected = 2-sp.cos(theta)
    A = -sp.sin(theta)/(2+sp.cos(theta))
    B = -sp.sin(theta)/(2-sp.cos(theta))
    d = sp.cancel(A-B)
    s = sp.cancel(A+B)
    p = sp.sin(theta)**2 * (4-sp.cos(theta)**2)**2
    zero(sp.diff(p,theta)-p*(2*sp.cot(theta)+2*d), 'exact Haar conditional density derivative')
    ...
    require(haar_term == r(28,113) and mean_dprime == -r(36,113), 'Haar integration-by-parts factor')
    ...
    zero(-(a/2)*laplacian_class(omega)+reconstructed*omega-e*omega, 'reconstructed ground equation')
    ...
    require(sp.Poly(wilson_residual,cc).coeff_monomial(cc**2) == -4*u,
    ...
    for cosine,sine in ((r(3,5),r(4,5)),(sp.Integer(0),sp.Integer(1)),(-r(3,5),r(4,5))):
        ...
        rot = sp.Matrix(3,3,lambda i,j:sp.simplify(-2*sp.trace(generators[i]*k*generators[j]*k.adjoint())))
        ...
        expected = sp.BlockMatrix([[2*a*sp.eye(3),a*sp.eye(3)],[a*sp.eye(3),a*sp.eye(3)]]).as_explicit()
        ...
        for j,tangent in enumerate(generators):
            ...
            zero(2*(aa*theta1+bb*theta2)-(aa+bb)*axis[j]/2,'actual quantum density score')
            angle_derivatives.append(str(theta1))
        geometry.append({'cos_theta':str(cosine),'sin_theta':str(sine),'both_angle_derivatives':angle_derivatives})
    polar,azimuth = sp.symbols('polar azimuth',real=True)
    nvec = sp.Matrix([sp.sin(polar)*sp.cos(azimuth),sp.sin(polar)*sp.sin(azimuth),sp.cos(polar)])
    covariance = sp.Matrix(3,3,lambda i,j:sp.integrate(sp.integrate(nvec[i]*nvec[j]*sp.sin(polar),(azimuth,0,2*sp.pi)),(polar,0,sp.pi))/(4*sp.pi))
    zero(covariance-sp.eye(3)/3,'uniform conjugacy-axis covariance')
    return {
        'passed':True,
        'wilson_scalar_identity':{'mean_s_squared':'128*u/a-32*e/a+3*mean_d_squared',
                                  'weighted_Fisher':str(weighted),'actual_bouquet_a':4},
        'positive_reconstructed_trial':{'omega':'2+cos(theta)','radial_normalizer':str(norm),
            'mean_s_squared':str(mean_s2),'mean_d_squared':str(mean_d2),
            'mean_two_cot_d':str(haar_term),'mean_d_prime':str(mean_dprime),
            'mean_potential_pair_minus_two_e':str(mean_potential_pair),
            'endpoint_fluxes':list(map(str,endpoint_limits)),
            'omitted_Haar_drift_rejected':True,'not_a_Wilson_ground':True,
            'Wilson_residual_cosine_squared_coefficient':'-4*u'},
        'SU2_geometry':{'fundamental_Casimir':'3/4','Cuu':'2*a*I','b':'I/2','S':'a*I/2',
            'cases':geometry,'axis_covariance':'I/3','per_axis_Fisher':'E(s^2)/12'},
        'scope':'Exact Riccati algebra, normalized Haar IBP in a reconstructed-potential trial, and finite exact SU(2) horizontal/metric controls. The trial is not a Wilson solution. The actual Wilson identity and large-u obstruction use the analytic positive ground equation and energy bound.',
    }
...
def main():
    ...
    with args.output.open('x',encoding='utf-8',newline='\n') as stream:
        ...
        stream.write('\n')
    print('PASS: exact Riccati, Haar and actual SU2 horizontal-score controls')
