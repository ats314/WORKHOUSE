...
def main():
    ...
    sources = (
        ...
        here / "LITERAL_VACUUM_COARSE_PROJECTION.md",
        repo / "paper/research_notes/G19_WILSON_TWO_SQUARE_PHYSICAL_SHELLS_20260905.md",
    ...
    before = hashes()
    payload = controls()
    require(before == hashes(), "sources unchanged during exact calculation")
    report = {
        "schema": "workhouse-literal-vacuum-controls/v1",
        "passed": True,
        "scope": __doc__,
        "controls": payload,
        "sources": before,
    ...
    print(
        json.dumps(
            {"output": str(args.output), "passed": True, "families": list(payload)}, indent=2
        )
    )
