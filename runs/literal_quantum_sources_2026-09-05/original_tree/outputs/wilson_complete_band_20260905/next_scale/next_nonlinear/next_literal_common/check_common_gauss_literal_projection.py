...
def main():
    ...
    proof = here.with_name("COMMON_GAUSS_LITERAL_FAST_FLOOR.md")
    result["source_sha256"] = {
        path.name: hashlib.sha256(path.read_bytes()).hexdigest() for path in (here, proof)
    }
    if args.output is None:
        ...
        with args.output.open("x", encoding="utf-8", newline="\n") as handle:
            ...
            handle.write("\n")
        print(f"PASS: exact common-Gauss controls; wrote {args.output}")
