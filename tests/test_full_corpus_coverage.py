...
@pytest.mark.parametrize(
    "field,value",
    [
        ("text_file", "../wrong-source.txt"),
        ("text_sha256", "0" * 64),
        ("method", "fabricated full visual and mathematical verification"),
    ],
)
def test_extraction_metadata_is_matched_to_pinned_sources(fixture, field, value):
    module, records, inputs = fixture
    index = next(i for i, row in enumerate(records) if row["extraction"].get("text_file"))
    bad = list(records)
    bad[index] = {**records[index], "extraction": {**records[index]["extraction"], field: value}}
    with pytest.raises(ValueError, match="Normalized extraction metadata"):
        module.validate_records(bad, inputs)


def test_dropped_structured_archive_chain_is_rejected(fixture):
    module, records, inputs = fixture
    index = next(i for i, row in enumerate(records) if row["provenance"]["archive_members"])
    bad = list(records)
    bad[index] = {
        **records[index],
        "provenance": {**records[index]["provenance"], "archive_members": []},
    }
    with pytest.raises(ValueError, match="Structured archive chain"):
        module.validate_records(bad, inputs)


def test_changed_archive_container_digest_is_rejected(fixture):
    module, records, inputs = fixture
    index = next(i for i, row in enumerate(records) if row["provenance"]["archive_members"])
    members = copy.deepcopy(records[index]["provenance"]["archive_members"])
    members[0]["container_sha256"] = "0" * 64
    bad = list(records)
    bad[index] = {
        **records[index],
        "provenance": {**records[index]["provenance"], "archive_members": members},
    }
    with pytest.raises(ValueError, match="Structured archive chain"):
        module.validate_records(bad, inputs)


def test_dropped_prior_review_is_rejected(fixture):
    module, records, inputs = fixture
    index = next(i for i, row in enumerate(records) if row["provenance"]["prior_review_refs"])
    bad = list(records)
    bad[index] = {
        **records[index],
        "provenance": {**records[index]["provenance"], "prior_review_refs": []},
    }
    with pytest.raises(ValueError, match="Prior review provenance"):
        module.validate_records(bad, inputs)


def test_fabricated_prior_review_is_not_self_authenticating(fixture):
    module, records, inputs = fixture
    index = next(i for i, row in enumerate(records) if not row["provenance"]["prior_review_refs"])
    fabricated = [{"archive": "invented", "digest": records[index]["digest"], "verdict": "import"}]
    bad = list(records)
    bad[index] = {
        **records[index],
        "provenance": {**records[index]["provenance"], "prior_review_refs": fabricated},
    }
    with pytest.raises(ValueError, match="Prior review provenance"):
        module.validate_records(bad, inputs)


def test_duplicate_census_path_cannot_be_silently_collapsed(fixture):
    module, _, inputs = fixture
    bad = {**inputs, "loose_files": [*inputs["loose_files"], inputs["loose_files"][0]]}
    with pytest.raises(ValueError, match="Duplicate loose file"):
        module.expected_inventory(bad)
