...
from workhouse import recent_research
...
def test_every_target_resolves_and_the_graph_carries_the_edges():
    ...
    provenance_ids = {f"DOC:{d['id']}" for d in DOCUMENTS}
    recent_ids = {source["id"] for source in recent_research.load().sources}
    assert not (provenance_ids & recent_ids)
    assert doc_ids == provenance_ids | recent_ids
