...
- `FINDING: an explicit second witness C_alt exhibits the C2 non-identifiability`
  - src/workhouse/invariants/channels.py:469 · COMPLETE_UNIFIED_MASTER_CLOSED §15.3.2
...
- `on an axial cut the mixed invariants vanish and the norm divides`
  - src/workhouse/invariants/channels.py:442 · MASTER paper §7
...
- `the shape fit's C row sums to zero, so no Gamma-anchor error can move C_shp at all`
  - src/workhouse/invariants/channels.py:990 · provenance nb-hodge-v10a26-alt2, _v10a3_extract_shape; UNIFIED v4.3 §5.1
...
- `FINDING: the certificate key 'bandmin' holds the band MAXIMUM, at both orders`
  - src/workhouse/invariants/charge_even.py:353 · RUN_TROM_d3_results.json vs PAPER, the §6 patch, and the arithmetic
...
- `at N = 2 the C-odd hopping vanishes and the C-even one does not`
  - src/workhouse/invariants/charge_even.py:558 · MASTER_THEORY §4.3; ledger/theorems.yaml (t_2 = 0)
...
- `both declared coincidences, checked: one is ell_N at all ranks, the other is bare`
  - src/workhouse/invariants/charge_even.py:511 · ENGINE_FLUX_su3_domino_d3.py / MASTER paper Prop. 8
...
- `one assembly formula gives every C-even value at both orders`
  - src/workhouse/invariants/charge_even.py:318 · ENGINE_FLUX_su3_domino_d3.py / MASTER paper eq. (30)
...
- `the Bloch cubic IS the finite L = 3 and L = 4 plaquette spectrum, exactly`
  - src/workhouse/invariants/charge_even.py:277 · UNIFIED §0.1; the corpus checks the analogous statement numerically
...
- `the C-even Gamma point pins t_+, exactly where no C-odd Gamma datum can`
  - src/workhouse/invariants/charge_even.py:413 · MASTER paper Rmk. 12 / PAPER Thm. (the C-even band)
...
- `the C-even band touches its floor exactly on the three planes k_j = pi`
  - src/workhouse/invariants/charge_even.py:235 · PAPER Rmk. (an exactly immobile excitation)
...
- `the C-even bandwidth is 16|t_+| at every order; the C-odd manifold width is 12|t_-|`
  - src/workhouse/invariants/charge_even.py:477 · PAPER Rmk. (an exactly immobile excitation)
...
- `the C-even curvature is (4/3)|t_+| at both orders, and isotropic`
  - src/workhouse/invariants/charge_even.py:446 · PAPER Thm. (the C-even band); §6 patch curvature 22/459
...
- `the C-even range is exact; the top is attained only at Gamma, the floor on three planes`
  ...
  - `workhouse verify --only 'the C-even range is exact; the top is attained only at Gamma, the floor on three planes'`
- `the C-even spectra at the four high-symmetry momenta`
  - src/workhouse/invariants/charge_even.py:298 · MASTER paper §4.5 / PAPER Thm. (the C-even band)
...
- `q at the four high-symmetry points is 0, 4, 8, 12`
  - src/workhouse/invariants/manuscript.py:167 · MASTER_DOC Fig. 2
...
- `the B=6 six-channel census IS the Weingarten four-channel ledger, channel by channel`
  - src/workhouse/invariants/two_cube.py:313 · R2; runs/two_cube_codd_o2_2026-08-29 §7; FINAL paper Thm. 12
...
- `FINDING: A = 5/48 pins the normal sector's whole C4 contribution`
  - src/workhouse/invariants/channels.py:589 · UNIFIED v4.3 §5.1/§6, block decomposition
...
- `FINDING: C_shp is carried by 6 of 189 records, not spread across the kernel`
  - src/workhouse/invariants/channels.py:519 · UNIFIED v4.3 §5.1/§6, block decomposition
...
- `FINDING: the adjudicator's THIRD-VALUE scalar verdict is a C1 anchoring artifact`
  - src/workhouse/invariants/channels.py:866 · provenance nb-hodge-v10a26-alt2, section 17
...
- `FINDING: the off-axis ledger carries two claims its own author later retracted`
  - src/workhouse/invariants/channels.py:1176 · notes UPLOADS_2026-08-28d OFF_AXIS_LEDGER §6/§7, corrected by the C2 status note
...
- `FINDING: the run's own re-anchor moved C_shp by 4.6e-15, as the zero row sum requires`
  - src/workhouse/invariants/channels.py:1053 · provenance nb-hodge-v10a26-alt2, sections 13 and 17
...
- `FINDING: the v10a.26 cluster ledger is exhausted by supports <= 2 until fourth order`
  - src/workhouse/invariants/channels.py:1101 · provenance nb-hodge-v10a26-alt2, section 16 rooted incidence transform
...
- `FINDING: the v10a.26 notebook carries a per-class ledger and attests its own blindness`
  - src/workhouse/invariants/channels.py:718 · provenance nb-hodge-v10a26-alt2
...
- `FINDING: the v10a.26 run gates on known values before it unblinds`
  - src/workhouse/invariants/channels.py:794 · provenance nb-hodge-v10a26-alt2, cell 17
...
- `FINDING: the v10a.26 side supplies A, B, D but no block structure, and its C fights its own A`
  - src/workhouse/invariants/channels.py:659 · provenance nb-hodge-v10a26-alt2 / GLUEBALL §10
...
- `the C_alt witness IS the balanced eps-free continuation, and Delta_C/(A/2) = 15/32`
  - src/workhouse/invariants/channels.py:936 · OFF AXIS LEDGER 2026-08-22 §2, UNIFIED v4.3 §5.1
...
- `every node the theory graph strands is stranded for a stated reason`
  - src/workhouse/invariants/manuscript.py:186 · index/graph.jsonl, index/claims.jsonl, ledger/theorems.yaml
