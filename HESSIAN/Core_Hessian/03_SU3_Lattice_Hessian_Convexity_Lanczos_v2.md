...
## Second-pass notes (corrections/flags)

- This code probes curvature of an **action written in exponential/algebra coordinates**. Curvature in coordinates is not itself a gauge-invariant quantity; treat it as a diagnostic.
- The Padé \([2/2]\) exponential is an approximation. If you push to large algebra norms, you must quantify (or correct) unitarity drift, since it can leak into plaquette values and Hessian–vector products.
- Any “uniform convexity” claim must be separated into:
  1) a *local* small-field statement (supported by the Haar Jacobian expansion), and
  2) a *global* statement (requires controlling excursions to large fields / chart boundaries, or proving they are negligible in capacity/probability).
