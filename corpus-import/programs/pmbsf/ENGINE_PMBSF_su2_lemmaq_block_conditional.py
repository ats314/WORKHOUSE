#!/usr/bin/env python3
"""
PMBSF SU(2) Lemma Q Block-Conditional Diagnostic

This is the frozen-exterior block-conditional run proposed in the
Rare-Source Probability Estimates draft.

It tests the local rare-source factorization target:

    E[ prod_{p in B} X_{p,eta} | F_{C^c} ] <= (C_Q q_eta)^|B|

and the cavity/rooted-source consequences:

    lambda_p({r}) / q_eta
      = E[X_p X_r | F_{C^c}] / (q_eta E[X_r | F_{C^c}]),

    lambda_root_p({r}) / q_eta
...
---------------------
1. Generates or loads full SU(2) Wilson link configurations.
2. Finds a smooth-source threshold t such that q_eta ~= Q_TARGET.
3. For random frozen-exterior blocks:
   - freezes exterior links,
   - resamples only interior block links by local Metropolis,
   - measures core plaquette sources X_{p,eta},
   - measures rooted bad-staple sources Y_p,
   - accumulates conditional single-source, pair, cavity, rooted-cavity,
     and cap-predictor statistics.

This is an actual numerical diagnostic, not a proof.

Default configuration follows the draft:
    L=16, beta=3.5, N_CFG=32,
    BLOCK_SIDE=6, CORE_MARGIN=2,
    N_BLOCKS_PER_CFG=4,
    BLOCK_THERM=128, BLOCK_BETWEEN=8, BLOCK_SAMPLES=256.

Outputs
-------
/content/PMBSF_SU2_LemmaQ_block_output/<RUN_ID>/
    full_config_summary.csv
    single_source_depth.csv
    single_source_depth_summary.csv
    DATA_PMBSF_cavity_ratio_by_distance.csv
...
CONFIG = {
    "RUN_NAME": "PMBSF_SU2_LemmaQ_block_conditional_stageA",
...
def periodic_delta(a: np.ndarray, b: np.ndarray, L: int) -> np.ndarray:
    d = np.abs(a - b) % L
    return np.minimum(d, L - d)

def periodic_l1(a: np.ndarray, b: np.ndarray, L: int) -> int:
    return int(periodic_delta(a, b, L).sum())
...
# ======================================================================================
# FULL-LATTICE METROPOLIS FOR GLOBAL CONFIGS
# ======================================================================================
...
# ======================================================================================
# LINK GATHER/SET AND LOCAL STAPLES FOR BLOCK UPDATES
# ======================================================================================
...
# ======================================================================================
# FULL CONFIG CACHE / GLOBAL THRESHOLD
# ======================================================================================
...
def summarize_single_source(single_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for depth, sub in single_df.groupby("depth"):
        ...
        rows.append({
            "depth": int(depth),
            "n": int(len(x)),
            "mean_ratio": float(np.mean(x)),
            "median_ratio": float(np.median(x)),
            "q90_ratio": float(np.quantile(x, 0.90)),
            "q95_ratio": float(np.quantile(x, 0.95)),
    ...
    for cfg_idx in range(n_cfg):
        ...
        for bidx in range(CONFIG["N_BLOCKS_PER_CFG"]):
            ...
            block_spec = make_block_spec(
                ...
                core_margin=CONFIG["CORE_MARGIN"],
                device=device
            )
            ...
            dist_accum: Dict[int, Dict[str, float]] = {}
            coords_np = [np.array(ent["coord"], dtype=np.int64) for ent in block_spec["core_entries"]]

            for i in range(ncore):
                for j in range(ncore):
                    if i == j:
                        continue
                    d = periodic_l1(coords_np[i], coords_np[j], L)
                    rec = dist_accum.setdefault(d, {
                        "pair_xx_sum": 0.0,
                        "pair_count": 0.0,
                        "lambda_num": 0.0,
                        "lambda_den": 0.0,
                    ...
                    rec["pair_xx_sum"] += float(mean_XX[i, j])
                    rec["pair_count"] += 1.0
                    rec["lambda_num"] += float(mean_XX[i, j])
                    rec["lambda_den"] += float(q_eta * bar_X[i])
                    rec["root_num"] += float(mean_YX[i, j])
                    rec["root_den"] += float(q_eta * bar_Y[i])

            for d, rec in dist_accum.items():
...
def run():
    ...
    readout = []
    readout.append("# PMBSF SU(2) Lemma Q Block-Conditional Diagnostic Readout")
    readout.append("")
    readout.append("## Status")
    readout.append("")
    readout.append("Finite-volume frozen-exterior block-conditional diagnostic. This is not a proof of Lemma Q.")
    readout.append("")
    ...
    if not ss_sum.empty:
        readout.append("## Table A. Single-source conditional control")
        readout.append("")
    ...
    if not cav.empty or not root.empty or not pair.empty:
        readout.append("## Table B. Cavity / rooted cavity / pair ratios")
        readout.append("")
        if not pair.empty:
            readout.append("### Pair ratio by distance")
            readout.append(pair.to_markdown(index=False))
            readout.append("")
        if not cav.empty:
            readout.append("### Cavity ratio by distance")
            readout.append(cav.to_markdown(index=False))
            readout.append("")
        if not root.empty:
            readout.append("### Rooted cavity ratio by distance")
            readout.append(root.to_markdown(index=False))
            readout.append("")

    reg = merged_outputs["cap_feature_regression"]
    if not reg.empty:
        readout.append("## Table C. Feature discrimination")
        readout.append("")
        readout.append(reg.to_markdown(index=False))
        readout.append("")

    readout.append("## Interpretation thresholds")
    readout.append("")
    readout.append("- Supportive single-source result: median `q_cond/q_eta` less than about 1.5 in the core, with q95 less than about 3.")
    readout.append("- Supportive cavity result: `cavity_Lambda(d)` remains O(1) and ideally decreases toward 1 with distance.")
    readout.append("- Supportive rooted result: `rooted_Lambda(d)` remains O(1); this is the direct test of rooted bad-staple absorption.")
    readout.append("- Supportive cap mechanism: regression slope of `log(q_cond)` versus `g_mean` is negative.")
    readout.append("- Failure mode: large boundary-conditioned ratios or cavity ratios that grow with distance/block index.")
    readout.append("")
