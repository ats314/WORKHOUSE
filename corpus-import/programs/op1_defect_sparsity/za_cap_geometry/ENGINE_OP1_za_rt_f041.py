#!/usr/bin/env python3
# ENGINE_OP1_za_rt_f041.py — F041 final engine (fresh path; mount sync). Measures the ROOTED/
# TILTED LCI object (reduction (7.2)) vs the pointwise min-over-subsets (F040), on the
# SU(2) heat-bath ensembles, with multiple independent configs to pin F040's fraction.
#
# Tilted ratio R(e,p;s)=nu^{B,s}(C_p)=[Σ_A s^|A| nu(C_p∩C_A)]/[Σ_A s^|A| nu(C_A)].
# LCI/(7.2): R<=C_LCI q (incident tilt must not inflate the target cap prob). Exact
# identity (7.4–7.7): Σ_A s^|A| nu(C_A)=E_nu[Π_{r∈B}(1+s 1_{C_r})]. Report amplification
# Amp(s)=R/nu(C_p) (LCI ⟺ O(1)); correlate with F040 pointwise-bad flag (min_A Δp==0).
# Gates G-RT1..4 + G-DT1 (see prints).
...
def gate_rt1(rng):
    ...
    for kap in (8.0,18.0,40.0):
        ...
        assert abs(emp-bess)<0.01, f'G-RT1 batch {kap}'
        assert abs(sc-bess)<0.02, f'G-RT1 scalar {kap}'
    return res
def gate_rt2(rng):
    worst=0.0
    for _ in range(6):
...
def main():
    ...
    def measure_config(U,seed):
        ...
        for (x,mu) in links:
            ...
            for tix in range(6):
                ...
                ratio_prod=float((bp*Wt2).mean())/float(Wt2.mean()); rt4=max(rt4,abs(ratio_sub-ratio_prod))
    ...
    all_rows=[]; per_cfg=[]; neg_w=0.0; rt3_w=0.0; rt4_w=0.0; plaqs=[]
    if a_.use_stored:
        ...
        if rows: per_cfg.append(float(np.mean(np.array(rows)[:,1]>0)))
        ncfg=1
    else:
        ncfg=0
        for ci in range(a_.ncfg):
            ...
            neg_w=max(neg_w,nw); rt3_w=max(rt3_w,r3); rt4_w=max(rt4_w,r4)
            if rows: per_cfg.append(float(np.mean(np.array(rows)[:,1]>0))); ncfg+=1
    assert all_rows, 'no records produced'
    assert neg_w<1e-9, f'G-DT1 {neg_w}'; assert rt3_w<1e-9, f'G-RT3 {rt3_w}'; assert rt4_w<1e-10, f'G-RT4 {rt4_w}'
    R=np.array(all_rows); kap=R[:,0]; mdp=R[:,1]; nu_p=R[:,2]; bad=mdp<=1e-12
    ampc={f'{S_LIST[j]}':R[:,3+j] for j in range(len(S_LIST))}
    def st(x):
        x=x[np.isfinite(x)]
        return {'median':float(np.median(x)),'mean':float(np.mean(x)),'q90':float(np.quantile(x,.90)),
    ...
    out={'tag':'F041','L':L,'beta':BETA,'a':aCAP,'core_margin':CM,'N_mc':a_.N,'ncfg':ncfg,
         ...
         'G-RT2_worst_MC_vs_quad':float(g2),'gates':'G-RT1,G-RT2,G-RT3,G-RT4,G-DT1 PASS',
         'pointwise_bad_frac':{'overall':float(np.mean(bad)),'per_config':per_cfg,
             'mean':float(np.mean(per_cfg)),'std':float(np.std(per_cfg))},
    ...
    print('  G-RT2 %.2e | G-RT3 %.1e | G-RT4 %.1e | G-DT1 %.1e  ALL PASS'%(g2,rt3_w,rt4_w,neg_w))
    print('pointwise-bad frac (F040): overall %.3f  per-cfg %.3f ± %.3f (ncfg=%d)'%(
        np.mean(bad),np.mean(per_cfg),np.std(per_cfg),ncfg))
    ...
    if bad.any():
        ...
        for s in ampc:
            d=out['Amp_on_pointwise_bad'][s]; print('   s=%-4s median %7.3f q90 %7.3f q99 %7.3f max %7.3f'%(s,d['median'],d['q90'],d['q99'],d['max']))
    print('MEASURE_DONE')
