#!/usr/bin/env python3
# One-shot final (v3): G-ZA2 = exact closed-form Delta_p check (k=0), not the
# leading-constant test (which mixes c0~+-1 configs outside the asymptotic regime).
import json, math, os
import numpy as np
...
g2=0.0                       # worst |Delta_p - closed_form| over k=0
n_asym=0; asym_dev=0.0       # leading-const check restricted to asymptotic regime
worst_multi=(0.0,0.0,1e9)
for _ in range(60000):
    ...
    if k == 0:
        ...
        c0 = float(m@n_p)
        # closed form: u_A=m, h(A)=1, h(A∪p)=c0*a+sqrt((1-c0^2)(1-a^2))
        cf = 1.0 - (c0*a + math.sqrt(max(0.0,(1-c0*c0)*(1-a*a))))
        g2 = max(g2, abs(Dp - cf))
        # leading-const, only deep in the asymptotic regime chi0 << (1-c0^2)
...
assert vp < 1e-7, f'GATE FAIL G-ZA1 (k=0 lower bound): {vp}'
assert g2 < 1e-7, f'GATE FAIL G-ZA2 (k=0 closed form): {g2}'
...
out = {
 ...
 'part1_curvature':{
   ...
   'G-ZA1_lower_bound':'Delta_p >= chi0^2/2 (PASS; min ratio %.4f)'%rmp,
   'G-ZA2_closed_form':'Delta_p = 1-(c0*a+sqrt((1-c0^2)(1-a^2))), worst dev %.2e'%g2,
   'leading_const':'Delta_p/chi0^2 -> 1/(2(1-c0^2)); asymptotic-regime dev %.3e over n=%d'%(asym_dev,n_asym),
   ...
   'finding':'incident caps (k>=1) VIOLATE Delta_p>=chi0^2/2: LCI-reduction Sec.9 (9.4)-(9.5) chi0 criterion is INVALID for incident subsets; the good event must be parametrized by the true height-drop Delta_p (which is exactly what cap-ratio law (8.4) needs)',
 },
 'part2_capratio':{
   ...
   'note':'nu(C_p) ~ C_geom kappa^{-M} e^{-kappa Delta_p}; rate=Delta_p confirmed, M~3/2',
 },
}
...
print('  worst incident exhibit: chi0=%.3f Delta_p=%.4f ratio=%.4f' % worst_multi)
