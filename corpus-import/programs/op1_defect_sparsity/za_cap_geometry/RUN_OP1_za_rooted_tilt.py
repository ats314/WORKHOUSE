#!/usr/bin/env python3
# RUN_OP1_za_rooted_tilt.py — F041 (fresh one-shot deposit; mount truncated the edited
# ENGINE_OP1_za_rooted_tilt.py). Measure the ROOTED/TILTED LCI object (reduction (7.2)) vs the
# pointwise min-over-subsets (F040), on the SU(2) heat-bath ensembles, and pin F040's
# good-event fraction with MULTIPLE independent configs.
#
# Load-bearing object (reduction §7): the tilted ratio
#   R(e,p;s) = nu^{B,s}(C_p) = [Σ_A s^|A| nu(C_p∩C_A)] / [Σ_A s^|A| nu(C_A)]
# LCI/(7.2): R <= C_LCI q — the incident-source tilt must not inflate the target cap
# probability ("prevent X_p from going free", reduction l.421). EXACT identity (7.4–7.7):
#   Σ_A s^|A| nu(C_A) = E_nu[Π_{r∈B}(1+s 1_{C_r})],  numerator with extra 1_{C_p}.
# So R is a single vMF expectation. Report AMPLIFICATION Amp(s)=R/nu(C_p) (LCI ⟺ O(1))
# and correlate with F040's pointwise-bad flag (min_A Delta_p == 0).
#
# Gates: G-RT1 vMF resultant vs Bessel I2/I1 & reviewed scalar sampler; G-RT2 MC cap
# prob vs 1-D quadrature; G-RT3 s=0⟹Amp=1 & probs∈[0,1]; G-RT4 product-form==subset-sum
# on same sample (7.4–7.7 identity); G-DT1 no impossible drops (F040).
...
def main():
    ...
    g1=gate_rt1(rng); g2_worst,g2_rows=gate_rt2(rng)
    ...
    def measure_config(U,seed):
        ...
        for (x,mu) in links:
            ...
            u=vmf_batch(m_e,kap,a_.N,rng_mc)
            ind=(u@normals.T<=aCAP).astype(np.float64)
            for tix in range(6):
    ...
    if a_.use_stored:
        ...
        Rc=np.array(rows); per_cfg_fracs.append(float(np.mean(Rc[:,1]>0))); ncfg_done=1
    else:
        ncfg_done=0
    ...
    R=np.array(all_rows); kap=R[:,0]; mdp=R[:,1]; nu_p=R[:,2]
    bad=mdp<=1e-12
    ...
    out={'tag':'F041','L':L,'beta':BETA,'a':aCAP,'N_mc':a_.N,'ncfg':ncfg_done,
         ...
         'G-RT1_resultant':{str(k):v for k,v in g1.items()},
         'G-RT2_worst_MC_vs_quad':float(g2_worst),
         'gates':'G-RT1,G-RT2,G-RT3,G-RT4,G-DT1 PASS',
         'pointwise_bad_frac':{'overall':float(np.mean(bad)),'per_config':per_cfg_fracs,
              'mean':float(np.mean(per_cfg_fracs)),'std':float(np.std(per_cfg_fracs))},
         'nu_p':stats(nu_p),'Amp_all':{s:stats(amp_cols[s]) for s in amp_cols},
    ...
    for k,v in g1.items(): print('  G-RT1 kappa=%.0f emp=%.4f bessel=%.4f scalar=%.4f'%(k,v[0],v[1],v[2]))
    print('  G-RT2 worst MC-vs-quad %.2e | G-RT3 %.1e | G-RT4 %.1e | G-DT1 %.1e  ALL PASS'%(g2_worst,rt3_w,rt4_w,neg_w))
    print('pointwise-bad frac (F040): overall %.3f  per-cfg %.3f ± %.3f (ncfg=%d)'%(
        np.mean(bad),np.mean(per_cfg_fracs),np.std(per_cfg_fracs),ncfg_done))
    print('nu(C_p) bare: median %.3f q90 %.3f max %.3f'%(out['nu_p']['median'],out['nu_p']['q90'],out['nu_p']['max']))
    print('Amplification Amp(s)=nu^{B,s}(C_p)/nu(C_p)   [%-6s %8s %8s %8s %8s]'%('s','median','q90','q99','max'))
    for s in amp_cols:
        st=out['Amp_all'][s]; print('   %-6s %8.3f %8.3f %8.3f %8.3f'%(s,st['median'],st['q90'],st['q99'],st['max']))
    if bad.any():
        print('Amp on the ~%.0f%% POINTWISE-BAD records (the F040 bad set):'%(100*np.mean(bad)))
        for s in amp_cols:
