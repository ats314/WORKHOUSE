#!/usr/bin/env python3
"""Canonical SU(3) one-flux O(u^4) production runner for Colab.

This file packages the byte-identical June-13 authoritative construction
sources and adds only fail-closed orchestration around them: an exact two-W
half-history schedule token, the independently audited ``u=beta_H/6``
normalization identity, fresh-run binding, deterministic provenance hashes,
and a pre-unblind certificate containing the computed 189-entry H4 kernel.

The construction worker is CPU exact arithmetic.  It does not use an A100.
No historical fourth-order reference is present in, or available to, the
construction path.
"""
...
RUNNER_VERSION = "2026-08-19-canonical-o4-production-v1"
SEALED_SOURCE_FD_ENV = "HODGE_Y4_SEALED_SOURCE_FD"
...
UPSTREAM_ARCHIVE_SHA256 = (
    "22ed81066bdb35f4b31ce7333d7c8865a6fbaee9c752f7e9b19e5b9c9fda3ca7"
)
UPSTREAM_COMPLETE_SCRIPT_SHA256 = (
    "1012a7b45a63e63bb2c1924ba9cb16f03cc336a1adbea51e876c99bafab57795"
)
NORMALIZATION_AUDIT_SHA256 = (
    "d7d66c62250cd4f2f969c4ce43bb6ffcfd6aff832b12692ba390946c51cc63f9"
)
V24C_AUTHORITY_SHA256 = (
    "935a3a5ba680d1373a5842486b10231d83232d8cb3393bbc250351bc51a68c8b"
)
# One-way commitment to the exact terminal-only reference envelope.  The
# construction module contains no historical coefficient or reversible payload.
HISTORICAL_REFERENCE_ENVELOPE_SHA256 = (
    "a85b5eca065fba3d41278cfddc5d8a3b7d6c1462c29d4c765a7c5ad266d8d329"
)
HISTORICAL_REFERENCE_SCHEMA = "hodge-y4-historical-q3-reference-v1"
HISTORICAL_REFERENCE_LABEL = "historical SU(3) 189-kernel q3 reference"
HISTORICAL_REFERENCE_VARIABLE = "u=beta_H/6=1/g_H^4"
HISTORICAL_REFERENCE_AUTHORITY_LOCATOR = (
    "v24c line 7311; canonical v1.4 master lines 443-458"
)

EMBEDDED_SOURCE_SHA256 = {
    "STAGE0_SOURCE": "914f5a36f6e66b74275e9cc7cc25a16201b263f008fd4f76ee4746bfcc10a655",
    "STAGE12_SOURCE": "ff61f29a0bf9838b1aab43c56ebaf5cc6b948254734623e4f2e0ddf7d6c1d9f1",
    "FINAL_SOURCE": "747ea03d4a71583211c23ff057ccde8cc37436aa451c80ad52d0beee0ff471f5",
}

INNER_SOURCE_SHA256 = {
    "y4_stage3b_stage3c_autobundle.py": (
        "4f6e3f2a7291a2753c29e9053fd79759679c07db52f674d99dedc1dcfa88af17"
    ),
    "y4_stage3e_trace_wiring_compiler.py": (
        "b5430622f948ccd4d176b57c5c92703f974e0317a3ba83aa69c6da150381aae2"
    ),
    "y4_stage3g_checkpointed.py": (
        "7431c8444032778d55d7f0edcdb6a4dd210b9af75305e1f5fb60ab010d50aad1"
    ),
    "y4_stage3i_complete_folded_descloizeaux.py": (
        "7662066c5516533ba531c414fc057c0f5c6e0d8bd1fe860a9c083fa8b2907abf"
    ),
    "y4_stage3j_final_flatband_verdict.py": (
        "3ef8b62effa9a66675a39e2de4ba1934b8c8607a85f0f9ebc4070fec9ebad9b6"
    ),
}

DECODED_STAGE_SOURCE_SHA256 = {
    "stage1": "e5052448852a0cdbe75f2154c6882b13ede0c7aaf07f975dbf99c5398e27d485",
    "stage2": "36f593f99baa78552591f13c65b6ed782ea84eda985c27941748c71df377893a",
}

DECODED_STAGE3BC_SOURCE_SHA256 = {
    "stage3b": "3223e740cc8cab160f67f75278c4406a8bc6649ff1e7607351e2b16eabf99484",
    "stage3c": "3e058ba3c470a09d8d9bbf8fd534e477d393e648a4a1e3859e0b46b2989a889d",
}

EXPECTED_CORE_ARTIFACT_SHA256 = {
    "Y4_STAGE1/y4_channel_denominator_manifest.json.gz": (
        "a39d737be7257e80ffd9f89c3e4f83c60eba1e0912d70296c060bdb812f86737"
    ),
    "Y4_STAGE3E/y4_trace_wiring_orbits.json.gz": (
        "3a96a67ca213394774daf21ef4dc840c1a7086d5d5d6891f71b78cd3edfaf49a"
    ),
    "Y4_STAGE3G/y4_exact_local_path_tensors.json.gz": (
        "93b1f4238cb528eed0073bcf0aa9eb3720b2f12efb1862a934906319c23ff726"
    ),
    "Y4_STAGE3I/y4_complete_folded_word_weights.json.gz": (
        "854a02e981098de7fcfd1a14dd5c9703aff0c36a2a81ea5589ddf4ff8c321bd0"
    ),
    "Y4_STAGE3J/DATA_Y4_full_real_space_h4_kernel.json.gz": (
        "635d40fa8a5d7da841fd30f36185eb96f14ec4c040678ddd8fb010379afb2900"
    ),
    "Y4_STAGE3J/y4_cube_boundary_residual.json": (
        "54f287d3bca14393b17ef5c63a358e87e4a0e8acffeee97b33e2c476183829ba"
    ),
}

# Every raw archive below is path-bearing, either directly or through an
# upstream raw-hash cascade.  Raw hashes are retained as provenance locators,
# never as cross-root scientific gates.
PORTABLE_RAW_HASH_GATES: tuple[str, ...] = ()

EXPECTED_PHYSICAL_IDENTITY_SHA256 = {
    "stage_i_word_weights": (
        "5a993a94b5802d0f5decad7e847990cc743b6e3204d318b495ef6e2106f04cc6"
    ),
    "kernel": "797f03063caf06eba2aaaf6cf6c553a165d7642e4d169e9030fc7096fbe7dd9b",
    "cube_boundary_residual": (
        "e1bd67e0d6daee37025e6a74e7751ef9709466f5c215cb1391c5d51767cad74d"
    ),
}

EXPECTED_COUNTS = {
    "connected_supports": 182440,
    "ordered_words": 4221,
    "orientation_orbits": 16835,
    "topologies": 3895,
    "fusion_tree_paths": 35979,
    "nonresonant_topologies": 1478,
    "mixed_topologies": 1541,
    "all_resonant_topologies": 876,
    "kernel_entries": 189,
}

EXPECTED_BASIS_PLANES = ((0, 1), (0, 2), (1, 2))
EXPECTED_H4_OPERATOR_IDENTITY = (
    "H4=PVRVRVRVP-a(PVR2VRVP+PVRVR2VP)+a2PVR3VP-1/2{PVRVP,PVR2VP}"
)

_FORBIDDEN_WORKER_MARKERS = (
    "Gelf" "and",
    "Factor" "52",
    "local_" "shift",
    "M4_" "ORACLE",
...
class ProductionFailure(RuntimeError):
    """Fail-closed production orchestration error."""


class ScheduleViolation(ProductionFailure):
    """An attempted transition outside the sealed O(u^4) half history."""
...
def _atomic_write_json(path: Path, obj: object) -> str:
...
def _atomic_write_bytes(path: Path, raw: bytes) -> str:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_name(f".{path.name}.{os.getpid()}.{time.time_ns()}.tmp")
    with tmp.open("xb") as handle:
        handle.write(raw)
...
def _assert_finite_tree(value: object, where: str = "root") -> None:
    if isinstance(value, float):
        if not (value == value and abs(value) != float("inf")):
            raise ProductionFailure(f"non-finite value at {where}: {value!r}")
    elif isinstance(value, dict):
        for key, item in value.items():
            _assert_finite_tree(item, f"{where}.{key}")
    elif isinstance(value, (list, tuple)):
        for index, item in enumerate(value):
            _assert_finite_tree(item, f"{where}[{index}]")


def _normalization_certificate() -> dict[str, object]:
    # beta * (1 - ReTr/3), ReTr=(chi+chibar)/2, with the constant removed.
    character_coefficient_per_beta = -Fraction(1, 3) * Fraction(1, 2)
    u_per_beta = Fraction(1, 6)
    worker_orientation_coefficient = Fraction(-1, 1)
    worker_character_coefficient_per_beta = (
        u_per_beta * worker_orientation_coefficient
    )
    if character_coefficient_per_beta != Fraction(-1, 6):
        raise ProductionFailure("magnetic Hamiltonian reduction failed")
    if worker_character_coefficient_per_beta != character_coefficient_per_beta:
        raise ProductionFailure("worker insertion normalization is not canonical u")
    return {
        "audit_sha256": NORMALIZATION_AUDIT_SHA256,
        "audit_locator": "AUDIT_SU3_glueball_coupling_normalization_2026-06-14.md",
        "hamiltonian": "H_beta=H0+beta*sum_p(1-ReTr(U_p)/3)",
        "constant_dropped_character_term": "-(beta/6)*sum_p(chi_p+chibar_p)",
        "canonical_variable": "u=beta_H/6=1/g_H^4",
        "worker_operator": "W=-sum_p(chi_p+chibar_p)",
        "series_identity": "H=H0+u*W",
        "character_coefficient_per_beta": "-1/6",
        "worker_character_coefficient_per_beta": "-1/6",
        "y_code_equals_u": True,
        "rescaling": "none",
        "passed": True,
    }
...
class _ScheduleToken:
    manifest: tuple[tuple[int, str, str, str, str, str, int], ...]
    manifest_sha256: str
    sentinel: object


def _schedule_manifest_json(
    manifest: tuple[tuple[int, str, str, str, str, str, int], ...],
...
class HalfHistorySchedule:
    """Typed abstract launch-policy callback guard; not a worker execution trace."""
    ...
    def __init__(self) -> None:
        self._index = 0
        self._manifest: list[tuple[int, str, str, str, str, str, int]] = []
    ...
    def transition(
        ...
        if (
            ...
            or type(source_layer) is not str
            or type(target_layer) is not str
            or type(history_depth) is not int
            or not callable(callback)
        ):
            raise ScheduleViolation(
                "ill-typed schedule transition rejected before policy callback"
            )
        if self._in_flight:
            raise ScheduleViolation(
                "reentrant schedule transition rejected before policy callback"
            )
        request = (
            operation,
            source,
        ...
        expected = (
            self.REQUIRED[self._index]
            if self._index < len(self.REQUIRED)
            else None
        )
        if request != expected:
            raise ScheduleViolation(
                json.dumps(
                    {
                        "error": "transition rejected before backend",
                        "step": self._index + 1,
                        "request": request,
                        "expected": expected,
    ...
    def seal(self) -> _ScheduleToken:
        if self._index != len(self.REQUIRED):
            raise ScheduleViolation(
                f"incomplete half history: {self._index}/{len(self.REQUIRED)}"
            )
        if sum(row[1] == "W" for row in self._manifest) != 2:
            raise ScheduleViolation("the sealed history must contain exactly two W actions")
        manifest = tuple(self._manifest)
        digest = _sha256_bytes(_canonical_json_bytes(_schedule_manifest_json(manifest)))
        return _ScheduleToken(manifest, digest, _SCHEDULE_SENTINEL)


def _build_schedule_token() -> _ScheduleToken:
    schedule = HalfHistorySchedule()
    noop = lambda: None
    schedule.transition("W", "P0", "W1", "P", "Q1", 0, noop)
    schedule.transition("R", "W1", "R1", "Q1", "Q1", 1, noop)
    schedule.transition("W", "R1", "W2", "Q1", "Q2", 1, noop)
    return schedule.seal()


def _launch_policy_certificate(schedule_token: _ScheduleToken) -> dict[str, object]:
    return {
        "path": "P0 -> W1 -> R1 -> W2",
        "w_action_count": 2,
        "manifest": _schedule_manifest_json(schedule_token.manifest),
        "manifest_sha256": schedule_token.manifest_sha256,
        "role": "fail-closed launch authorization policy",
        "mechanically_mediates_authority_worker_calls": False,
        "worker_execution_claim": "none; construction order is certified from outputs",
        "passed": True,
    }


def _extract_inner_sources() -> dict[str, str]:
    tree = ast.parse(FINAL_SOURCE, filename="<authoritative-final-bundle>")
    ...
    raise ProductionFailure("authoritative FINAL_SOURCE has no literal SOURCES map")


def _extract_stage12_sources() -> dict[str, str]:
    tree = ast.parse(STAGE12_SOURCE, filename="<authoritative-stage12-bundle>")
    ...
    for node in tree.body:
        if (
            ...
            and len(node.targets) == 1
            and isinstance(node.targets[0], ast.Name)
            and node.targets[0].id in {"STAGE1_GZ_B64", "STAGE2_GZ_B64"}
        ):
            value = ast.literal_eval(node.value)
            encoded[node.targets[0].id] = str(value)
    if set(encoded) != {"STAGE1_GZ_B64", "STAGE2_GZ_B64"}:
        raise ProductionFailure("authoritative Stage-1/2 bundle payloads are missing")
    try:
        return {
            "stage1": gzip.decompress(base64.b64decode(encoded["STAGE1_GZ_B64"])).decode(
                "utf-8"
            ),
            "stage2": gzip.decompress(base64.b64decode(encoded["STAGE2_GZ_B64"])).decode(
                "utf-8"
            ),
        }
    except Exception as exc:
        raise ProductionFailure("authoritative Stage-1/2 payload decode failed") from exc


def _extract_stage3bc_sources() -> dict[str, str]:
    bundle_name = "y4_stage3b_stage3c_autobundle.py"
    bundle = _extract_inner_sources().get(bundle_name)
    if bundle is None:
        raise ProductionFailure("authoritative Stage-3B/3C bundle is missing")
    tree = ast.parse(bundle, filename="<authoritative-stage3bc-bundle>")
    decoded: dict[str, str] = {}
    names = {
        "STAGE3B_SOURCE": "stage3b",
        "STAGE3C_SOURCE": "stage3c",
    }
    for node in tree.body:
        if (
            isinstance(node, ast.Assign)
            ...
            if not isinstance(value, str):
                raise ProductionFailure(
                    f"authoritative {node.targets[0].id} is not literal source text"
                )
            decoded[names[node.targets[0].id]] = value
    if set(decoded) != {"stage3b", "stage3c"}:
        raise ProductionFailure("authoritative Stage-3B/3C payloads are missing")
    return decoded


def _verify_embedded_authority() -> dict[str, object]:
    outer = {
        "STAGE0_SOURCE": STAGE0_SOURCE,
        "STAGE12_SOURCE": STAGE12_SOURCE,
        "FINAL_SOURCE": FINAL_SOURCE,
    }
    observed_outer = {key: _source_sha256(value) for key, value in outer.items()}
    if observed_outer != EMBEDDED_SOURCE_SHA256:
        raise ProductionFailure(
            f"embedded outer source hash mismatch: {observed_outer!r}"
        )
    inner = _extract_inner_sources()
    observed_inner = {key: _source_sha256(value) for key, value in inner.items()}
    if observed_inner != INNER_SOURCE_SHA256:
        raise ProductionFailure(
            f"embedded inner source hash mismatch: {observed_inner!r}"
        )
    decoded = _extract_stage12_sources()
    observed_decoded = {
        key: _source_sha256(value) for key, value in decoded.items()
    }
    if observed_decoded != DECODED_STAGE_SOURCE_SHA256:
        raise ProductionFailure(
            f"embedded decoded Stage-1/2 hash mismatch: {observed_decoded!r}"
        )
    decoded_stage3bc = _extract_stage3bc_sources()
    observed_stage3bc = {
        key: _source_sha256(value) for key, value in decoded_stage3bc.items()
    }
    if observed_stage3bc != DECODED_STAGE3BC_SOURCE_SHA256:
        raise ProductionFailure(
            f"embedded decoded Stage-3B/3C hash mismatch: {observed_stage3bc!r}"
        )
    for name, source in {
        **outer,
        **inner,
        ...
        **decoded_stage3bc,
        ...
        compact_lowered = "".join(lowered.split())
        for marker in _FORBIDDEN_WORKER_MARKERS:
            if "".join(marker.lower().split()) in compact_lowered:
                raise ProductionFailure(
                    f"forbidden construction marker {marker!r} in {name}"
                )
    return {
        "archive_sha256": UPSTREAM_ARCHIVE_SHA256,
        "archive_locator": "Y4_FINAL_ARCHIVE_2026-06-13.zip",
        "complete_script_sha256_from_archive_manifest": UPSTREAM_COMPLETE_SCRIPT_SHA256,
        "complete_script_locator": "scripts/y4_complete_from_scratch.py",
        "embedded_outer_sha256": observed_outer,
        "embedded_inner_sha256": observed_inner,
        "decoded_stage_source_sha256": observed_decoded,
        "decoded_stage3bc_source_sha256": observed_stage3bc,
        "stage3i_authority_sha256": INNER_SOURCE_SHA256[
            "y4_stage3i_complete_folded_descloizeaux.py"
        ],
        "stage3j_authority_sha256": INNER_SOURCE_SHA256[
            "y4_stage3j_final_flatband_verdict.py"
        ],
...
def _strict_json_loads(raw: bytes | str, label: str) -> object:
    def reject_constant(value: str) -> object:
        raise ProductionFailure(f"non-finite JSON constant in {label}: {value}")

    def unique_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
        result: dict[str, object] = {}
        for key, value in pairs:
            if key in result:
                raise ProductionFailure(f"duplicate JSON key in {label}: {key!r}")
            result[key] = value
        return result
    ...
    except ProductionFailure:
        raise
    except Exception as exc:
        raise ProductionFailure(f"malformed strict JSON in {label}") from exc
    _assert_finite_tree(value, label)
    return value


def _strict_json_gzip_loads(raw: bytes, label: str) -> object:
    try:
        decompressed = gzip.decompress(raw)
    except Exception as exc:
        raise ProductionFailure(f"malformed gzip in {label}") from exc
    return _strict_json_loads(decompressed, label)
...
def _require_exact_keys(
    ...
    if not isinstance(value, dict) or frozenset(value) != expected:
        ...
        raise ProductionFailure(
            f"{label} schema mismatch: {observed!r} != {sorted(expected)!r}"
        )
    return value


def read_json(path: Path):
    return _strict_json_loads(path.read_bytes(), str(path))


def read_json_gz(path: Path):
    return _strict_json_gzip_loads(path.read_bytes(), str(path))


def _sealed_memfd_from_bytes(source_bytes: bytes, label: str) -> tuple[int, str, int]:
    """Return a Linux kernel-sealed fd containing exactly ``source_bytes``."""
    try:
        fcntl = __import__("fcntl")
        memfd_create = os.memfd_create
        allow_sealing = os.MFD_ALLOW_SEALING
        close_on_exec = os.MFD_CLOEXEC
        ...
        raise ProductionFailure(
            "Linux memfd write-sealing is required for authoritative execution"
        ) from exc
    if os.name != "posix" or not Path("/proc/self/fd").is_dir():
        raise ProductionFailure(
            "authoritative execution requires Linux /proc/self/fd"
        )
    source_sha = _sha256_bytes(source_bytes)
    fd = -1
    try:
        fd = memfd_create(
            "hodge_y4_sealed_source",
        ...
        while remaining:
            ...
            if written <= 0:
                raise ProductionFailure(f"short write while sealing {label}")
            remaining = remaining[written:]
        os.fsync(fd)
        observed_before = os.pread(fd, len(source_bytes) + 1, 0)
        if observed_before != source_bytes:
            raise ProductionFailure(f"memfd copy mismatch before sealing {label}")
        fcntl.fcntl(fd, add_seals, required_seals)
        observed_seals = int(fcntl.fcntl(fd, get_seals))
        if observed_seals != required_seals:
            raise ProductionFailure(
                f"incomplete memfd seals for {label}: {observed_seals:#x}"
            )
        try:
            os.pwrite(fd, source_bytes[:1] or b"x", 0)
        except OSError as exc:
            if exc.errno != errno.EPERM:
                raise ProductionFailure(
                    f"unexpected sealed-memfd write error for {label}: {exc}"
                ) from exc
        else:
            raise ProductionFailure(f"sealed memfd remained writable for {label}")
        if os.pread(fd, len(source_bytes) + 1, 0) != source_bytes:
            raise ProductionFailure(f"memfd content changed while sealing {label}")
        return fd, source_sha, required_seals
...
def _verify_sealed_memfd(
    ...
    try:
        fcntl = __import__("fcntl")
        observed_seals = int(fcntl.fcntl(fd, fcntl.F_GET_SEALS))
    except (AttributeError, ImportError, OSError) as exc:
        raise ProductionFailure(f"cannot verify sealed memfd for {label}") from exc
    observed = os.pread(fd, len(source_bytes) + 1, 0)
    if (
        observed != source_bytes
        or _sha256_bytes(observed) != expected_sha256
        or observed_seals != required_seals
    ):
        raise ProductionFailure(f"sealed execution source changed for {label}")


def run_source(source: str, name: str, args: list[str], *, cwd: Path) -> None:
    source_bytes = source.encode("utf-8")
    fd, source_sha, required_seals = _sealed_memfd_from_bytes(source_bytes, name)
    try:
        path = f"/proc/self/fd/{fd}"
        cmd = [sys.executable, '-u', path, *args]
        print(
            f"\n[complete] RUN SEALED {name} SHA256={source_sha}:",
            ' '.join(cmd),
            flush=True,
        )
        ...
        environment['HODGE_Y4_SEALED_WORKER_FD'] = str(fd)
        environment['HODGE_Y4_SEALED_WORKER_SHA256'] = source_sha
        _verify_sealed_memfd(
            fd, source_bytes, source_sha, required_seals, f"{name} before launch"
        )
        subprocess.run(
            cmd,
            check=True,
        ...
        _verify_sealed_memfd(
            fd, source_bytes, source_sha, required_seals, f"{name} after launch"
        )
    finally:
        os.close(fd)
...
def valid_stage1(root: Path) -> bool:
    manifest = root / 'Y4_STAGE1' / 'y4_channel_denominator_manifest.json.gz'
    try:
        return manifest.exists() and len(read_json_gz(manifest).get('words', [])) == 4221
    except Exception:
        return False


def valid_stage2(root: Path) -> bool:
    stage2_dir = root / 'Y4_STAGE2'
    summary = stage2_dir / 'y4_stage2_summary.json'
    cards = stage2_dir / 'y4_link_tensor_cards.json.gz'
    library = stage2_dir / 'su3_local_haar_projectors.json'
    try:
        ...
        return (
            ...
            and counts.get('unique_link_token_signatures') == 182
            and counts.get('c_orbit_representative_link_tensor_occurrences') == 187632
        )
    except Exception:
        return False
...
def valid_stage12(root: Path) -> bool:
    return valid_stage1(root) and valid_stage2(root)


def valid_stage3b(root: Path) -> bool:
    paths = root / 'Y4_STAGE3B' / 'y4_local_irrep_paths.json.gz'
    try:
        data = read_json_gz(paths)
        return (
            paths.exists()
            and len(data.get('signatures', [])) == 182
            and len(data.get('irreps', [])) == 10
            and len(data.get('feasible_transfer_edges', [])) == 36
        )
    except Exception:
        return False


def valid_stage3c(root: Path) -> bool:
    stage3c_dir = root / 'Y4_STAGE3C'
    summary = stage3c_dir / 'y4_stage3c_summary.json'
    cards = stage3c_dir / 'y4_path_projector_cards.json.gz'
    carriers = stage3c_dir / 'y4_irrep_carriers.json.gz'
    projectors = stage3c_dir / 'y4_casimir_channel_projectors.json.gz'
    expected_counts = {
        'exact_irrep_carriers': 10,
        'tensor_domains': 20,
        'full_fusion_projectors': 44,
...
def _authoritative_main(root: Path, schedule_token: _ScheduleToken) -> None:
    required_manifest = _build_schedule_token()
    if (
        not isinstance(schedule_token, _ScheduleToken)
        or schedule_token.sentinel is not _SCHEDULE_SENTINEL
        or schedule_token.manifest != required_manifest.manifest
        or schedule_token.manifest_sha256 != required_manifest.manifest_sha256
    ):
        raise ScheduleViolation("authoritative backend requires the sealed two-W token")
    print('=' * 104)
    ...
    stage12_sources = _extract_stage12_sources()
    stage1_dir = root / 'Y4_STAGE1'
    stage2_dir = root / 'Y4_STAGE2'
    stage1 = stage1_dir / 'y4_channel_denominator_manifest.json.gz'
    if valid_stage1(root):
        print('[complete] Stage 1 valid; reusing.')
    else:
        run_source(
            stage12_sources['stage1'],
            'y4_stage1_channel_denominator_manifest.py',
            ['--input', str(stage0_words), '--output-dir', str(stage1_dir)],
            cwd=root,
        )
    if not valid_stage1(root):
        raise ProductionFailure('direct sealed Stage-1 validation failed')
    print('[complete] Stage 1 validation PASS')

    if valid_stage2(root):
        print('[complete] Stage 2 valid; reusing.')
    else:
        run_source(
            stage12_sources['stage2'],
            'y4_stage2_exact_haar_library.py',
            ['--input', str(stage1), '--output-dir', str(stage2_dir)],
            cwd=root,
        )
    if not valid_stage2(root):
        raise ProductionFailure('direct sealed Stage-2 validation failed')
    print('[complete] Stage 1/2 validation PASS')

    # Execute the byte-identical leaf stage sources directly through run_source.
    # The accepted autobundle bytes remain hash-bound provenance, but their
    # pathname-based child launch tails are never executed.  This preserves the
    # authoritative stage order while closing nested script-substitution races.
    # Stage-I also receives an explicit guaranteed-nonexistent Stage-3H path, so
    # its optional regression lookup cannot discover any unrelated Colab upload.
    sources = _extract_inner_sources()
    ...
    stage3c_carriers = stage3c_dir / 'y4_irrep_carriers.json.gz'
    stage3c_projectors = stage3c_dir / 'y4_casimir_channel_projectors.json.gz'
    valid_3bc = valid_stage3b(root) and valid_stage3c(root)
    if valid_3bc:
        print('[complete] Stage 3B/3C valid; reusing.')
    else:
        stage3bc_sources = _extract_stage3bc_sources()
        if valid_stage3b(root):
            print('[complete] Stage 3B valid; reusing.')
        else:
            run_source(
                stage3bc_sources['stage3b'],
                'y4_stage3b_time_ordered_state_graph.py',
                ['--input', str(stage1), '--output-dir', str(stage3b_dir)],
                cwd=root,
            )
        if not valid_stage3b(root):
            raise ProductionFailure('direct sealed Stage-3B validation failed')
        run_source(
            stage3bc_sources['stage3c'],
            'y4_stage3c_exact_casimir_projectors.py',
            ['--input', str(stage3b_paths), '--output-dir', str(stage3c_dir)],
            cwd=root,
        )
        if not valid_stage3c(root):
            raise ProductionFailure('direct sealed Stage-3C validation failed')
    if not (valid_stage3b(root) and valid_stage3c(root)):
        raise ProductionFailure('direct sealed Stage-3B/3C validation failed')

    stage3e_dir = root / 'Y4_STAGE3E'
    stage3e_summary = stage3e_dir / 'y4_stage3e_summary.json'
    stage3e_orbits = stage3e_dir / 'y4_trace_wiring_orbits.json.gz'
    valid_3e = (
        stage3e_summary.exists()
        and read_json(stage3e_summary).get('passed') is True
        and stage3e_orbits.exists()
    )
    if valid_3e:
        print('[complete] Stage 3E valid; reusing.')
    else:
        run_source(
            sources['y4_stage3e_trace_wiring_compiler.py'],
            'y4_stage3e_trace_wiring_compiler.py',
            [
                '--stage1', str(stage1),
                '--stage3c', str(stage3c_cards),
    ...
    stage3g_tensors = stage3g_dir / 'y4_exact_local_path_tensors.json.gz'
    valid_3g = (
        stage3g_summary.exists()
        and read_json(stage3g_summary).get('passed') is True
        and stage3g_tensors.exists()
    )
    if valid_3g:
        print('[complete] Stage 3G valid; reusing.')
    else:
        run_source(
            sources['y4_stage3g_checkpointed.py'],
            'y4_stage3g_checkpointed.py',
            [
                '--stage2-cards', str(cards),
                '--stage3b-paths', str(stage3b_paths),
                '--stage3c-carriers', str(stage3c_carriers),
                '--stage3c-projectors', str(stage3c_projectors),
                '--output-dir', str(stage3g_dir),
    ...
    no_stage3h = root / '_SEALED_NO_STAGE3H_INPUT.json.gz'
    if no_stage3h.exists():
        raise ProductionFailure(
            f'reserved no-Stage3H sentinel path must not exist: {no_stage3h}'
        )
    stage3i_dir = root / 'Y4_STAGE3I'
    stage3i_words = stage3i_dir / 'y4_complete_folded_word_weights.json.gz'
    run_source(
        sources['y4_stage3i_complete_folded_descloizeaux.py'],
        'y4_stage3i_complete_folded_descloizeaux.py',
        [
            '--stage1', str(stage1),
            '--stage3e', str(stage3e_orbits),
            '--stage3g', str(stage3g_tensors),
            '--stage3h-topologies', str(no_stage3h),
            '--output-dir', str(stage3i_dir),
        ],
        cwd=root,
    )
    stage3i_summary = stage3i_dir / 'y4_stage3i_summary.json'
    _require_stage3h_sealed_out(read_json(stage3i_summary), no_stage3h)

    stage3j_dir = root / 'Y4_STAGE3J'
    run_source(
        sources['y4_stage3j_final_flatband_verdict.py'],
        'y4_stage3j_final_flatband_verdict.py',
        ['--input', str(stage3i_words), '--output-dir', str(stage3j_dir)],
        cwd=root,
    )
...
def _runtime_script_path() -> Path:
    ...
    if sealed_fd_text is not None:
        if (
            ...
            raise ProductionFailure("sealed runtime source descriptor is malformed")
        path = Path("/proc/self/fd") / sealed_fd_text
        if raw_path != path:
            raise ProductionFailure(
                "runtime __file__ is not the inherited sealed source descriptor"
            )
        if not path.is_file():
            raise ProductionFailure(f"sealed runtime source is not readable: {path}")
        return path
    path = raw_path.resolve()
    if not path.is_file():
        raise ProductionFailure(f"runtime script is not a file: {path}")
    return path


def _package_versions() -> dict[str, str]:
    versions = {"python": platform.python_version()}
    for package in ("numpy", "sympy"):
        try:
            versions[package] = package_version(package)
        except PackageNotFoundError as exc:
            raise ProductionFailure(f"required package is missing: {package}") from exc
    return versions


def _binding_payload(
    script_sha256: str,
    schedule_token: _ScheduleToken,
    authority: dict[str, object],
    normalization: dict[str, object],
) -> dict[str, object]:
    return {
        "schema": "hodge-y4-canonical-run-binding-v1",
        "runner_version": RUNNER_VERSION,
        "runtime_script_sha256": script_sha256,
        "schedule_manifest_sha256": schedule_token.manifest_sha256,
        "archive_sha256": authority["archive_sha256"],
        "embedded_outer_sha256": authority["embedded_outer_sha256"],
        "embedded_inner_sha256": authority["embedded_inner_sha256"],
        "normalization_audit_sha256": normalization["audit_sha256"],
        "canonical_variable": normalization["canonical_variable"],
    }
...
def _prepare_run_root(
    ...
    if resolved in blocked:
        raise ProductionFailure(
            f"run root must be a dedicated child directory, not {resolved}"
        )
    if resolved.exists() and not resolved.is_dir():
        raise ProductionFailure(f"run root exists but is not a directory: {resolved}")
    resolved.mkdir(parents=True, exist_ok=True)
    binding_path = resolved / "Y4_CANONICAL_RUN_BINDING.json"
    existing_entries = list(resolved.iterdir())
    if binding_path.exists():
        if not resume:
            raise ProductionFailure(
                f"bound run already exists at {resolved}; pass --resume to continue it"
            )
        observed = read_json(binding_path)
        if observed != binding:
            raise ProductionFailure("resume binding does not match this script/config/source")
    elif existing_entries:
        raise ProductionFailure(
            f"refusing unbound non-empty run directory: {resolved}"
        )
    else:
...
def _write_status(
    ...
    envelope: dict[str, object] = {
        "schema": "hodge-y4-canonical-status-v1",
        "runner_version": RUNNER_VERSION,
        "status": status,
        "runtime_script_sha256": script_sha256,
        "unix_time": time.time(),
    ...
    return _atomic_write_json(root / "Y4_CANONICAL_RUN_STATUS.json", envelope)
...
def _require_passed_gates(
    ...
    if not isinstance(gates, dict) or not gates:
        raise ProductionFailure(f"{label} gate map is absent or empty")
    observed_names = frozenset(str(name) for name in gates)
    if observed_names != expected_names:
        raise ProductionFailure(
            f"{label} gate manifest mismatch: {sorted(observed_names)!r} != "
            f"{sorted(expected_names)!r}"
        )
    for name, gate in gates.items():
        if not isinstance(gate, dict) or gate.get("passed") is not True:
            raise ProductionFailure(f"{label} gate did not pass: {name}")


def _require_stage_i_formula_and_lineage(
    stage_i: dict[str, object],
    stage1_raw_sha256: str,
) -> None:
    try:
        gates = stage_i["gates"]
        formula = gates["I0_folded_formula"]["operator_identity"]
        stage1_bound = gates["I1_dependencies"]["stage1_sha256"]
    except Exception as exc:
        raise ProductionFailure("Stage-I formula/lineage fields are missing") from exc
    if formula != EXPECTED_H4_OPERATOR_IDENTITY or stage1_bound != stage1_raw_sha256:
        raise ProductionFailure("Stage-I formula or Stage-1 lineage is not canonical")


def _require_stage3h_sealed_out(stage_i: object, sentinel: Path) -> None:
    if sentinel.exists():
        raise ProductionFailure(f"reserved Stage-3H sentinel unexpectedly exists: {sentinel}")
    if not isinstance(stage_i, dict):
        raise ProductionFailure("Stage-I summary is malformed")
    counts = stage_i.get("counts", {})
    gates = stage_i.get("gates", {})
    i2 = gates.get("I2_complete_topology_contraction", {}) if isinstance(gates, dict) else {}
    if (
        not isinstance(counts, dict)
        or counts.get("stage3h_regression_topologies") != 0
        or not isinstance(i2, dict)
        or i2.get("stage3h_topologies_regressed") != 0
    ):
        raise ProductionFailure("Stage-I was not constructed with Stage-3H sealed out")
...
def _load_stage_i_word_payload_bytes(raw: bytes) -> dict[str, object]:
    payload = _strict_json_gzip_loads(raw, "Stage-I word weights")
    if not isinstance(payload, dict) or set(payload) != {"meta", "words"}:
        raise ProductionFailure("Stage-I word payload has an unexpected schema")
    ...
    if not isinstance(meta, dict) or set(meta) != {
        ...
        raise ProductionFailure("Stage-I word metadata has an unexpected schema")
    if meta.get("version") != "2026-06-13-stage3i-v1":
        raise ProductionFailure("unexpected Stage-I word version")
    if meta.get("orbit_file") != "y4_complete_folded_orbit_amplitudes.json.gz":
        raise ProductionFailure("unexpected Stage-I orbit filename")
    if not _is_sha256(meta.get("orbit_sha256")):
        raise ProductionFailure("Stage-I orbit hash is malformed")
    if meta.get("rooted_multiplicity") != (
        "orbit size under the 8-element proper cubic stabilizer of the rooted input "
        "plaquette"
    ):
        raise ProductionFailure("unexpected Stage-I rooted-multiplicity convention")
    if not isinstance(words, list) or len(words) != EXPECTED_COUNTS["ordered_words"]:
        raise ProductionFailure(
            f"Stage-I word count is {len(words) if isinstance(words, list) else None!r}, "
            f"expected {EXPECTED_COUNTS['ordered_words']}"
        )
    if any(not isinstance(record, dict) for record in words):
        raise ProductionFailure("Stage-I words contain a malformed record")
    _assert_finite_tree(payload, "stage_i_word_weights")
    return payload


def _stage_i_word_physical_identity(
    payload: dict[str, object],
) -> tuple[str, dict[str, object]]:
    meta = payload["meta"]
    words = payload["words"]
    identity = {
        "schema": "hodge-y4-stage3i-word-weights-physical-v1",
        "metadata": {
            "version": meta["version"],
            "rooted_multiplicity": meta["rooted_multiplicity"],
        },
        "word_count": len(words),
        "words": words,
    }
    return _sha256_bytes(_canonical_json_bytes(identity)), identity


def _load_stage0_ordered_payload_bytes(raw: bytes) -> dict[str, object]:
    payload = _strict_json_gzip_loads(raw, "Stage-0 ordered words")
    payload = _require_exact_keys(
        payload, frozenset({"meta", "words"}), "Stage-0 ordered words"
    )
        ...
        frozenset(
            {
                ...
                "root",
                "word_length",
                "proper_rotation_quotient",
                "reflection_quotient",
                "scope",
            }
        ),
        "Stage-0 ordered-word metadata",
    )
    if (
        meta.get("version") != "2026-06-13-stage0-v1"
        or meta.get("root") != [0, 0, 0, 0, 1]
        or meta.get("word_length") != 4
        or meta.get("proper_rotation_quotient") is not True
        or meta.get("reflection_quotient") is not False
    ):
        raise ProductionFailure("Stage-0 ordered-word convention is not canonical")
    words = payload["words"]
    if not isinstance(words, list) or len(words) != EXPECTED_COUNTS["ordered_words"]:
        raise ProductionFailure("Stage-0 ordered-word corpus is incomplete")
    return payload


def _load_stage1_manifest_payload_bytes(raw: bytes) -> dict[str, object]:
    payload = _strict_json_gzip_loads(raw, "Stage-1 channel manifest")
    payload = _require_exact_keys(
        payload, frozenset({"meta", "words"}), "Stage-1 channel manifest"
    )
    meta = _require_exact_keys(
        payload["meta"],
        frozenset(
            {
                ...
                "stage0_sha256",
                "energy_convention",
                "resolvent_convention",
                "representation_path_multiplicity_is_not_amplitude",
            }
        ),
        "Stage-1 channel metadata",
    )
    if (
        meta.get("version") != "2026-06-13-stage1-v1"
        or meta.get("energy_convention") != "E6=6E; E0_6=16"
        or meta.get("resolvent_convention") != "1/(E0-Ej)=6/(16-E6_j)"
        or meta.get("representation_path_multiplicity_is_not_amplitude") is not True
        or not isinstance(meta.get("stage0_input"), str)
        or not _is_sha256(meta.get("stage0_sha256"))
    ):
        raise ProductionFailure("Stage-1 channel convention is not canonical")
    words = payload["words"]
    if not isinstance(words, list) or len(words) != EXPECTED_COUNTS["ordered_words"]:
        raise ProductionFailure("Stage-1 word corpus is incomplete")
    return payload


def _validate_four_insertion_chain(
    stage0_payload: dict[str, object],
    stage1_payload: dict[str, object],
    stage_i_payload: dict[str, object],
    *,
    expected_word_count: int = EXPECTED_COUNTS["ordered_words"],
    expected_moment_occurrences: dict[tuple[int, int, int], int] | None = None,
    expected_corpus_totals: tuple[int, int, int, int, int, int] = (
        33738,
        16869,
        16835,
        34,
    ...
    if expected_moment_occurrences is None:
        expected_moment_occurrences = {
            (0, 3, 1): 720,
            (0, 6, 5): 24,
            (1, 1, 1): 173328,
            (2, 2, 2): 13140,
    ...
    stage0_words = stage0_payload.get("words")
    stage1_words = stage1_payload.get("words")
    stage_i_words = stage_i_payload.get("words")
    if not all(isinstance(words, list) for words in (stage0_words, stage1_words, stage_i_words)):
        raise ProductionFailure("ordered-word chain is malformed")
    if not (
        len(stage0_words) == len(stage1_words) == len(stage_i_words) == expected_word_count
    ):
        raise ProductionFailure("ordered-word chain does not have exact complete cardinality")

    stage0_keys = frozenset(
        {
            "ordered_id",
            "root",
            "ordered_insertions",
            "output",
            "triality_sign_assignment_count",
            "triality_sign_assignments",
        }
    )
    stage1_keys = frozenset(
        {
            "ordered_id",
            "root",
            "ordered_insertions",
            ...
            "has_link_sharing_contact",
            "has_site_only_corner_contact",
            "triality_sign_orbits",
            "exact_singlet_orbits",
            "rejected_sign_orbits",
            "word_max_link_degree",
            "word_max_local_invariant_dim",
            "word_max_energy_signature_count",
            "word_orbits_with_resonance",
            "word_nonresonant_only_orbits",
            "complexity_score",
            "orientation_orbits",
        }
    )
    stage_i_keys = frozenset(
        {
            "canonical_complete_sum_even",
            "canonical_complete_sum_odd",
            "canonical_direct_sum_even",
            "canonical_direct_sum_odd",
            "canonical_folded_sum_even",
            "canonical_folded_sum_odd",
            "ordered_id",
            "ordered_insertions",
            "orientation_orbit_count",
            "output",
            "output_displacement_plane",
            "rooted_complete_weight_even",
            "rooted_complete_weight_odd",
            "rooted_cubic_multiplicity_even",
            "rooted_cubic_signed_multiplicity_odd",
            "topology_count",
        }
    )
    orbit_keys = frozenset(
        {
            ...
            "sign_representative",
            "charge_conjugate",
            "charge_conjugation_multiplicity",
            "c_even_phase",
            "c_odd_phase",
            "resonance_class",
            "exact_singlet_feasible",
            "touched_links",
            "max_link_degree",
            "max_local_invariant_dim",
            "global_channel_path_multiplicity",
            "local_moment_types",
            "link_token_signatures",
            "energy_signature_count",
            "resonant_energy_signatures",
            "nonresonant_energy_signatures",
            "vacuum_energy_signatures",
            "denominator_signatures",
        }
    )
    allowed_moments = frozenset(expected_moment_occurrences)
    observed_moments: dict[tuple[int, int, int], int] = {}
    signature_count = 0
    unique_signatures: set[tuple[int, ...]] = set()
    denominator_signature_count = 0
    stage0_assignment_count = 0
    stage1_charge_conjugation_orbits = 0
    stage1_exact_singlet_orbits = 0
    stage1_rejected_orbits = 0
    maximum_link_degree = 0
    maximum_invariant_dimension = 0
    for index, (stage0_word, stage1_word, stage_i_word) in enumerate(
        zip(stage0_words, stage1_words, stage_i_words), start=1
    ):
        stage0_word = _require_exact_keys(stage0_word, stage0_keys, "Stage-0 word")
        stage1_word = _require_exact_keys(stage1_word, stage1_keys, "Stage-1 word")
        stage_i_word = _require_exact_keys(stage_i_word, stage_i_keys, "Stage-I word")
        expected_id = f"W4-{index:05d}"
        shared0 = (
            stage0_word.get("ordered_id"),
            stage0_word.get("ordered_insertions"),
            stage0_word.get("output"),
        )
        shared1 = (
            stage1_word.get("ordered_id"),
            stage1_word.get("ordered_insertions"),
            stage1_word.get("output"),
        )
        shared_i = (
            stage_i_word.get("ordered_id"),
            stage_i_word.get("ordered_insertions"),
            stage_i_word.get("output"),
        )
        if shared0 != shared1 or shared1 != shared_i or shared0[0] != expected_id:
            raise ProductionFailure(f"ordered-word chain diverges at {expected_id}")
        if (
            stage0_word.get("root") != [0, 0, 0, 0, 1]
            or stage1_word.get("root") != [0, 0, 0, 0, 1]
            or not isinstance(shared0[1], list)
            or len(shared0[1]) != 4
        ):
            raise ProductionFailure(f"non-four-insertion word at {expected_id}")
        triality_assignments = stage0_word.get("triality_sign_assignments")
        if (
            not isinstance(triality_assignments, list)
            or stage0_word.get("triality_sign_assignment_count")
            != len(triality_assignments)
            or any(
                not isinstance(assignment, list)
                or len(assignment) != 6
                or any(type(value) is not int or value not in (-1, 1) for value in assignment)
                for assignment in triality_assignments
            )
        ):
            raise ProductionFailure(
                f"Stage-0 factor ordering is malformed at {expected_id}"
            )
        stage0_assignment_count += len(triality_assignments)
        for plaquette in [*shared0[1], shared0[2]]:
            if (
                not isinstance(plaquette, list)
                or len(plaquette) != 5
                or any(type(value) is not int for value in plaquette)
            ):
                raise ProductionFailure(f"malformed plaquette at {expected_id}")
        if (
            type(stage1_word.get("word_max_link_degree")) is not int
            or type(stage1_word.get("word_max_local_invariant_dim")) is not int
            or stage1_word["word_max_link_degree"] > 6
            or stage1_word["word_max_local_invariant_dim"] > 6
        ):
            raise ProductionFailure(f"local occurrence bound exceeded at {expected_id}")
        maximum_link_degree = max(maximum_link_degree, stage1_word["word_max_link_degree"])
        maximum_invariant_dimension = max(
            maximum_invariant_dimension, stage1_word["word_max_local_invariant_dim"]
        )
        orbits = stage1_word.get("orientation_orbits")
        if not isinstance(orbits, list):
            raise ProductionFailure(f"orientation orbits are malformed at {expected_id}")
        triality_orbits = stage1_word.get("triality_sign_orbits")
        exact_orbits = stage1_word.get("exact_singlet_orbits")
        rejected_orbits = stage1_word.get("rejected_sign_orbits")
        if (
            type(triality_orbits) is not int
            or type(exact_orbits) is not int
            or type(rejected_orbits) is not int
            or 2 * triality_orbits != len(triality_assignments)
            or exact_orbits != len(orbits)
            or rejected_orbits != triality_orbits - exact_orbits
            or stage_i_word.get("orientation_orbit_count") != len(orbits)
        ):
            raise ProductionFailure(f"Stage-I orbit count diverges at {expected_id}")
        stage1_charge_conjugation_orbits += triality_orbits
        stage1_exact_singlet_orbits += exact_orbits
        stage1_rejected_orbits += rejected_orbits
        for orbit in orbits:
            orbit = _require_exact_keys(orbit, orbit_keys, "Stage-1 orientation orbit")
            if orbit.get("exact_singlet_feasible") is not True:
                raise ProductionFailure("an infeasible orbit entered the Stage-1 manifest")
            representative = orbit.get("sign_representative")
            conjugate = orbit.get("charge_conjugate")
            if (
                not isinstance(representative, list)
                or not isinstance(conjugate, list)
                or len(representative) != 6
                or len(conjugate) != 6
                or any(type(value) is not int or value not in (-1, 1) for value in representative)
                or conjugate != [-value for value in representative]
                or orbit.get("charge_conjugation_multiplicity") != 2
            ):
                raise ProductionFailure("charge-conjugation orbit factor order is malformed")
            signatures = orbit.get("link_token_signatures")
            if not isinstance(signatures, list):
                raise ProductionFailure("link-token signatures are malformed")
            for signature in signatures:
                if (
                    not isinstance(signature, list)
                    or len(signature) != 6
                    or any(type(value) is not int or value not in (-1, 0, 1) for value in signature)
                ):
                    raise ProductionFailure(
                        "local signature is not ket + four insertions + bra"
                    )
                signature_count += 1
                unique_signatures.add(tuple(signature))
            moments = orbit.get("local_moment_types")
            if not isinstance(moments, list):
                raise ProductionFailure("local-moment manifest is malformed")
            for moment in moments:
                moment = _require_exact_keys(
                    moment,
                    frozenset(
                        {
                            "n_fund_canonical",
                            "n_antifund_canonical",
                            "singlet_multiplicity",
                            "link_count",
                        }
                    ),
                    "Stage-1 local moment",
                )
                key = (
                    moment["n_fund_canonical"],
                    moment["n_antifund_canonical"],
                    moment["singlet_multiplicity"],
                )
                if key not in allowed_moments or type(moment["link_count"]) is not int:
                    raise ProductionFailure(f"forbidden local occurrence family: {key!r}")
                observed_moments[key] = observed_moments.get(key, 0) + moment["link_count"]
            denominators = orbit.get("denominator_signatures")
            if not isinstance(denominators, list):
                raise ProductionFailure("denominator signatures are malformed")
            for denominator in denominators:
                denominator_signature_count += 1
                    ...
                    frozenset(
                        {
                            ...
                            "channel_path_multiplicity",
                            "resonant_mask",
                            "vacuum_mask",
                            "raw_resolvent_product",
                            "requires_folded_terms",
                        }
                    ),
                    "Stage-1 denominator signature",
                )
                if (
                    not isinstance(denominator.get("E6"), list)
                    or len(denominator["E6"]) != 3
                    or not isinstance(denominator.get("denominator_numerators"), list)
                    or len(denominator["denominator_numerators"]) != 3
                    or any(type(value) is not int for value in denominator["E6"])
                    or any(
                        type(value) is not int
                        for value in denominator["denominator_numerators"]
                    )
                    or denominator["denominator_numerators"]
                    != [16 - value for value in denominator["E6"]]
                    or denominator.get("resonant_mask")
                    != sum(
                        (value == 16) << position
                        for position, value in enumerate(denominator["E6"])
                    )
                    or denominator.get("vacuum_mask")
                    != sum(
                        (value == 0) << position
                        for position, value in enumerate(denominator["E6"])
                    )
                    ...
                    raise ProductionFailure(
                        "Stage-1 word does not have exactly three intermediate denominators"
                    )
            if orbit.get("energy_signature_count") != len(denominators):
                raise ProductionFailure("Stage-1 energy-signature count diverges")
    if observed_moments != expected_moment_occurrences:
        raise ProductionFailure(
            f"local occurrence census mismatch: {observed_moments!r}"
        )
    if maximum_link_degree != 6 or maximum_invariant_dimension != 6:
        raise ProductionFailure("local occurrence maxima did not reproduce")
    (
        expected_assignments,
        expected_charge_orbits,
        expected_exact_orbits,
        expected_rejects,
        expected_unique_signatures,
        expected_denominators,
    ) = expected_corpus_totals
    if (
        stage0_assignment_count != expected_assignments
        or stage1_charge_conjugation_orbits != expected_charge_orbits
        or stage1_exact_singlet_orbits != expected_exact_orbits
        or stage1_rejected_orbits != expected_rejects
        or signature_count != sum(expected_moment_occurrences.values())
        or len(unique_signatures) != expected_unique_signatures
        or denominator_signature_count != expected_denominators
    ):
        raise ProductionFailure("four-insertion corpus totals did not reproduce")
    return {
        "schema": "hodge-y4-four-insertion-output-certificate-v1",
        "evidence_kind": "output-derived; not a claim of direct W-call mediation",
        "ordered_word_count": expected_word_count,
        "insertion_count_per_word": 4,
        "factor_count_per_local_signature": 6,
        "intermediate_denominator_count_per_channel": 3,
        "factor_order": ["ket", "insertion_1", "insertion_2", "insertion_3", "insertion_4", "bra"],
        "fifth_insertion_or_fourth_intermediate_cut_present": False,
        "stage0_triality_assignments": stage0_assignment_count,
        "stage1_charge_conjugation_orbits": stage1_charge_conjugation_orbits,
        "stage1_exact_singlet_orbits": stage1_exact_singlet_orbits,
        "stage1_rejected_orbits": stage1_rejected_orbits,
        "local_occurrence_families": [list(key) for key in sorted(observed_moments)],
        "local_occurrence_counts": {
            str(key): value for key, value in sorted(observed_moments.items())
        },
        "maximum_link_degree": maximum_link_degree,
        "maximum_local_invariant_dimension": maximum_invariant_dimension,
        "link_token_signature_count": signature_count,
        "unique_link_token_signatures": len(unique_signatures),
        "denominator_signature_count": denominator_signature_count,
        "passed": True,
    }


def _load_kernel_payload_bytes(
    raw: bytes,
    expected_stage_i_raw_sha256: str | None = None,
) -> dict[str, object]:
    payload = _strict_json_gzip_loads(raw, "Stage-J kernel")
    payload = _require_exact_keys(
        payload, frozenset({"meta", "kernel"}), "Stage-J kernel"
    )
    meta = _require_exact_keys(
        payload.get("meta"),
        frozenset(
            {"version", "stage3i_input", "stage3i_sha256", "basis_planes"}
        ),
        "Stage-J kernel metadata",
    )
    if meta.get("version") != "2026-06-13-stage3j-v1":
        raise ProductionFailure("unexpected Stage-J kernel version")
    if not isinstance(meta.get("stage3i_input"), str) or not meta["stage3i_input"]:
        raise ProductionFailure("kernel Stage-I input locator is malformed")
    basis = meta.get("basis_planes")
    if basis != [list(plane) for plane in EXPECTED_BASIS_PLANES]:
        raise ProductionFailure(f"unexpected kernel basis/order: {basis!r}")
    stage_i_raw_sha256 = meta.get("stage3i_sha256")
    if not _is_sha256(stage_i_raw_sha256):
        raise ProductionFailure("kernel Stage-I raw-hash binding is malformed")
    if (
        expected_stage_i_raw_sha256 is not None
        and stage_i_raw_sha256 != expected_stage_i_raw_sha256
    ):
        raise ProductionFailure(
            "kernel is not bound to the Stage-I weights produced in this run"
        )
    records = payload["kernel"]
    if not isinstance(records, list):
        raise ProductionFailure("kernel records are malformed")
    if len(records) != EXPECTED_COUNTS["kernel_entries"]:
        raise ProductionFailure(f"kernel record count is {len(records)}, expected 189")
    return payload


def _load_kernel_records(kernel_path: Path) -> list[dict[str, object]]:
    return _load_kernel_payload_bytes(kernel_path.read_bytes())["kernel"]


def _verify_kernel_hermiticity(records: list[dict[str, object]]) -> None:
    kernel: dict[tuple[tuple[int, int], tuple[int, int], tuple[int, int, int]], Fraction] = {}
    for record in records:
        try:
            input_plane = tuple(int(x) for x in record["input_plane"])
            output_plane = tuple(int(x) for x in record["output_plane"])
            displacement = tuple(int(x) for x in record["displacement"])
            weight = Fraction(str(record["weight"]))
        except Exception as exc:
            raise ProductionFailure(f"malformed kernel record: {record!r}") from exc
        if len(input_plane) != 2 or len(output_plane) != 2 or len(displacement) != 3:
            raise ProductionFailure(f"bad kernel key shape: {record!r}")
        if input_plane not in EXPECTED_BASIS_PLANES or output_plane not in EXPECTED_BASIS_PLANES:
            raise ProductionFailure(f"kernel record uses an unknown plane: {record!r}")
        if weight == 0:
            raise ProductionFailure("zero entry is present in sparse kernel")
        key = (input_plane, output_plane, displacement)
        if key in kernel:
            raise ProductionFailure(f"duplicate kernel key: {key!r}")
        kernel[key] = weight
    for (input_plane, output_plane, displacement), weight in kernel.items():
        adjoint = (
            output_plane,
            input_plane,
            tuple(-value for value in displacement),
        )
        if kernel.get(adjoint) != weight:
            raise ProductionFailure(f"kernel is not exactly Hermitian at {adjoint!r}")


def _gamma_scalar_from_records(
    records: list[dict[str, object]],
) -> tuple[list[list[Fraction]], Fraction]:
    """Compute H4(Gamma)=sum_r K(r) and require a scalar 3x3 matrix."""
    plane_index = {
        plane: index for index, plane in enumerate(EXPECTED_BASIS_PLANES)
    }
    ...
    for record in records:
        try:
            ...
            output_plane = tuple(int(x) for x in record["output_plane"])
            weight = Fraction(str(record["weight"]))
            matrix[plane_index[output_plane]][plane_index[input_plane]] += weight
        except Exception as exc:
            raise ProductionFailure(f"malformed Gamma kernel record: {record!r}") from exc
    off_diagonal = [
        matrix[row][column]
        for row in range(3)
    ...
    if any(value != 0 for value in off_diagonal):
        raise ProductionFailure(f"Gamma kernel has nonzero off-diagonal entries: {matrix!r}")
    if len(set(diagonal)) != 1:
        raise ProductionFailure(f"Gamma kernel is not scalar: {matrix!r}")
    return matrix, diagonal[0]


def _observe_and_gate_artifact_hashes(root: Path) -> dict[str, str]:
    observed: dict[str, str] = {}
    for relative, expected in EXPECTED_CORE_ARTIFACT_SHA256.items():
        path = root / relative
        if not path.is_file():
            raise ProductionFailure(f"missing canonical artifact: {relative}")
        digest = sha256_file(path)
        if relative in PORTABLE_RAW_HASH_GATES and digest != expected:
            raise ProductionFailure(
                f"portable canonical artifact hash mismatch for {relative}: "
                f"{digest} != {expected}"
            )
...
def _kernel_physical_identity(
    payload: dict[str, object],
    stage_i_word_physical_sha256: str | None = None,
) -> tuple[str, dict[str, object]]:
    meta = payload["meta"]
    stable_upstream = (
        stage_i_word_physical_sha256
        if stage_i_word_physical_sha256 is not None
        else EXPECTED_PHYSICAL_IDENTITY_SHA256["stage_i_word_weights"]
    )
    if not _is_sha256(stable_upstream):
        raise ProductionFailure("Stage-I portable physical identity is malformed")
    identity = {
        "schema": "hodge-y4-kernel-physical-v2",
        "basis_planes": meta["basis_planes"],
        "stage_i_word_weights_physical_identity_sha256": stable_upstream,
        "records": payload["kernel"],
    }
    return _sha256_bytes(_canonical_json_bytes(identity)), identity


def _validate_stage_i_kernel_binding_bytes(
    stage_i_raw: bytes,
    kernel_raw: bytes,
    *,
    expected_stage_i_physical_sha256: str = EXPECTED_PHYSICAL_IDENTITY_SHA256[
        "stage_i_word_weights"
    ],
    expected_kernel_physical_sha256: str = EXPECTED_PHYSICAL_IDENTITY_SHA256["kernel"],
) -> tuple[dict[str, object], dict[str, object], str, str, str]:
    """Validate one run's raw linkage and both cross-root physical identities."""
    stage_i_raw_sha256 = _sha256_bytes(stage_i_raw)
    stage_i_payload = _load_stage_i_word_payload_bytes(stage_i_raw)
    stage_i_physical_sha256, _stage_i_identity = _stage_i_word_physical_identity(
        stage_i_payload
    )
    if stage_i_physical_sha256 != expected_stage_i_physical_sha256:
        raise ProductionFailure(
            "Stage-I word physical identity mismatch: "
            f"{stage_i_physical_sha256} != {expected_stage_i_physical_sha256}"
        )
    kernel_payload = _load_kernel_payload_bytes(
        kernel_raw, expected_stage_i_raw_sha256=stage_i_raw_sha256
    )
    kernel_physical_sha256, _kernel_identity = _kernel_physical_identity(
        kernel_payload, stage_i_physical_sha256
    )
    if kernel_physical_sha256 != expected_kernel_physical_sha256:
        raise ProductionFailure(
            "kernel physical identity mismatch: "
            f"{kernel_physical_sha256} != {expected_kernel_physical_sha256}"
        )
    return (
        stage_i_payload,
        kernel_payload,
        stage_i_raw_sha256,
        stage_i_physical_sha256,
        kernel_physical_sha256,
    )


def _residual_physical_identity(
    raw: bytes,
    expected_kernel_raw_sha256: str,
) -> tuple[str, dict[str, object]]:
    payload = _strict_json_loads(raw, "Stage-J cube-boundary residual")
    payload = _require_exact_keys(
        payload,
        frozenset(
            {
        ...
        "Stage-J cube-boundary residual",
    )
    meta = _require_exact_keys(
        payload["meta"],
        frozenset({"version", "kernel_file", "kernel_sha256"}),
        "Stage-J residual metadata",
    )
    if (
        meta.get("version") != "2026-06-13-stage3j-v1"
        or meta.get("kernel_file") != "DATA_Y4_full_real_space_h4_kernel.json.gz"
        or meta.get("kernel_sha256") != expected_kernel_raw_sha256
    ):
        raise ProductionFailure("Stage-J residual is not bound to this run's kernel")
    identity = {
        "schema": "hodge-y4-residual-physical-v1",
        "version": meta["version"],
        "cube_state": payload["cube_state"],
        "rigid_component_c4": payload["rigid_component_c4"],
        "H4_cube_image": payload["H4_cube_image"],
        "residual": payload["residual"],
        "dominant_leakage": payload["dominant_leakage"],
    }
    _assert_finite_tree(identity, "residual_identity")
    return _sha256_bytes(_canonical_json_bytes(identity)), identity


def _seal_kernel_snapshot(
    root: Path,
    stage_i_raw: bytes,
) -> tuple[
    Path,
    ...
    source_path = root / "Y4_STAGE3J" / "DATA_Y4_full_real_space_h4_kernel.json.gz"
    ...
    (
        ...
        payload,
        stage_i_raw_sha,
        stage_i_physical_sha,
        physical_sha,
    ) = _validate_stage_i_kernel_binding_bytes(stage_i_raw, raw)
    records = payload["kernel"]
    _verify_kernel_hermiticity(records)
    snapshot = root / "Y4_KERNEL_SEALED_SNAPSHOT.json.gz"
    if snapshot.exists():
        if snapshot.read_bytes() != raw:
            raise ProductionFailure("existing sealed kernel snapshot differs from fresh kernel")
    else:
...
def _validate_production_outputs(root: Path) -> dict[str, object]:
    artifact_hashes = _observe_and_gate_artifact_hashes(root)
    stage0_ordered_path = (
        root / "Y4_STAGE0" / "y4_ordered_transition_words.json.gz"
    )
    stage0_summary_path = root / "Y4_STAGE0" / "y4_stage0_summary.json"
    stage1_manifest_path = (
        root / "Y4_STAGE1" / "y4_channel_denominator_manifest.json.gz"
    )
    stage_i_path = root / "Y4_STAGE3I" / "y4_stage3i_summary.json"
    stage_i_words_path = (
        root / "Y4_STAGE3I" / "y4_complete_folded_word_weights.json.gz"
    )
    stage_j_path = root / "Y4_STAGE3J" / "CERT_Y4_stage3j_verdict.json"
    if (
        not stage0_ordered_path.is_file()
        or not stage0_summary_path.is_file()
        or not stage1_manifest_path.is_file()
        or
        not stage_i_path.is_file()
        or not stage_i_words_path.is_file()
        or not stage_j_path.is_file()
    ):
        raise ProductionFailure("Stage-0/1/I/J canonical artifact chain is incomplete")
    stage0_ordered_raw = stage0_ordered_path.read_bytes()
    stage0_summary_raw = stage0_summary_path.read_bytes()
    stage1_manifest_raw = stage1_manifest_path.read_bytes()
    if _sha256_bytes(stage1_manifest_raw) != artifact_hashes[
        "Y4_STAGE1/y4_channel_denominator_manifest.json.gz"
    ]:
        raise ProductionFailure("Stage-1 manifest changed during validation")
    stage0_ordered = _load_stage0_ordered_payload_bytes(stage0_ordered_raw)
    stage0_summary = _require_exact_keys(
        _strict_json_loads(stage0_summary_raw, "Stage-0 summary"),
        frozenset(
            {
                "meta",
                "scope",
    ...
    stage0_meta = _require_exact_keys(
        stage0_summary["meta"],
        frozenset(
            {
                "version",
        ...
        "Stage-0 summary metadata",
    )
    _require_passed_gates(
        stage0_summary.get("gates"),
        "Stage-0",
        frozenset(
            {
                "G0_o3_exact_arithmetic",
                "G1_cubic_group",
                "G2_local_anchors",
                "G3_support_enumeration",
                "G4_triality_filter",
                "G5_survivor_classification",
                "G6_ordered_words",
                "G7_round_trip",
            }
        ),
    ...
    if (
        stage0_summary.get("passed") is not True
        or stage0_meta.get("version") != "2026-06-13-stage0-v1"
        or stage0_summary["gates"]["G3_support_enumeration"].get("depth_counts")
        != [1, 6, 156, 5082, 182440]
        or stage0_summary["gates"]["G6_ordered_words"].get(
            "raw_words_before_rooted_symmetry"
        )
        != 4296
        or stage0_summary["gates"]["G6_ordered_words"].get(
            "canonical_ordered_transitions"
        )
        != 4221
        or stage0_summary.get("files", {})
        .get("y4_ordered_transition_words.json.gz", {})
        .get("sha256")
        != _sha256_bytes(stage0_ordered_raw)
    ):
        raise ProductionFailure("Stage-0 fourth-order census or file binding failed")
    stage1_manifest = _load_stage1_manifest_payload_bytes(stage1_manifest_raw)
    if stage1_manifest["meta"]["stage0_sha256"] != _sha256_bytes(stage0_ordered_raw):
        raise ProductionFailure("Stage-1 is not bound to this run's Stage-0 ordered words")
    stage_i_words_raw = stage_i_words_path.read_bytes()
    stage_i_runtime_raw_sha = _sha256_bytes(stage_i_words_raw)
    if stage_i_runtime_raw_sha != artifact_hashes[
        "Y4_STAGE3I/y4_complete_folded_word_weights.json.gz"
    ]:
        raise ProductionFailure("Stage-I word file changed during validation")
    stage_i_summary_raw = stage_i_path.read_bytes()
    stage_j_verdict_raw = stage_j_path.read_bytes()
    stage_i = _strict_json_loads(stage_i_summary_raw, "Stage-I summary")
    stage_j = _strict_json_loads(stage_j_verdict_raw, "Stage-J verdict")
    stage_i = _require_exact_keys(
        stage_i,
        frozenset(
            {
                "meta",
    ...
    expected_runtime_meta = frozenset(
        {"version", "date", "python", "platform", "walltime_s", "hardware", "a100_required"}
    )
    stage_i_meta = _require_exact_keys(
        stage_i["meta"], expected_runtime_meta, "Stage-I summary metadata"
    )
    stage_j_meta = _require_exact_keys(
        stage_j["meta"], expected_runtime_meta, "Stage-J verdict metadata"
    )
    if stage_i_meta.get("version") != "2026-06-13-stage3i-v1":
        raise ProductionFailure("unexpected Stage-I summary version")
    if stage_j_meta.get("version") != "2026-06-13-stage3j-v1":
        raise ProductionFailure("unexpected Stage-J verdict version")
    _assert_finite_tree(stage_i, "stage_i")
    _assert_finite_tree(stage_j, "stage_j")
    if stage_i.get("passed") is not True or stage_j.get("passed") is not True:
        raise ProductionFailure("Stage-I or Stage-J reports passed=false")
    _require_stage3h_sealed_out(
        stage_i, root / "_SEALED_NO_STAGE3H_INPUT.json.gz"
    )
    _require_passed_gates(
        stage_i.get("gates"),
        "Stage-I",
        ...
        frozenset(
            {
                ...
                "J1_cube_boundary",
                "J2_dispersion_witness",
                "J3_round_trip",
            }
        ),
    )
    _require_stage_i_formula_and_lineage(
        stage_i, _sha256_bytes(stage1_manifest_raw)
    )

    counts = stage_i.get("counts", {})
    expected_stage_i_counts = {
        "complete_topologies": EXPECTED_COUNTS["topologies"],
        "complete_orientation_orbits": EXPECTED_COUNTS["orientation_orbits"],
        "complete_ordered_words": EXPECTED_COUNTS["ordered_words"],
        "global_fusion_tree_paths": EXPECTED_COUNTS["fusion_tree_paths"],
    }
    for key, expected in expected_stage_i_counts.items():
        if counts.get(key) != expected:
            raise ProductionFailure(f"Stage-I count {key}={counts.get(key)!r}, expected {expected}")

    topology_hist = (
        stage_i["gates"]["I2_complete_topology_contraction"]
        ["resonance_class_topology_histogram"]
    )
    expected_hist = {
        "all_resonant": EXPECTED_COUNTS["all_resonant_topologies"],
        "mixed": EXPECTED_COUNTS["mixed_topologies"],
        "nonresonant_only": EXPECTED_COUNTS["nonresonant_topologies"],
    }
    if topology_hist != expected_hist:
        raise ProductionFailure(
            f"Stage-I topology partition mismatch: {topology_hist!r}"
        )

    j0 = stage_j["gates"]["J0_real_space_kernel"]
    if (
        j0.get("nonzero_full_kernel_entries") != EXPECTED_COUNTS["kernel_entries"]
        or j0.get("stage3i_sha256") != stage_i_runtime_raw_sha
        or j0.get("exact_hermiticity") is not True
        or j0.get("proper_cubic_covariance") is not True
    ):
        raise ProductionFailure("Stage-J kernel structural gate is incomplete")
    (
        sealed_kernel_path,
        sealed_kernel_raw_sha,
        kernel_physical_sha,
        kernel_records,
        bound_stage_i_raw_sha,
        stage_i_physical_sha,
        stage_i_words_payload,
    ) = _seal_kernel_snapshot(root, stage_i_words_raw)
    if bound_stage_i_raw_sha != stage_i_runtime_raw_sha:
        raise ProductionFailure("Stage-I raw binding changed during kernel validation")
    if sealed_kernel_raw_sha != artifact_hashes[
        "Y4_STAGE3J/DATA_Y4_full_real_space_h4_kernel.json.gz"
    ]:
        raise ProductionFailure("Stage-J kernel changed during validation")
    order_certificate = _validate_four_insertion_chain(
        stage0_ordered,
        stage1_manifest,
        stage_i_words_payload,
    )
    static_authority = _verify_embedded_authority()
    stage0_ordered_physical_sha = _sha256_bytes(
        _canonical_json_bytes(stage0_ordered)
    )
    stage1_physical_payload = {
        "schema": "hodge-y4-stage1-channel-manifest-physical-v1",
        "metadata": {
            key: value
            for key, value in stage1_manifest["meta"].items()
            if key not in {"stage0_input", "stage0_sha256"}
        },
        "words": stage1_manifest["words"],
    }
    stage1_manifest_physical_sha = _sha256_bytes(
        _canonical_json_bytes(stage1_physical_payload)
    )
    order_certificate.update(
        {
            "stage0_ordered_raw_sha256": _sha256_bytes(stage0_ordered_raw),
            "stage0_summary_raw_sha256": _sha256_bytes(stage0_summary_raw),
            "stage0_ordered_physical_sha256": stage0_ordered_physical_sha,
            "stage1_manifest_raw_sha256": _sha256_bytes(stage1_manifest_raw),
            "stage1_manifest_physical_sha256": stage1_manifest_physical_sha,
            "stage_i_word_physical_sha256": stage_i_physical_sha,
            "stage0_to_stage1_raw_binding": True,
            "stage0_stage1_stage_i_word_geometry_equal": True,
            "stage0_support_depth_counts": [1, 6, 156, 5082, 182440],
            "stage0_raw_ordered_words": 4296,
            "static_authority_worker_forbidden_markers_absent": static_authority[
                "forbidden_markers_absent"
            ],
            "static_evidence_kind": "embedded authoritative source hash + marker scan",
        }
    )
    residual_path = root / "Y4_STAGE3J" / "y4_cube_boundary_residual.json"
    residual_raw = residual_path.read_bytes()
    if _sha256_bytes(residual_raw) != artifact_hashes[
        "Y4_STAGE3J/y4_cube_boundary_residual.json"
    ]:
        raise ProductionFailure("Stage-J residual changed during validation")
    residual_physical_sha, _residual_identity = _residual_physical_identity(
        residual_raw, sealed_kernel_raw_sha
    )
    if (
        residual_physical_sha
        != EXPECTED_PHYSICAL_IDENTITY_SHA256["cube_boundary_residual"]
    ):
        raise ProductionFailure(
            f"residual physical identity mismatch: {residual_physical_sha} != "
            f"{EXPECTED_PHYSICAL_IDENTITY_SHA256['cube_boundary_residual']}"
        )

    result = stage_j.get("verdict", {})
    required_result = {
        "flat_through_order_y4": False,
        "first_nonzero_bandwidth_order": "y^4",
        "cube_boundary_residual_nonzero": True,
        "maximum_exact_leakage": "5/48",
        "exact_dispersion_witness": "17607806155349/275331901291200",
    }
    for key, expected in required_result.items():
        if result.get(key) != expected:
            raise ProductionFailure(
                f"Stage-J physical result {key}={result.get(key)!r}, expected {expected!r}"
            )
    return {
        "artifact_sha256": artifact_hashes,
        "archive_reference_raw_sha256": EXPECTED_CORE_ARTIFACT_SHA256,
        "portable_raw_hash_gates": list(PORTABLE_RAW_HASH_GATES),
        "raw_hash_policy": (
            "provenance-only; all raw artifacts are path-bearing directly or by "
            "upstream raw-hash cascade"
        ),
        "physical_identity_sha256": {
            "stage_i_word_weights": stage_i_physical_sha,
            "kernel": kernel_physical_sha,
            "cube_boundary_residual": residual_physical_sha,
        },
        "stage_i_summary_sha256": _sha256_bytes(stage_i_summary_raw),
        "stage_j_verdict_sha256": _sha256_bytes(stage_j_verdict_raw),
        "stage_i_counts": expected_stage_i_counts,
        "topology_partition": expected_hist,
        "formula": EXPECTED_H4_OPERATOR_IDENTITY,
        "four_insertion_order_certificate": order_certificate,
        "stage_i_word_weights": {
            "physical_identity_schema": (
                "hodge-y4-stage3i-word-weights-physical-v1"
            ),
            "canonical_metadata": {
                "version": stage_i_words_payload["meta"]["version"],
                "rooted_multiplicity": stage_i_words_payload["meta"][
                    "rooted_multiplicity"
                ],
            },
            "record_count": len(stage_i_words_payload["words"]),
            "source_run_raw_sha256": bound_stage_i_raw_sha,
            "portable_physical_identity_sha256": stage_i_physical_sha,
            "excluded_locator_metadata": ["orbit_file", "orbit_sha256"],
        },
        "stage3h_exclusion": {
            "sentinel_absent_after_stage_i_and_at_final_validation": True,
            "summary_regression_topologies": counts[
                "stage3h_regression_topologies"
            ],
            "I2_regression_topologies": stage_i["gates"][
                "I2_complete_topology_contraction"
            ]["stage3h_topologies_regressed"],
            "passed": True,
        },
        "kernel_entries": len(kernel_records),
        "kernel": {
            "definition": "H4_ba(k)=sum_r K_ba(r)*exp(i*k.r)",
            "basis_planes": [list(plane) for plane in EXPECTED_BASIS_PLANES],
            "records": kernel_records,
            "source_run_raw_sha256": artifact_hashes[
                "Y4_STAGE3J/DATA_Y4_full_real_space_h4_kernel.json.gz"
            ],
            "sealed_snapshot": sealed_kernel_path.name,
            "sealed_snapshot_raw_sha256": sealed_kernel_raw_sha,
            "portable_physical_identity_sha256": kernel_physical_sha,
            "record_count": len(kernel_records),
            "exact_hermiticity": True,
            "proper_cubic_covariance": True,
        },
        "physical_result": result,
        "high_symmetry_corrections": stage_j.get("high_symmetry_corrections"),
        "all_stage_i_gates_passed": True,
        "all_stage_j_gates_passed": True,
    }


def _freeze_preunblind_certificate(
    root: Path,
    script_sha256: str,
    schedule_token: _ScheduleToken,
    authority: dict[str, object],
    ...
    certificate = {
        "schema": "hodge-y4-canonical-preunblind-v1",
        "runner_version": RUNNER_VERSION,
        "status": "PREUNBLIND_CANONICAL_PASS",
        "canonical_variable": "u=beta_H/6=1/g_H^4",
        "runtime": {
            "script_path": str(_runtime_script_path()),
        ...
        "half_history_launch_policy": _launch_policy_certificate(schedule_token),
        "construction": outputs,
        "preunblind": {
            "kernel_frozen": True,
            "historical_reference_loaded": False,
            "historical_reference_available_to_construction": False,
        },
        "determinism_scope": (
            "Every raw stage artifact can carry a direct or transitive runtime-path "
            "dependency, so raw hashes are provenance only. The complete 4,221-word "
            "Stage-I payload, its stable normalization metadata, the 189-record kernel, "
            "and the residual are gated by canonical exact physical-content identities. "
            "The Stage-J raw input hash must additionally equal this run's exact Stage-I "
            "raw hash. Runtime paths, raw-hash cascades, platform strings, and timing are "
            "not part of the portable physical identity."
        ),
    }
    _assert_finite_tree(certificate)
    path = root / "Y4_CANONICAL_CERTIFICATE_PREUNBLIND.json"
    digest = _atomic_write_json(path, certificate)
    try:
        path.chmod(0o444)
    except OSError:
        pass
...
def _intrinsic_gamma_payload(
    root: Path,
    preunblind_path: Path,
    preunblind_sha256: str,
) -> tuple[dict[str, object], Fraction]:
    preunblind_raw = preunblind_path.read_bytes()
    if _sha256_bytes(preunblind_raw) != preunblind_sha256:
        raise ProductionFailure("pre-unblind certificate changed before comparison")
    preunblind = _strict_json_loads(preunblind_raw, "pre-unblind certificate")
    if (
        preunblind.get("status") != "PREUNBLIND_CANONICAL_PASS"
        or preunblind.get("preunblind", {}).get("historical_reference_loaded") is not False
    ):
        raise ProductionFailure("pre-unblind seal is not valid")
    kernel_cert = preunblind["construction"]["kernel"]
    snapshot_name = kernel_cert.get("sealed_snapshot")
    if snapshot_name != "Y4_KERNEL_SEALED_SNAPSHOT.json.gz":
        raise ProductionFailure("pre-unblind certificate has an invalid kernel snapshot")
    kernel_path = root / snapshot_name
    raw = kernel_path.read_bytes()
    expected_kernel_sha = kernel_cert["sealed_snapshot_raw_sha256"]
    if _sha256_bytes(raw) != expected_kernel_sha:
        raise ProductionFailure("sealed kernel snapshot changed before Gamma derivation")
    payload = _load_kernel_payload_bytes(raw)
    physical_sha, _identity = _kernel_physical_identity(payload)
    if (
        physical_sha != kernel_cert["portable_physical_identity_sha256"]
        or physical_sha != EXPECTED_PHYSICAL_IDENTITY_SHA256["kernel"]
    ):
        raise ProductionFailure("sealed kernel physical identity changed before Gamma derivation")
    records = payload["kernel"]
    sealed_records = preunblind["construction"]["kernel"]["records"]
    if records != sealed_records:
        raise ProductionFailure("kernel records do not match the pre-unblind certificate")
    matrix, q3 = _gamma_scalar_from_records(records)
    intrinsic = {
        "schema": "hodge-y4-gamma-intrinsic-v1",
        "runner_version": RUNNER_VERSION,
        "preunblind_certificate_sha256": preunblind_sha256,
        "kernel_sha256": expected_kernel_sha,
        "kernel_physical_identity_sha256": physical_sha,
        "definition": "H_ba(Gamma)=sum_r K_ba(r)",
        "gamma_matrix": [[str(value) for value in row] for row in matrix],
        "off_diagonal_zero": True,
        "diagonal_scalar": True,
        "computed_q3_u4": str(q3),
        "historical_reference_loaded": False,
        "passed": True,
    }
    _assert_finite_tree(intrinsic, "intrinsic_gamma")
    return intrinsic, q3


def _derive_gamma_from_sealed_kernel(
    root: Path,
    preunblind_path: Path,
    preunblind_sha256: str,
) -> tuple[Path, str, Fraction]:
    intrinsic, q3 = _intrinsic_gamma_payload(
        root, preunblind_path, preunblind_sha256
    )
    path = root / "Y4_GAMMA_INTRINSIC_PREUNBLIND.json"
    digest = _atomic_write_json(path, intrinsic)
    try:
        path.chmod(0o444)
    except OSError:
        pass
    return path, digest, q3


def _verify_existing_sealed_run(
    root: Path,
    *,
    require_terminal_pass: bool = False,
) -> dict[str, object]:
    script_sha = sha256_file(_runtime_script_path())
    authority = _verify_embedded_authority()
    normalization = _normalization_certificate()
    schedule_token = _build_schedule_token()
    binding = _binding_payload(script_sha, schedule_token, authority, normalization)
    run_root = _prepare_run_root(root, True, binding)
    required_paths = {
        "status": run_root / "Y4_CANONICAL_RUN_STATUS.json",
        "preunblind": run_root / "Y4_CANONICAL_CERTIFICATE_PREUNBLIND.json",
        "intrinsic": run_root / "Y4_GAMMA_INTRINSIC_PREUNBLIND.json",
        "kernel_snapshot": run_root / "Y4_KERNEL_SEALED_SNAPSHOT.json.gz",
        "stage_i_words": (
            run_root / "Y4_STAGE3I" / "y4_complete_folded_word_weights.json.gz"
        ),
        "stage_j_kernel": (
            run_root / "Y4_STAGE3J" / "DATA_Y4_full_real_space_h4_kernel.json.gz"
        ),
        "stage_j_verdict": (
            run_root / "Y4_STAGE3J" / "CERT_Y4_stage3j_verdict.json"
        ),
    }
    missing = [name for name, path in required_paths.items() if not path.is_file()]
    if missing:
        raise ProductionFailure(f"sealed run is missing physical artifacts: {missing!r}")
    ...
    if (
        status.get("schema") != "hodge-y4-canonical-status-v1"
        or status.get("runner_version") != RUNNER_VERSION
        or status.get("runtime_script_sha256") != script_sha
    ):
        raise ProductionFailure("run status is not bound to this exact runner")
    state = status.get("status")
    if require_terminal_pass and state != "PASS":
        raise ProductionFailure(f"terminal PASS is required, got {state!r}")
    if state not in {"PREUNBLIND_PASS", "PASS", "FAIL"}:
        raise ProductionFailure(f"run has no verifiable sealed state: {state!r}")

    pre_path = required_paths["preunblind"]
    intrinsic_path = required_paths["intrinsic"]
    pre_sha = sha256_file(pre_path)
    intrinsic_sha = sha256_file(intrinsic_path)
    detail = status["detail"]
    if not isinstance(detail, dict):
        raise ProductionFailure("run status detail is malformed")
    if state == "PREUNBLIND_PASS":
        detail = _require_exact_keys(
            detail,
            frozenset(
                {
                    "preunblind_certificate",
                    "preunblind_sha256",
                    "intrinsic_gamma_certificate",
                    "intrinsic_gamma_sha256",
                    "kernel_physical_identity_sha256",
                    "computed_q3_u4",
                    "historical_reference_loaded",
                    "walltime_s",
                }
            ),
            "PREUNBLIND_PASS detail",
        )
        if (
            Path(str(detail["preunblind_certificate"])).resolve() != pre_path.resolve()
            or Path(str(detail["intrinsic_gamma_certificate"])).resolve()
            != intrinsic_path.resolve()
            or detail.get("historical_reference_loaded") is not False
        ):
            raise ProductionFailure("PREUNBLIND_PASS points to different seals")
    elif state == "PASS":
        detail = _require_exact_keys(
            detail,
            frozenset(
                {
                    ...
                    "final_sha256",
                    "computed_q3_u4",
                    "historical_exact_equal",
                }
            ),
            "PASS detail",
        )
        if detail.get("phase") != "terminal_unblind":
            raise ProductionFailure("PASS did not come from terminal unblind")
    else:
        detail = _require_exact_keys(
            detail,
            frozenset(
            ...
            "terminal retry detail",
        )
        if detail.get("phase") != "terminal_unblind":
            raise ProductionFailure("non-terminal failure is not a sealed construction")
    if (
        detail.get("preunblind_sha256") != pre_sha
        or detail.get("intrinsic_gamma_sha256") != intrinsic_sha
    ):
        raise ProductionFailure("status hashes do not match the sealed construction")

    pre = read_json(pre_path)
    pre = _require_exact_keys(
        pre,
        frozenset(
            {
                "schema",
                ...
                "authority",
                "normalization",
                "half_history_launch_policy",
                "construction",
                "preunblind",
                "determinism_scope",
            }
        ),
        "pre-unblind certificate",
    )
    runtime = _require_exact_keys(
        pre["runtime"],
        frozenset({"script_path", "script_sha256", "python", "platform", "package_versions"}),
        "pre-unblind runtime",
    )
        ...
        frozenset(
            {
                "kernel_frozen",
                "historical_reference_loaded",
                "historical_reference_available_to_construction",
            }
        ),
        "pre-unblind state",
    )
    if (
        pre.get("schema") != "hodge-y4-canonical-preunblind-v1"
        or pre.get("runner_version") != RUNNER_VERSION
        or pre.get("status") != "PREUNBLIND_CANONICAL_PASS"
        or pre.get("canonical_variable") != "u=beta_H/6=1/g_H^4"
        or runtime.get("script_sha256") != script_sha
        or pre.get("authority") != authority
        or pre.get("normalization") != normalization
        or pre.get("half_history_launch_policy")
        != _launch_policy_certificate(schedule_token)
        or preblind
        != {
            "kernel_frozen": True,
            "historical_reference_loaded": False,
        ...
        or not isinstance(pre.get("determinism_scope"), str)
    ):
        raise ProductionFailure("pre-unblind certificate provenance is not exact")

    outputs = _validate_production_outputs(run_root)
    if pre.get("construction") != outputs:
        raise ProductionFailure(
            "pre-unblind construction does not reproduce from physical Stage-0/1/I/J artifacts"
        )
    expected_intrinsic, computed_q3 = _intrinsic_gamma_payload(
        run_root, pre_path, pre_sha
    )
    observed_intrinsic = read_json(intrinsic_path)
    if observed_intrinsic != expected_intrinsic:
        raise ProductionFailure("intrinsic Gamma does not reproduce from the sealed kernel")
    if (
        detail.get("computed_q3_u4", str(computed_q3)) != str(computed_q3)
        or (
            state == "PREUNBLIND_PASS"
            and detail.get("kernel_physical_identity_sha256")
            != EXPECTED_PHYSICAL_IDENTITY_SHA256["kernel"]
        )
    ):
        raise ProductionFailure("status result does not match recomputed exact Gamma")

    verification: dict[str, object] = {
        "schema": "hodge-y4-sealed-run-verification-v1",
        "status": state,
        "runtime_script_sha256": script_sha,
        "preunblind_sha256": pre_sha,
        "intrinsic_gamma_sha256": intrinsic_sha,
        "stage_i_word_physical_sha256": EXPECTED_PHYSICAL_IDENTITY_SHA256[
            "stage_i_word_weights"
        ],
        "kernel_physical_sha256": EXPECTED_PHYSICAL_IDENTITY_SHA256["kernel"],
        "computed_q3_u4": str(computed_q3),
        "physical_artifacts_recomputed": True,
        "passed": True,
    }
    if state == "PASS":
        final_path = run_root / "Y4_CANONICAL_CERTIFICATE_FINAL.json"
        if not final_path.is_file():
            raise ProductionFailure("PASS is missing the final certificate")
        final_sha = sha256_file(final_path)
        if (
            Path(str(detail["final_certificate"])).resolve() != final_path.resolve()
            or detail.get("final_sha256") != final_sha
            or detail.get("historical_exact_equal") is not True
        ):
            raise ProductionFailure("PASS status is not bound to the final certificate")
            ...
            frozenset(
                {
                    "schema",
                    "runner_version",
                    "preunblind_certificate_sha256",
                    "intrinsic_gamma_certificate_sha256",
                    "computed_q3_u4",
            ...
            frozenset(
                {
                    ...
                    "authority_sha256",
                    "authority_locator",
                    "sealed_for_terminal_only",
                    "reference_envelope_sha256",
                    "loaded_after_kernel_and_gamma_seals",
                }
            ),
            "final historical reference",
        )
        _validate_historical_reference_copy(historical, computed_q3)
        if (
            final.get("schema") != "hodge-y4-terminal-unblind-v1"
            or final.get("runner_version") != RUNNER_VERSION
            or final.get("preunblind_certificate_sha256") != pre_sha
            or final.get("intrinsic_gamma_certificate_sha256") != intrinsic_sha
            or final.get("computed_q3_u4") != str(computed_q3)
            or final.get("difference") != "0"
            or final.get("exact_equal") is not True
            or final.get("passed") is not True
            or historical.get("reference_envelope_sha256")
            != HISTORICAL_REFERENCE_ENVELOPE_SHA256
            or historical.get("authority_sha256") != V24C_AUTHORITY_SHA256
            or historical.get("value_u4") != str(computed_q3)
            or historical.get("loaded_after_kernel_and_gamma_seals") is not True
        ):
            raise ProductionFailure("final terminal seal chain is not exact")
        verification["final_sha256"] = final_sha
        verification["historical_reference_commitment_verified"] = True
    return verification


def verify_sealed_run_main(root: Path, require_terminal_pass: bool = False) -> dict[str, object]:
    script_sha = sha256_file(_runtime_script_path())
    authority = _verify_embedded_authority()
    normalization = _normalization_certificate()
    token = _build_schedule_token()
    expected_binding = _binding_payload(script_sha, token, authority, normalization)
    try:
        result = _verify_existing_sealed_run(
            root, require_terminal_pass=require_terminal_pass
        )
    except BaseException as exc:
        resolved = root.expanduser().resolve()
        binding_path = resolved / "Y4_CANONICAL_RUN_BINDING.json"
        bound_to_this_runner = False
        if binding_path.is_file():
            try:
                bound_to_this_runner = read_json(binding_path) == expected_binding
            except Exception:
                bound_to_this_runner = False
        if bound_to_this_runner:
            _write_status(
                resolved,
                "FAIL",
                script_sha,
                {
                    "phase": "sealed_run_verification",
    ...
    print("SEALED PHYSICAL RUN VERIFICATION PASS")
    return result


def _historical_reference_envelope_from_copy(
    historical: dict[str, object],
) -> dict[str, object]:
    return {
        "schema": historical.get("schema"),
        "label": historical.get("label"),
        "canonical_variable": historical.get("canonical_variable"),
        "value_u4": historical.get("value_u4"),
        "authority_sha256": historical.get("authority_sha256"),
        "authority_locator": historical.get("authority_locator"),
        "sealed_for_terminal_only": historical.get("sealed_for_terminal_only"),
    }


def _validate_historical_reference_copy(
    historical: dict[str, object],
    computed_q3: Fraction,
) -> None:
    envelope = _historical_reference_envelope_from_copy(historical)
    expected = {
        "schema": HISTORICAL_REFERENCE_SCHEMA,
        "label": HISTORICAL_REFERENCE_LABEL,
        "canonical_variable": HISTORICAL_REFERENCE_VARIABLE,
        "value_u4": str(computed_q3),
        "authority_sha256": V24C_AUTHORITY_SHA256,
        "authority_locator": HISTORICAL_REFERENCE_AUTHORITY_LOCATOR,
        "sealed_for_terminal_only": True,
    }
    if envelope != expected:
        raise ProductionFailure("terminal historical reference fields are not exact")
    reconstructed_sha = _sha256_bytes(_canonical_json_bytes(envelope))
    if (
        reconstructed_sha != HISTORICAL_REFERENCE_ENVELOPE_SHA256
        or historical.get("reference_envelope_sha256") != reconstructed_sha
        or historical.get("loaded_after_kernel_and_gamma_seals") is not True
    ):
        raise ProductionFailure(
            "terminal historical reference commitment does not reconstruct"
        )


def _load_historical_q3_after_seal(
    intrinsic_path: Path,
    intrinsic_sha256: str,
    reference_path: Path,
) -> dict[str, object]:
    # The companion construction program contains no historical coefficient.
    # A separate reference envelope is created by the notebook only after the
    # construction subprocess has exited and both canonical seals exist.
    intrinsic_raw = intrinsic_path.read_bytes()
    if _sha256_bytes(intrinsic_raw) != intrinsic_sha256:
        raise ProductionFailure("intrinsic Gamma certificate changed before unblind")
    intrinsic = _strict_json_loads(intrinsic_raw, "intrinsic Gamma certificate")
    if (
        intrinsic.get("passed") is not True
        or intrinsic.get("historical_reference_loaded") is not False
    ):
        raise ProductionFailure("intrinsic Gamma certificate is not pre-unblind")
    reference_raw = reference_path.read_bytes()
    reference_sha = _sha256_bytes(reference_raw)
    if reference_sha != HISTORICAL_REFERENCE_ENVELOPE_SHA256:
        raise ProductionFailure(
            "terminal historical reference does not match the sealed authority commitment"
        )
    reference = _strict_json_loads(reference_raw, "terminal historical reference")
    required_keys = {
        "schema",
        "label",
        "canonical_variable",
    ...
    if not isinstance(reference, dict) or set(reference) != required_keys:
        raise ProductionFailure("terminal historical reference manifest is not exact")
    if (
        reference.get("schema") != HISTORICAL_REFERENCE_SCHEMA
        or reference.get("label") != HISTORICAL_REFERENCE_LABEL
        or reference.get("canonical_variable") != HISTORICAL_REFERENCE_VARIABLE
        or reference.get("authority_sha256") != V24C_AUTHORITY_SHA256
        or reference.get("authority_locator")
        != HISTORICAL_REFERENCE_AUTHORITY_LOCATOR
        or reference.get("sealed_for_terminal_only") is not True
    ):
        raise ProductionFailure("terminal historical reference provenance is invalid")
    try:
        value = Fraction(str(reference["value_u4"]))
    except Exception as exc:
        raise ProductionFailure("terminal historical coefficient is not an exact fraction") from exc
    if str(value) != reference["value_u4"]:
        raise ProductionFailure("terminal historical coefficient is not canonicalized")
    loaded = dict(reference)
    loaded["reference_envelope_sha256"] = reference_sha
    loaded["loaded_after_kernel_and_gamma_seals"] = True
    loaded["fraction"] = value
    return loaded


def _terminal_unblind(
    root: Path,
    preunblind_path: Path,
    preunblind_sha256: str,
    intrinsic_path: Path,
    ...
    intrinsic_raw = intrinsic_path.read_bytes()
    if _sha256_bytes(intrinsic_raw) != intrinsic_sha256:
        raise ProductionFailure("intrinsic Gamma seal changed before terminal comparison")
    observed_intrinsic = _strict_json_loads(intrinsic_raw, "intrinsic Gamma seal")
    if observed_intrinsic != expected_intrinsic:
        raise ProductionFailure("intrinsic Gamma seal does not reproduce from the frozen kernel")
    reference = _load_historical_q3_after_seal(
        intrinsic_path, intrinsic_sha256, reference_path
    )
    historical_q3 = reference.pop("fraction")
    equal = computed_q3 == historical_q3
    comparison = {
        "schema": "hodge-y4-terminal-unblind-v1",
        "runner_version": RUNNER_VERSION,
        "preunblind_certificate_sha256": preunblind_sha256,
        "intrinsic_gamma_certificate_sha256": intrinsic_sha256,
        "computed_q3_u4": str(computed_q3),
        "historical_reference": reference,
        "difference": str(computed_q3 - historical_q3),
        "exact_equal": equal,
        "passed": equal,
    }
    if not equal:
        raise ProductionFailure(
            f"terminal historical comparison mismatch: {comparison!r}"
        )
    path = root / "Y4_CANONICAL_CERTIFICATE_FINAL.json"
    digest = _atomic_write_json(path, comparison)
    try:
        path.chmod(0o444)
    except OSError:
        pass
    return path, digest, comparison


def _cheap_self_test() -> dict[str, object]:
    authority = _verify_embedded_authority()
    normalization = _normalization_certificate()
    token = _build_schedule_token()
    ...
    try:
        schedule.transition(
            "W", "W2", "W3", "Q2", "Q2", 2,
            lambda: poison_calls.append("called"),
        )
    except ScheduleViolation:
        pass
    else:
        raise ProductionFailure("third-W poison control did not fail")
    if poison_calls:
        raise ProductionFailure("invalid schedule reached the poison backend")
    reentrant_calls: list[str] = []
    reentrant = HalfHistorySchedule()

    def first_w_backend() -> None:
        reentrant_calls.append("first-W")
        reentrant.transition(
            "W", "P0", "W1", "P", "Q1", 0,
            lambda: reentrant_calls.append("reentrant-W"),
        )

    try:
        reentrant.transition("W", "P0", "W1", "P", "Q1", 0, first_w_backend)
    except ScheduleViolation:
        pass
    else:
        raise ProductionFailure("reentrant schedule poison control did not fail")
    if reentrant_calls != ["first-W"]:
        raise ProductionFailure(f"reentrant backend was reached: {reentrant_calls!r}")
    typed_poison: list[str] = []
    try:
        HalfHistorySchedule().transition(
            "W", "P0", "W1", "P", "Q1", 0.9,
        ...
        raise ProductionFailure("ill-typed schedule poison control did not fail")
    if typed_poison:
        raise ProductionFailure("ill-typed transition reached its backend")
    return {
        "authority": authority["passed"],
        "normalization": normalization["passed"],
        "schedule_sha256": token.manifest_sha256,
        "w_actions": sum(row[1] == "W" for row in token.manifest),
        "third_w_rejected_before_policy_callback": True,
        "reentrant_w_rejected_before_policy_callback": True,
        "ill_typed_depth_rejected_before_policy_callback": True,
        "passed": True,
    }


def production_main(root: Path, resume: bool) -> dict[str, object]:
    started = time.time()
    ...
    schedule_token = _build_schedule_token()
    binding = _binding_payload(script_sha, schedule_token, authority, normalization)
    run_root = _prepare_run_root(root, resume, binding)
    _write_status(
        run_root,
        "RUNNING",
        script_sha,
    ...
    try:
        print("=" * 108)
        print("CANONICAL SU(3) O(u^4) PRODUCTION")
        print("=" * 108)
        print("canonical variable : u=beta_H/6=1/g_H^4")
        print("launch policy      : P0 -> W1 -> R1 -> W2 (abstract two-W authorization)")
        print("worker order proof : output-derived four-insertion chain certificate")
        print("backend            : authoritative June-13 exact CPU construction")
        print("historical target  : SEALED / NOT LOADED")
        print("run root           :", run_root)
        print("script SHA256      :", script_sha)
        print()
        _authoritative_main(run_root, schedule_token)
        outputs = _validate_production_outputs(run_root)
        preunblind_path, preunblind_sha = _freeze_preunblind_certificate(
            run_root,
            script_sha,
            schedule_token,
            authority,
        ...
        print("PREUNBLIND CANONICAL PASS")
        print("PREUNBLIND SHA256:", preunblind_sha)
        intrinsic_path, intrinsic_sha, computed_q3 = _derive_gamma_from_sealed_kernel(
            run_root, preunblind_path, preunblind_sha
        )
        elapsed = time.time() - started
        status_detail = {
            "preunblind_certificate": str(preunblind_path),
            "preunblind_sha256": preunblind_sha,
            "intrinsic_gamma_certificate": str(intrinsic_path),
            "intrinsic_gamma_sha256": intrinsic_sha,
            "kernel_physical_identity_sha256": outputs[
                "physical_identity_sha256"
            ]["kernel"],
            "computed_q3_u4": str(computed_q3),
            "historical_reference_loaded": False,
        ...
        _write_status(run_root, "PREUNBLIND_PASS", script_sha, status_detail)
        print("INTRINSIC GAMMA PASS")
        print("COMPUTED q3(u^4):", computed_q3)
        print("INTRINSIC SHA256:", intrinsic_sha)
        print("CANONICAL CONSTRUCTION PASS — HISTORICAL REFERENCE NOT LOADED — NO GPU USED")
...
def terminal_unblind_main(root: Path, reference_path: Path) -> dict[str, object]:
    script_path = _runtime_script_path()
    script_sha = sha256_file(script_path)
    authority = _verify_embedded_authority()
    normalization = _normalization_certificate()
    ...
    binding = _binding_payload(script_sha, schedule_token, authority, normalization)
    run_root = _prepare_run_root(root, True, binding)
    reference_resolved = reference_path.expanduser().resolve()
    try:
        reference_resolved.relative_to(run_root)
    except ValueError:
        pass
    else:
        raise ProductionFailure("terminal reference envelope must be outside the run root")
    if not reference_resolved.is_file():
        raise ProductionFailure(f"terminal reference envelope is missing: {reference_resolved}")

    # Recompute the complete physical Stage-0/1/I/J chain and exact Gamma from
    # disk before entering the historical-reference phase.
    _verify_existing_sealed_run(run_root, require_terminal_pass=False)

    preunblind_path = run_root / "Y4_CANONICAL_CERTIFICATE_PREUNBLIND.json"
    intrinsic_path = run_root / "Y4_GAMMA_INTRINSIC_PREUNBLIND.json"
    if not preunblind_path.is_file() or not intrinsic_path.is_file():
        raise ProductionFailure("canonical pre-unblind seals are missing")
    preunblind_sha = sha256_file(preunblind_path)
    intrinsic_sha = sha256_file(intrinsic_path)
    status_path = run_root / "Y4_CANONICAL_RUN_STATUS.json"
    status = read_json(status_path)
    if status.get("status") not in {"PREUNBLIND_PASS", "FAIL"}:
        raise ProductionFailure(
            f"terminal unblind requires a completed pre-unblind construction, got {status!r}"
        )
    if (
        status.get("status") == "FAIL"
        and status.get("detail", {}).get("phase") != "terminal_unblind"
    ):
        raise ProductionFailure("construction failure cannot enter terminal unblind")
    prior_detail = status.get("detail", {})
    if (
        prior_detail.get("preunblind_sha256") != preunblind_sha
        or prior_detail.get("intrinsic_gamma_sha256") != intrinsic_sha
    ):
        raise ProductionFailure("run status is not bound to the current pre-unblind seals")
    preunblind = read_json(preunblind_path)
    if (
        preunblind.get("runtime", {}).get("script_sha256") != script_sha
        or preunblind.get("construction", {})
        .get("physical_identity_sha256", {})
        .get("kernel")
        != EXPECTED_PHYSICAL_IDENTITY_SHA256["kernel"]
    ):
        raise ProductionFailure("pre-unblind certificate is not bound to this canonical runner")
    _write_status(
        run_root,
        "TERMINAL_UNBLIND_RUNNING",
        script_sha,
        {
            "phase": "terminal_unblind",
            "preunblind_sha256": preunblind_sha,
            "intrinsic_gamma_sha256": intrinsic_sha,
            "reference_envelope_sha256": sha256_file(reference_resolved),
        },
    )
    try:
        final_path, final_sha, comparison = _terminal_unblind(
            run_root,
            preunblind_path,
            preunblind_sha,
            intrinsic_path,
        ...
        detail = {
            ...
            "historical_exact_equal": comparison["exact_equal"],
        }
        _write_status(run_root, "PASS", script_sha, detail)
        print("TERMINAL HISTORICAL COMPARISON PASS")
        print("COMPUTED q3(u^4):", comparison["computed_q3_u4"])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Canonical exact SU(3) O(u^4) 189-kernel production"
    )
    parser.add_argument(
        '--root',
        type=Path,
        default=(
            Path('/content/Y4_CANONICAL_PRODUCTION')
            if Path('/content').exists()
            else Path.cwd() / 'Y4_CANONICAL_PRODUCTION'
        ),
    )
    parser.add_argument(
        '--resume',
        action='store_true',
        help='reuse only an existing run bound to this exact script and authority',
    )
    parser.add_argument(
        '--self-test',
        action='store_true',
        help='run cheap source/schedule/normalization checks only',
    )
    parser.add_argument(
        '--terminal-unblind',
        action='store_true',
        help='compare a sealed intrinsic result with a separately supplied reference',
    )
    parser.add_argument(
        '--verify-sealed-run',
        action='store_true',
        help='recompute all physical identities and seal links without running workers',
    )
    parser.add_argument(
        '--require-terminal-pass',
        action='store_true',
        help='with --verify-sealed-run, also require the complete final reference chain',
    )
    parser.add_argument(
        '--reference',
        type=Path,
        default=None,
        help='terminal-only historical reference envelope (never used by construction)',
    )
    args, _unknown = parser.parse_known_args()
    if args.self_test:
        if (
            args.terminal_unblind
            or args.verify_sealed_run
            or args.require_terminal_pass
            or args.reference is not None
        ):
            parser.error('--self-test cannot be combined with terminal reference options')
        print(json.dumps(_cheap_self_test(), indent=2, sort_keys=True))
        print('SELF-TEST PASS — NO PHYSICAL CONTRACTION, NO GPU')
    elif args.verify_sealed_run:
        if args.terminal_unblind or args.reference is not None:
            parser.error('--verify-sealed-run cannot be combined with terminal options')
        verify_sealed_run_main(args.root, args.require_terminal_pass)
    elif args.terminal_unblind:
        if args.require_terminal_pass:
            parser.error('--require-terminal-pass requires --verify-sealed-run')
        if args.reference is None:
            parser.error('--terminal-unblind requires --reference')
        terminal_unblind_main(args.root, args.reference)
    else:
        if args.reference is not None or args.require_terminal_pass:
            parser.error('--reference is accepted only with --terminal-unblind')
        production_main(args.root, args.resume)
