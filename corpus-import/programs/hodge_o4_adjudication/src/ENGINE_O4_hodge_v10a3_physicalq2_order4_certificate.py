...
# =============================================================================
# v10a.3 — PHYSICAL Q2 GRAM/H0 QUOTIENT + DIRECT ORDER-FOUR CERTIFICATE
# =============================================================================
# Scope:
#   * retain the full T1 fiber and K=2 simple-loop P graph before Q2;
#   * use the exact source-facing physical Q1 quotient from v10a.2;
#   * apply W=-M to the anchored source-bright Q1 block, resolve every second
#     action into exact joint H0 irreps, and project out the COMPLETE retained
#     P + one-step-Q1 span block-by-block using a Gram pseudoinverse;
#   * factor the positive physical Q2 Gram range and build W12;
#   * certify W12 W21 against the raw projected second-step kernel;
#   * form ONLY the direct Q2 fourth-order component
#         B R1 W12 R2 W21 R1 B^T
#     on the anchored T1 block.
#
# Deliberate hard stop:
#   m4_rest and C3_full^shp remain BLINDED.  The direct Q2 component is not the
#   full fourth-order effective operator: P-return paths, folds/energy
#   derivatives, determinant-sensitive adjacent pieces, and linked interacting
#   vacuum subtraction still have to be assembled and then checked against the
#   protected mobility theorem.

V10A3_P_DEPTH=int(os.environ.get('V10A3_P_DEPTH','2'))
V10A3_ANCHOR_POLS=int(os.environ.get('V10A3_ANCHOR_POLS','3'))
V10A3_PROGRESS=int(os.environ.get('V10A3_PROGRESS','1'))
V10A3_GRAM_TOL=float(os.environ.get('V10A3_GRAM_TOL','2e-11'))
V10A3_VEC_TOL=float(os.environ.get('V10A3_VEC_TOL','2e-12'))
V10A3_BLOCK_LIMIT=int(os.environ.get('V10A3_BLOCK_LIMIT','0'))
V10A3_BLOCK_OFFSET=int(os.environ.get('V10A3_BLOCK_OFFSET','0'))
V10A3_GATE_START=len(gates)


def _v10a3_flux_codd(k):
    k=tuple(sorted((int(l),int(z)) for l,z in k if int(z)))
    kc=tuple(sorted((int(l),-int(z)) for l,z in k if int(z)))
    return min(k,kc)


def _v10a3_flux_from_state(st):
    return _v10a3_flux_codd(_v9_flux_key_state(st))


def _v10a3_flux_from_arr(q):
    out=[]
    for l in np.flatnonzero(q):
        z=int(q[int(l)])%3
        if z==2: z=-1
        if z: out.append((int(l),int(z)))
    return _v10a3_flux_codd(tuple(out))


def _v10a3_addvec(dst,src,c):
    c=float(c)
    if abs(c)<1e-15: return
    for st,x in src.items(): dst[st]+=c*float(x)


def _v10a3_prune(v,tol=None):
    if tol is None: tol=V10A3_VEC_TOL
    return {st:float(c) for st,c in v.items() if abs(c)>tol}


def _v10a3_endpoint_engine(q2haar):
    old=set(V10_SUPPORTED_ENDPOINT_PATTERNS)
    cache={}
    counts=Counter()
    def ep(a,b):
        aa,bb=_joint_canon_states(a,b)
        ...
        z=cache.get(key)
        if z is not None:
            counts['cache_hit']+=1; return z
        pats=_v10_endpoint_patterns(aa,bb)
        if set(pats)<=old:
            z=float(_vec_inner_fast({aa:1.0},{bb:1.0})); counts['v9_fast']+=1
        else:
            z=float(q2haar(aa,bb)); counts['q2_extended']+=1
        cache[key]=z
        return z
    return ep,cache,counts


def _v10a3_vec_gram(vecs,ep):
    """Gram matrix using one endpoint matrix per union of trace-network states."""
    n=len(vecs)
    if n==0: return np.zeros((0,0),dtype=np.float64)
    states=[]; idx={}
    for v in vecs:
        for st in v:
            if st not in idx:
                idx[st]=len(states); states.append(st)
    C=np.zeros((n,len(states)),dtype=np.float64)
    for i,v in enumerate(vecs):
        for st,c in v.items(): C[i,idx[st]]=float(c)
    H=np.zeros((len(states),len(states)),dtype=np.float64)
    for i,a in enumerate(states):
        for j in range(i,len(states)):
            z=ep(a,states[j]); H[i,j]=H[j,i]=z
    return C@H@C.T


def _v10a3_cross_gram(left,right,ep):
    if not left or not right: return np.zeros((len(left),len(right)),dtype=np.float64)
    states=[];idx={}
    for v in list(left)+list(right):
        for st in v:
            if st not in idx:
                idx[st]=len(states);states.append(st)
    CL=np.zeros((len(left),len(states)));CR=np.zeros((len(right),len(states)))
    for i,v in enumerate(left):
        for st,c in v.items():CL[i,idx[st]]=float(c)
    for i,v in enumerate(right):
        for st,c in v.items():CR[i,idx[st]]=float(c)
...
def _v10a3_p_codd_vec(model,ip):
    q=np.asarray(model['states'][int(ip)],dtype=np.int8)
    st=lx_trace_state(ordered_loop_steps(q,links,shift))
    stc=lx_trace_state(ordered_loop_steps(-q,links,shift))
    s=1.0/math.sqrt(2.0)
    return _v10a3_prune({st:s,stc:-s},1e-14)


def _v10a3_retained_lookup(model):
    """Index retained P states by (canonical joint rep signature, exact energy, C- flux)."""
    out=defaultdict(list)
    for i,q in enumerate(model['states']):
        sig=_v10a2_sig_canon(_v10a2_sig_dyn(_rep_sig_from_flux(q)))
        # Exact bare H0 energy in the dynamic (p,q) signature.
        ef=_v10a2_sig_E(sig)
        st=lx_trace_state(ordered_loop_steps(q,links,shift))
        fk=_v10a3_flux_from_state(st)
        out[(sig,ef,fk)].append(int(i))
    return out


def _v10a3_q1_record_flux(model,rec):
    q=np.asarray(model['states'][int(rec.parent)],dtype=np.int8)
    b=(int(rec.orient)*model['v9_face_flux_fn'](int(rec.face))).astype(np.int8)
    r=mod3_reduce_flux(q+b)
    return _v10a3_flux_from_arr(r)


def _v10a3_build_second_step_actions(model,qquot,anchor):
    """Build exact H0-resolved W Q1 -> second-step parent action vectors.

    The vectors are grouped by (canonical rep signature, exact H0 energy,
    canonical C- center-flux key).  No P/Q1 subtraction is done here; that is
    performed independently by the block Gram projector below.
    """
    Y=np.asarray(qquot['B_W'].T@anchor,dtype=np.float64)
    bright=np.flatnonzero(np.linalg.norm(Y,axis=1)>1e-12)
    bright_map={int(j):a for a,j in enumerate(bright)}
    actions=defaultdict(lambda:defaultdict(lambda:defaultdict(float)))
    class_hint=Counter(); raw_projected=0; recs_touched=0
    t0=time.time()
    for meta in qquot['blocks']:
        a0,b0=meta['q_slice']
        gl=[int(j) for j in bright if a0<=int(j)<b0]
        if not gl: continue
        loc=[j-a0 for j in gl]
        X=np.asarray(meta['X'],dtype=np.float64); recs=meta['records']; recs_touched+=len(recs)
        for i,rec in enumerate(recs):
            coeffs=X[i,loc]
            if np.max(np.abs(coeffs),initial=0.0)<1e-14: continue
            sig0=model['v9_rec_sig'](rec)
            for conj,csign in ((False,+1.0),(True,-1.0)):
                vv,_=model['v9_joint_vec'](rec,conj)
                if not vv: continue
                sig=_rep_sig_conj(sig0) if conj else sig0
                cand=set()
                for st in vv:
                    for l,_ in st.occ: cand.update(link_faces[int(l)])
                physfac=float(csign)/math.sqrt(2.0)
                for f in cand:
                    for orient in (-1,+1):
                        for pv,osig in _v10a2_project_action(model,vv,sig,int(f),int(orient)):
                            if not pv: continue
                            raw_projected+=1
                            st0=next(iter(pv)); fk=_v10a3_flux_from_state(st0)
                            gsig=_v10a2_sig_canon(osig); ee=_v10a2_sig_E(osig)
                            key=(gsig,ee,fk)
                            # Classification here is diagnostic only.  Physical subtraction
                            # is done later with a Gram pseudoinverse, not by flux labels.
                            iplist=_V10A3_PLOOK.get(key,())
                            if iplist: class_hint['P-compatible']+=1
                            elif key[:2] in _V10A3_Q1_KEYS: class_hint['Q1-signature-compatible']+=1
                            else: class_hint['new-signature/flux']+=1
                            for j,cx in zip(gl,coeffs):
                                if abs(cx)<1e-14: continue
                                # W=-M.
                                _v10a3_addvec(actions[key][bright_map[j]],pv,-physfac*float(cx))
        if V10A3_PROGRESS and recs_touched and recs_touched%500<max(1,len(recs)):
            print(f"      Q2 action build: touched raw-Q1 recs={recs_touched:,}; blocks={len(actions):,}; projected={raw_projected:,}; elapsed={time.time()-t0:.1f}s")
    # prune cancellation zeros
    for key,d in list(actions.items()):
        for p,v in list(d.items()):
            z=_v10a3_prune(v)
            if z: d[p]=z
            else: del d[p]
        if not d: del actions[key]
    return {'bright':bright,'Y':Y[bright,:],'actions':actions,'raw_projected':raw_projected,
            'raw_q1_records_touched':recs_touched,'class_hint':dict(class_hint)}


def _v10a3_relevant_q1_vectors(model,groups,key):
    """All raw one-step Q1 generators in the same exact H0/rep/flux block."""
    gsig,ee,fk=key
    recs=groups.get((gsig,ee),())
    out=[]
    for rec in recs:
        if _v10a3_q1_record_flux(model,rec)!=fk: continue
        v=_v10_physical_raw_vec(model,rec)
        if v: out.append(v)
    return out


def _v10a3_factor_q2(model,qquot,anchor,q2haar):
    """Project second-step actions off retained P+Q1, then Gram-factor physical Q2."""
    ep,epcache,epcounts=_v10a3_endpoint_engine(q2haar)
    global _V10A3_PLOOK,_V10A3_Q1_KEYS
    _V10A3_PLOOK=_v10a3_retained_lookup(model)
    rawgroups=_v9_collision_groups(model['v9_collisions'])
    groups=defaultdict(list)
    for (sig,ee),recs in rawgroups.items():
        dsig=_v10a2_sig_canon(_v10a2_sig_dyn(sig))
        groups[(dsig,ee)].extend(recs)
    _V10A3_Q1_KEYS=set(groups)
    built=_v10a3_build_second_step_actions(model,qquot,anchor)
    actions=built['actions']; bright=built['bright']; nb=len(bright)
    keys=sorted(actions,key=lambda k:(float(k[1]),repr(k[0]),repr(k[2])))
    if V10A3_BLOCK_OFFSET>0:
        keys=keys[V10A3_BLOCK_OFFSET:]
    if V10A3_BLOCK_LIMIT>0:
        keys=keys[:V10A3_BLOCK_LIMIT]
    rr=[];cc=[];vv=[];E2=[];qoff=0
    raw_parent_dim=retained_raw_dim=q2_rank=q2_null=0
    min_q2_eig=0.0; min_ret_eig=0.0; max_proj_asym=0.0
    Kraw=np.zeros((nb,nb),dtype=np.float64)
    Kq2=np.zeros((nb,nb),dtype=np.float64)
    trace_raw=trace_removed=trace_q2=0.0
    blocks_multi=0; e0_rank=0; retained_blocks=0
    t0=time.time()
    for ib,key in enumerate(keys):
        d=actions[key]; parents=sorted(d); avec=[d[p] for p in parents]
        na=len(parents); raw_parent_dim+=na
        Gaa=_v10a3_vec_gram(avec,ep); Gaa=0.5*(Gaa+Gaa.T)
        trace_raw+=float(np.trace(Gaa))
        # Complete retained subspace in this exact H0/rep/flux block: P plus ALL
        # global first-step Q1 generators, not merely the source-facing Q1 basis.
        rvec=[]
        for ip in _V10A3_PLOOK.get(key,()): rvec.append(_v10a3_p_codd_vec(model,ip))
        rvec.extend(_v10a3_relevant_q1_vectors(model,groups,key))
        retained_raw_dim+=len(rvec)
        if rvec:
            retained_blocks+=1
            Grr=_v10a3_vec_gram(rvec,ep);Grr=0.5*(Grr+Grr.T)
            er,Ur=np.linalg.eigh(Grr); min_ret_eig=min(min_ret_eig,float(er.min(initial=0.0)))
            sr=max(1.0,float(er.max(initial=0.0))); mr=er>V10A3_GRAM_TOL*sr
            if float(er.min(initial=0.0))<-2e-8:
                raise RuntimeError(f'v10a.3 retained P+Q1 Gram not PSD: {er.min():.3e} key={key}')
            if np.any(mr):
                Gri=_v10a3_cross_gram(rvec,avec,ep)
                pinv=(Ur[:,mr]/er[mr][None,:])@Ur[:,mr].T
                corr=Gri.T@pinv@Gri
            else: corr=np.zeros_like(Gaa)
        else: corr=np.zeros_like(Gaa)
        G2=0.5*((Gaa-corr)+(Gaa-corr).T)
        max_proj_asym=max(max_proj_asym,float(np.max(np.abs(G2-G2.T),initial=0.0)))
        trace_removed+=float(np.trace(corr)); trace_q2+=float(np.trace(G2))
        ev,U=np.linalg.eigh(G2); min_q2_eig=min(min_q2_eig,float(ev.min(initial=0.0)))
        scale=max(1.0,float(ev.max(initial=0.0))); keep=ev>V10A3_GRAM_TOL*scale
        if float(ev.min(initial=0.0))<-2e-8:
            raise RuntimeError(f'v10a.3 projected Q2 Gram not PSD: {ev.min():.3e} key={key}')
        r=int(np.count_nonzero(keep)); q2_rank+=r; q2_null+=na-r
        # Accumulate independent raw/projected second-step kernels in bright-Q1 coords.
        for i,p in enumerate(parents):
            for j,p2 in enumerate(parents):
                Kraw[p,p2]+=Gaa[i,j]; Kq2[p,p2]+=G2[i,j]
        if r:
            if abs(float(key[1])-E0_PLAQ)<1e-11: e0_rank+=r
            Bl=U[:,keep]@np.diag(np.sqrt(ev[keep]))
            for i,p in enumerate(parents):
                for a,x in enumerate(Bl[i]):
                    if abs(x)>1e-14:
                        rr.append(p);cc.append(qoff+a);vv.append(float(x))
            E2.extend([float(key[1])]*r);qoff+=r
        if na>1: blocks_multi+=1
        if V10A3_PROGRESS and (ib==0 or (ib+1)%max(1,len(keys)//10)==0):
            print(f"      Q2 Gram {ib+1:,}/{len(keys):,}; rank={q2_rank:,}; retained-blocks={retained_blocks:,}; endpoint-cache={len(epcache):,}; elapsed={time.time()-t0:.1f}s")
    B12=coo_matrix((vv,(rr,cc)),shape=(nb,q2_rank),dtype=np.float64).tocsr()
    recon=B12@B12.T
    recon=recon.toarray() if hasattr(recon,'toarray') else np.asarray(recon)
    recon=recon-Kq2
    recon_err=float(np.max(np.abs(recon),initial=0.0))
    # Order-independent kernel check: all physical information in the factor is B12 B12^T.
    q2_psd=float(np.linalg.eigvalsh(0.5*(Kq2+Kq2.T)).min(initial=0.0))
    return {**built,'keys_processed':len(keys),'partial':bool(V10A3_BLOCK_LIMIT>0 or V10A3_BLOCK_OFFSET>0),
            'B12':B12,'E_Q2':np.asarray(E2,dtype=np.float64),'raw_parent_dim':raw_parent_dim,
            'retained_raw_dim':retained_raw_dim,'q2_rank':q2_rank,'q2_nullity':q2_null,
            'min_q2_eig':min_q2_eig,'min_retained_eig':min_ret_eig,'q2_kernel_min_eig':q2_psd,
            'reconstruction_error':recon_err,'e0_physical_rank':e0_rank,
            'trace_raw':trace_raw,'trace_removed':trace_removed,'trace_q2':trace_q2,
            'Kraw':Kraw,'Kq2':Kq2,'endpoint_cache':len(epcache),'endpoint_counts':dict(epcounts),
            'blocks_multi_parent':blocks_multi,'projection_asymmetry':max_proj_asym}


def _v10a3_direct_q2_order4(qquot,q2,anchor):
    """Direct P-W-Q1-W-Q2-W-Q1-W-P component on anchored T1 block only."""
    bright=np.asarray(q2['bright'],dtype=int)
    Y=np.asarray(qquot['B_W'].T@anchor,dtype=np.float64)[bright,:]
    E1=np.asarray(qquot['E_Q'],dtype=np.float64)[bright]
    E2=np.asarray(q2['E_Q2'],dtype=np.float64)
    if np.any(np.abs(E0_PLAQ-E1)<1e-12): raise RuntimeError('v10a.3 illegal Q1 E0 denominator')
    if np.any(np.abs(E0_PLAQ-E2)<1e-12): raise RuntimeError('v10a.3 illegal physical Q2 E0 denominator')
    R1=1.0/(E0_PLAQ-E1);R2=1.0/(E0_PLAQ-E2)
    Z=R1[:,None]*Y
    Z=np.asarray(q2['B12'].T@Z)
    Z=R2[:,None]*Z
    Z=np.asarray(q2['B12']@Z)
    Z=R1[:,None]*Z
    M=Y.T@Z
    return 0.5*(M+M.T)
...
# =============================================================================
print('='*128)
print('HODGE v10a.3 — PHYSICAL Q2 GRAM/H0 QUOTIENT + DIRECT ORDER-FOUR CERTIFICATE')
print('='*128)
print(f'backend                       : {DEVICE}')
print(f'retained P depth              : K={V10A3_P_DEPTH}')
print(f'anchored T1 polarizations     : {V10A3_ANCHOR_POLS}')
print(f'Q2 block offset/limit         : {V10A3_BLOCK_OFFSET} / {V10A3_BLOCK_LIMIT if V10A3_BLOCK_LIMIT else "ALL"}')
print('m4_rest / C3_full^shp         : BLINDED / DISABLED')
print('direct Q2 u^4 component       : REPORTED AS A COMPONENT ONLY')
print()

gate('v10a.3 SU(3) F x F weights sum to one',abs(W_A+W_S-1)<1e-15)
gate('v10a.3 SU(3) F x Fbar weights sum to one',abs(W_1+W_8-1)<1e-15)
validate_fast_haar_tensor_contract()

print('\n[1] INSTALL / CERTIFY EXTENDED Q2 HAAR')
q2haar,q2supported,q2cert,_q2cache=_v10a2_install_q2_haar(globals())
gate('v10a.3 balanced k=3 Weingarten inverse',q2cert['wg3_inverse_error']<5e-13,f"err={q2cert['wg3_inverse_error']:.3e}")
gate('v10a.3 pure-six SU(3) singlet rank is five',q2cert['rank60']==5,f"rank={q2cert['rank60']}")
gate('v10a.3 pure-six projector is idempotent',q2cert['T60_idempotence_error']<5e-13,f"err={q2cert['T60_idempotence_error']:.3e}")

print('\n[2] FULL-T1 RETAINED P + SOURCE-FACING PHYSICAL Q1')
source_faces=list(range(P))
t0=time.time();model=build_global_glueball_model(V10A3_P_DEPTH)
print(f"P={len(model['E_P']):,}; build={time.time()-t0:.2f}s; Q-actions={model['action_channels']:,}; global-Q null={model['gram_nullity']:,}")
expected={0:81,1:567,2:4077}.get(V10A3_P_DEPTH)
gate('v10a.3 retained full-T1 P dimension matches certified graph',expected is None or len(model['E_P'])==expected,f"P={len(model['E_P'])}, expected={expected}")
S=_v10a2_full_t1_source_block(model);A=_v10a2_anchor_block(model)[:,:V10A3_ANCHOR_POLS]
gate('v10a.3 full T1 source Gram remains I3',float(np.max(np.abs(S.T@S-np.eye(3))))<3e-12,f"err={np.max(np.abs(S.T@S-np.eye(3))):.3e}")
t0=time.time();sq=_v10a2_source_facing_q(model,S,keep_vectors=True)
print(f"Q1 source-facing raw={sq['raw_dim']:,} -> physical={sq['phys_dim']:,}+null={sq['nullity']:,}; elapsed={time.time()-t0:.2f}s")
gate('v10a.3 Q1 quotient reproduces Schur source kernels',sq['src_kernel_error']<5e-12,f"err={sq['src_kernel_error']:.3e}")
gate('v10a.3 Q1 physical vectors normalized',sq['orth_error']<5e-11,f"err={sq['orth_error']:.3e}")

print('\n[3] BUILD EXACT SECOND-STEP ACTIONS AND PROJECT P + COMPLETE GLOBAL Q1')
t0=time.time();q2=_v10a3_factor_q2(model,sq,A,q2haar)
print(f"anchored bright Q1              : {len(q2['bright']):,}")
print(f"raw projected second actions    : {q2['raw_projected']:,}")
print(f"H0/rep/flux blocks processed    : {q2['keys_processed']:,}" + (' (PARTIAL SMOKE)' if q2['partial'] else ''))
print(f"raw parent action vectors       : {q2['raw_parent_dim']:,}")
print(f"retained P+Q1 raw vectors used  : {q2['retained_raw_dim']:,}")
print(f"physical Q2 rank/nullity        : {q2['q2_rank']:,} / {q2['q2_nullity']:,}")
print(f"trace raw/removed/Q2            : {q2['trace_raw']:.12g} / {q2['trace_removed']:.12g} / {q2['trace_q2']:.12g}")
print(f"Q2 Gram min eig                 : {q2['min_q2_eig']:.3e}")
print(f"Q2 kernel min eig               : {q2['q2_kernel_min_eig']:.3e}")
print(f"B12 B12^T reconstruction err    : {q2['reconstruction_error']:.3e}")
print(f"physical E0=8/3 Q2 rank         : {q2['e0_physical_rank']}")
print(f"endpoint engine                 : cache={q2['endpoint_cache']:,}, counts={q2['endpoint_counts']}")
print(f"elapsed Q2 build/factor          : {time.time()-t0:.2f}s")
gate('v10a.3 physical Q2 complement is nonempty (or partial smoke)',q2['partial'] or q2['q2_rank']>0,f"rank={q2['q2_rank']}, partial={q2['partial']}")
gate('v10a.3 retained P+Q1 projection leaves PSD Q2 Gram',q2['min_q2_eig']>-2e-8,f"min={q2['min_q2_eig']:.3e}")
gate('v10a.3 physical Q2 kernel is PSD',q2['q2_kernel_min_eig']>-2e-8,f"min={q2['q2_kernel_min_eig']:.3e}")
gate('v10a.3 explicit W12 factor reproduces projected second-step kernel',q2['reconstruction_error']<2e-10,f"err={q2['reconstruction_error']:.3e}")
gate('v10a.3 no physical Q2 state remains resonant at E0=8/3',q2['e0_physical_rank']==0,f"rank={q2['e0_physical_rank']}")
gate('v10a.3 retained subtraction is nontrivial (or partial smoke)',q2['partial'] or q2['trace_removed']>1e-10,f"removed trace={q2['trace_removed']:.6g}, partial={q2['partial']}")

print('\n[4] DIRECT Q2 ORDER-FOUR COMPONENT — NOT THE PHYSICAL m4')
if q2['partial']:
    print('PARTIAL smoke mode: direct Q2 component intentionally not interpreted.')
    K4=None
else:
    K4=_v10a3_direct_q2_order4(sq,q2,A)
    print('K4_Q2_direct anchored T1 block =')
    print(K4)
    print(f"symmetry error={np.max(np.abs(K4-K4.T)):.3e}")
    gate('v10a.3 direct Q2 order-four anchored block is symmetric',float(np.max(np.abs(K4-K4.T)))<2e-10,f"err={np.max(np.abs(K4-K4.T)):.3e}")

print('\n[5] PROTECTED FOURTH-ORDER TARGETS — STILL REFERENCE ONLY')
print('A3^shp=5/48, B3^shp=0, D3^shp=0, c3^square=-5/48, alpha3=5/12')
print('NOT counted as passes: folds, P-return ledger, linked vacuum subtraction, and momentum operator extraction remain next.')

newg=gates[V10A3_GATE_START:]
print('\n'+'='*128)
print('FINAL v10a.3 GATE SUMMARY')
print('='*128)
for i,(name,ok,detail) in enumerate(newg,1): print(f"{i:02d}. {'PASS' if ok else 'FAIL'} — {name}"+(f" :: {detail}" if detail else ''))
passed=sum(ok for _,ok,_ in newg)
print('-'*128);print(f'PASSED {passed}/{len(newg)} v10a.3 GATES')
if passed!=len(newg): raise AssertionError('v10a.3 hard gate failure; do not interpret direct Q2 kernel')
print('\nV10A.3 CONCLUSION')
print('-----------------')
print('* The second-step space is projected against retained P and the complete relevant global one-step Q1 span by a block Gram pseudoinverse.')
print('* The surviving positive Gram range defines the anchored physical Q2 quotient and exact bare H0 resolvent.')
print('* W12 W21 reproduces the independently assembled projected second-step kernel.')
print('* No E0=8/3 physical Q2 resonance survives.')
print('* The reported K4_Q2_direct block is one component only; m4_rest and C3_full^shp remain blinded until folds/linked subtraction and protected mobility gates are installed.')
