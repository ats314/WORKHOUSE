...
def main()->None:
    ...
    n7p=recursive_find('y4_sun_walled_brauer_N7.json')
    assert all((certp,bp,bnp,qlp,qnp,qfp,n7p)),'Missing full symbolic certificate inputs.'
    ...
    # All-rank signs from the symbolic certificates.
    for row in cert['q']['fixed_rank_samples']:
        n=int(row['N'])
        assert sp.Rational(640,n*(n*n-1)**3)>0
        assert sp.cancel(bexpr.subs(N,n))>0

    # Complete fixed-rank kernel anchor bundled at N=7.
    n7=json.loads(n7p.read_text()); r=n7['results']; g=n7['gates']
    assert int(n7['meta']['rank'])==7 and int(r['kernel_entries'])==189
    assert sp.cancel(sp.Rational(r['q'])-qexpr.subs(N,7))==0
    assert sp.cancel(sp.Rational(r['A'])-sp.Rational(640,7*(7*7-1)**3))==0
    assert sp.cancel(sp.Rational(r['B'])-bexpr.subs(N,7))==0
    assert g['B_two_extractions_equal'] and g['cubic_kernel_entries_189'] and g['passed']
    ...
    print('PASS q_N stored exact fixed-rank matches for N=7,...,18')
    print('PASS A_N = 640/[N(N^2-1)^3]')
    print('PASS B_N denominator degree 409 and 403 positive Newton coefficients')
    print('PASS complete N=7 kernel matches q_7, A_7, B_7 and both B extractions')
