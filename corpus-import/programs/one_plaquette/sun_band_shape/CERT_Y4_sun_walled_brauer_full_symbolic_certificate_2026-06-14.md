...
**Date:** June 14, 2026  
**Scope:** integer $N\ge7$.  **Validation wording corrected in certificate v2.**
...
Hence $Q_{32}(N^2)>0$ and $q_N<0$ for every integer $N\ge7$. The 33 exact integers $b_j$, the ordinary polynomial coefficients, and the complete compact formula are included in the machine-readable ledgers. Stored exact fixed-rank values at every rank $N=7,8,\ldots,18$ match this $q_N$ formula. The bundled complete fixed-rank kernel is at $N=7$ and independently anchors $q_7$, $A_7$, and $B_7$.
...
- The two independent extractions of $B_N$ agree exactly:
  ...
  \]
- Stored exact $q_N$ values match the compact formula for $N=7,\ldots,18$, with $N=18$ used as a $q_N$ holdout.
- The bundled complete $N=7$ kernel has 189 entries and independently matches $q_7$, $A_7$, and $B_7$, including both $B_7$ extraction identities.
- The all-rank $A_N$ and $B_N$ formulas and signs are certified by exact symbolic identities, denominator factorizations, and positive Newton expansions; this bundle does not claim a complete $A/B$ fixed-rank kernel sweep for $N=8,\ldots,18$.
...
The bundle contains the certified output expressions, coefficient ledgers, hashes, the active symbolic verifier, the fixed-rank contraction engine, and a complete $N=7$ kernel artifact. It does not contain a standalone generator that derives the compact $q_N$ polynomial and structured $B_N$ expression directly from the 35,130-path corpus. Accordingly, these formulas are certified output artifacts with an independent $N=7$ full-kernel anchor, not a fully cold-regenerable symbolic-output chain from the packaged files alone.
