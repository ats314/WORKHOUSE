...
def section_A():
    ...
    gate("A8  wrong Fourier sign gives the TRANSPOSE (silent at 2nd order)",
         ...
         "same spectrum, flipped at 4th order")
...
def section_D():
    ...
    gate("D4  F_det = -6/343 and Delta q_6 = 6/343",
         Fdet == F(-6, 343) and -Fdet == F(6, 343), "343 = 7^3, three cuts")
...
def section_F():
    ...
    import sympy as sp
    N = sp.symbols('N')
    d5 = sp.Rational(45, 4096)*(N**2-1)**3*(N**2-4)**3*(N**2-9)*(N**2-16)/N**3
    d7 = sp.Rational(14175, 2**34)*(N**2-1)**8*(N**2-4)**7*(N**2-9)**4 \
        * (N**2-16)**3*(N**2-25)*(N**2-36)/N**7
    z5 = [n for n in range(2, 10) if sp.simplify(d5.subs(N, n)) == 0]
    z7 = [n for n in range(2, 10) if sp.simplify(d7.subs(N, n)) == 0]
    gate("F1  deg<=5 Gram vanishes at N=2,3,4 (valid N>=5); "
         "deg<=7 also at N=5,6 (valid N>=7)",
         z5 == [2, 3, 4] and z7 == [2, 3, 4, 5, 6],
         f"deg5 zeros {z5}, deg7 zeros {z7}")
