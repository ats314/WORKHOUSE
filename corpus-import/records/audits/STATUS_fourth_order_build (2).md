...
> **!! RETRACTED 2026-08-08 — see Stage 5 !!**
> The two conclusions originally recorded here are **WRONG** and must not be
> quoted. They are preserved only as a record of the reasoning that failed.
>
> ~~(1) Max link pile-up over all of fourth order is 3, so epsilon channels open
> only at N=3 and every N>=4 is generic.~~
> ~~(2) A (4,0) terminal family is unreachable at fourth order in 3D, so the
> certified SU(4) exceptional sector cannot originate where the documents place
> it.~~
>
> **Why they are wrong.** Both capped fundamentals-per-link by capping *cluster
> size* at three plaquettes, conflating distinct plaquettes in a cluster with
> the number of character insertions on a link. The SU(6) certificate settles
> it: its exceptional word is root + four insertions + output all on the SAME
> plaquette, putting (6,0) on every boundary link with a cluster of size one.
> Repeating one plaquette piles indices on its own links without growing the
> cluster at all. The correct selection rule is the N-ality family rule
> (p+q <= 6, p-q = +-N, matching parity), verified in Section E of VERIFY_ALL.
>
> There was never an inconsistency in the source documents — only in the model
> recorded here.

3. **ΔE is uniform only for the two-hop chain.** Every two-hop intermediate has
   exactly 4+4−2 = 6 excited links *when the shared link cancels*. Of the 36
   neighbour relations 24 have s=−1 (antiparallel, singlet available, 6 links)
   and 12 have s=+1 (parallel, no singlet in fund⊗fund for N>=3, 7 links with
   one at higher Casimir). The three-plaquette intermediates have three distinct
   multiplicity profiles. So denominators are **not** uniform.
...
4. **The completeness diagnostic must wait.** Because the twelve two-hop vectors
   ...
   open new shapes, only shift coefficients.
