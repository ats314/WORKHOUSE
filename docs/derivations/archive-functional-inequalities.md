...
F16 tests the original pairing targets; F17 supplies an exact signed SU(2) repair.
...
## F16. Extensive center defects obstruct uniform pairing coercivity

For every even L>=2, consider the periodic four-dimensional SU(2) lattice.
For a site x with coordinates in {0,...,L-1}, set

    U_mu(x)=(-1)^(sum_{nu<mu} x_nu) I.

This link field is periodic because L is even. For mu<nu, increasing x_mu
changes the sign of U_nu, including across the periodic seam; increasing
x_nu does not change U_mu. The four factors in each square therefore give
U_p=-I. With the fundamental trace defect z_p=1-Tr(U_p)/2,

    P=6L^4, z_p=2 for every p, D=2P, Bbar=2,
    grad z_p=0, P_S=sum_p z_p Gamma(S,z_p)=0.         (F16)

The gradient claim follows by varying any link: each first trace variation
is plus or minus Tr(X)=0 for X in su(2). The vanishing pairing holds for any
smooth action S; the Wilson action itself has zero gradient on this field.
Consequently these fields are stationary under the original Wilson gradient
flow. They have the following precise implications for Appendix I:

1. A bound P_S>=aD-b with a>0 and volume-independent finite b cannot hold
   globally: it would require 0>=12aL^4-b for arbitrarily large even L.
   Thus the Wilson premise in (I.5), and the corresponding premise of F7,
   is false on this family, although F7's conditional implication remains valid.
2. The off-core bound (I.9), P_S>=a_epsilon D outside {Bbar<=epsilon},
   fails for every epsilon<2 and a_epsilon>0: each displayed field lies outside.
3. The quadratic lower bound (I.8), P_S>=c2 P Bbar^2-C2 P with c2>0,
   can hold globally only if C2>=4c2. Its right side is then nonpositive
   throughout 0<=Bbar<=2. Combined with the nonnegative absolute diffusion
   budget in F5, this bound alone cannot prove negative drift at any Bbar.

These conclusions refute the specific global pairing targets, not all
Lyapunov drift. In particular Delta V can be negative at an action maximum;
F5's absolute diffusion bound discards that helpful sign. A signed-diffusion
argument, another core or weight, or an integrated estimate would require
its own proof. No claim about a physical transfer gap follows from this
stationary sampling-flow example. Source: retained pairing-targets.md,
Appendix I.2, equations (I.5), (I.8), (I.9). The construction and obstruction
are new deductions from the source targets during this follow-up.

## F17. The exact SU(2) diffusion budget retains favorable signs

Specialize to the fundamental trace on SU(2) with the unit-S3 metric,
whose orthonormal Lie algebra generators are i times the three Pauli
matrices. On this sphere, t=Tr(g)/2 obeys Delta_G t=-3t and
|grad_G t|^2=1-t^2. For z=1-t this gives

    Delta_G z=3(1-z), |grad_G z|^2=z(2-z).

One direct proof writes g as the unit quaternion (t,a1,a2,a3).
Along the three unit generator geodesics the scalar coordinate is
t cos(s)-aj sin(s). Its second derivatives sum to -3t, while the sum
of squared first derivatives is sum aj^2=1-t^2. This verifies both
identities without a radial-coordinate singularity.

For any distinct-link square plaquette, F3 now gives
Delta z_p=12(1-z_p) and Gamma(z_p)=4z_p(2-z_p). Consequently, at every
configuration, not only at center fields,

    Delta V=sum_p [2z_p Delta z_p+2Gamma(z_p)]
           =40D-32V,
    LW/W=k(40D-32V-2P_S)+k^2 Gamma(V),
    Gamma(V)<=16nu sum_p z_p^3(2-z_p).               (F17)

The last estimate is the same overlap Cauchy-Schwarz calculation as F5,
now using the exact single-plaquette gradient instead of its absolute
linear bound. In particular the diffusion contribution is negative when
V>(5/4)D. At an all-center link field, every z_p is 0 or 2 and every
plaquette gradient is zero, so V=2D, P_S=0 and Gamma(V)=0. Thus

    LW/W=-24kD,

for any smooth action, with strict negativity whenever D>0 and k>0.
On the extensive family F16 this is -48kP. The obstruction to uniform
positive pairing is therefore fully compatible with favorable actual
drift at precisely those configurations. The exact signed budget supplies
a better starting identity than Cv D, but a global off-core bound must
still control the pairing and overlap term away from center fields.
A different metric rescales these constants; no metric-independent or
physical-transfer conclusion is asserted.

## Application boundary

F1–F6, F8–F13 and F17 are analytic lemmas with explicit hypotheses; F7 is the
conditional Wilson-facing drift interface; F14–F16 are exact negative
