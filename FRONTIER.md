...
- Scope: Actual four-face SU(2) conditional source law, uniformly as g tends to zero, with chart-complement and rare-fiber contributions retained.
- Source inputs: `DERIV:W6_CONDITIONAL_SCORE_TAIL_CONTROL:ACTUAL_SOURCE_MOMENT` (proven), `DERIV:W6_CONDITIONAL_SCORE_TAIL_CONTROL:INTERVAL_OBSTRUCTION_REPAIR` (proven)
...
- Decisive next test: Derive the actual fiber score and compare its squared weight with 1+g^-2 E(V|w) on the near-well region and a rare-fiber/tail region; either obtain common constants or exhibit the precise failing regime before choosing a weaker sufficient pairing estimate.
