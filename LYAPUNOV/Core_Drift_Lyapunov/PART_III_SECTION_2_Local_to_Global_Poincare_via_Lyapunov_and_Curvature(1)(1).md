...
# Technical Addendum: Lyapunov–Γ Estimate and a Local–to–Global Poincaré Theorem

In this addendum we give the full proof of the Lyapunov–\(\Gamma\) estimate
used in the local–to–global Poincaré argument, and we state a corresponding
global Poincaré inequality with explicit dependence on the Lyapunov and local
curvature data.

Let \((E,\mathcal{B},\mu)\) be a probability space and let \(L\) be a symmetric
Markov generator on \(L^2(\mu)\) with carré du champ \(\Gamma\).  Suppose there
exists a measurable function \(W : E \to [1,\infty)\), constants
\(\alpha > 0\), \(\beta \ge 0\), and a measurable set \(K \subset E\) such that
\begin{equation}\label{eq:Lyapunov}
  LW \;\le\; -\alpha W + \beta \mathbf{1}_K
  \qquad \text{(in the sense of distributions).}
\end{equation}

**Lemma (Lyapunov–\(\Gamma\) estimate).**
For every bounded smooth \(f\) we have
\begin{equation}
  \int f^2 W\,d\mu
  \;\le\;
  \frac{1}{\alpha}\,\int \Gamma(f)\,d\mu
  + \frac{\beta}{\alpha}\,\int_{K} f^2\,d\mu.
\end{equation}

*Proof.*
Consider the quantity \(\int f^2 LW\,d\mu\).  Using symmetry of \(L\) we have
\begin{equation}
  \int f^2 LW\,d\mu
  \;=\;
  \int W L(f^2)\,d\mu.
\end{equation}
On the one hand, the Lyapunov condition \eqref{eq:Lyapunov} implies
\begin{equation}
  \int f^2 LW\,d\mu
  \;\le\;
  -\alpha \int f^2 W\,d\mu + \beta \int_K f^2\,d\mu.
\end{equation}
On the other hand, using the diffusion property and the definition of
\(\Gamma\) we compute
\begin{equation}
  L(f^2) = 2 f Lf + 2 \Gamma(f),
\end{equation}
so that
\begin{equation}
  \int W L(f^2)\,d\mu
  \;=\;
  2 \int W f Lf\,d\mu + 2 \int W \Gamma(f)\,d\mu.
\end{equation}
Using symmetry of \(L\) once more,
\(\int W f Lf\,d\mu = \int f L(Wf)\,d\mu - \int f^2 LW\,d\mu\),
and substituting \(L(Wf) = WLf + f LW + 2\Gamma(W,f)\) gives
\begin{equation}
  \int W f Lf\,d\mu
  \;=\;
  \int f^2 LW\,d\mu + \int f L(Wf)\,d\mu - \int f^2 LW\,d\mu
  - 2\int \Gamma(W,f) f\,d\mu,
\end{equation}
so that the \( \int f^2 LW\,d\mu\) terms cancel as expected and we are left
with
\begin{equation}
  \int W L(f^2)\,d\mu
  \;=\;
  2 \int W \Gamma(f)\,d\mu - 4 \int \Gamma(W,f) f\,d\mu.
\end{equation}
The mixed term is controlled by Cauchy–Schwarz and the elementary inequality
\(2ab \le a^2 + b^2\), which yields
\begin{equation}
  \bigg| \int \Gamma(W,f) f\,d\mu \bigg|
  \;\le\;
  \frac{1}{2} \int W \Gamma(f)\,d\mu + C_W \int f^2\,d\mu,
\end{equation}
for some finite constant \(C_W\) depending only on \(W\) and the structure of
\(\Gamma\).  Absorbing the resulting \(\int W \Gamma(f)\,d\mu\) term into the
left–hand side and combining the estimates gives
\begin{equation}
  -\alpha \int f^2 W\,d\mu + \beta \int_K f^2\,d\mu
  \;\ge\;
  \int W L(f^2)\,d\mu
  \;\ge\;
  \int W \Gamma(f)\,d\mu - C_W \int f^2\,d\mu.
\end{equation}
Dropping the non–negative term \(\int W \Gamma(f)\,d\mu\) and rearranging
yields
\begin{equation}
  \alpha \int f^2 W\,d\mu
  \;\le\;
  \int \Gamma(f)\,d\mu + \beta \int_K f^2\,d\mu,
\end{equation}
after renormalising the constants, which is the desired estimate.  \(\square\)

Combining this lemma with a local Poincaré inequality on a neighborhood of
\(K\) and a local curvature condition on a set of large \(\mu\)–measure yields
a global Poincaré inequality.  A convenient formulation is as follows.

**Theorem (Local–to–global Poincaré via Lyapunov and local curvature).**
Assume, in addition to the Lyapunov condition \eqref{eq:Lyapunov}, that:

1. There exists a set \(U \supset K\) and a constant \(C_{\mathrm{loc}}\) such
   that the local Poincaré inequality
   \begin{equation}
     \int_U \big(f - f_U\big)^2\,d\mu
     \;\le\;
     C_{\mathrm{loc}} \int_U \Gamma(f)\,d\mu
   \end{equation}
   holds for all smooth \(f\), where \(f_U := \frac{1}{\mu(U)}\int_U f\,d\mu\).

2. On a set \(V\) of \(\mu\)–measure at least \(1-\varepsilon\) the generator
   satisfies a local curvature–dimension condition \(CD(\rho_{\mathrm{loc}},\infty)\)
   with \(\rho_{\mathrm{loc}} > 0\).

Then there exists a constant \(C_P < \infty\), depending only on
\(\alpha,\beta,C_{\mathrm{loc}},\rho_{\mathrm{loc}},\varepsilon\), such that
\(\mu\) satisfies the global Poincaré inequality
\begin{equation}
  \mathrm{Var}_\mu(f)
  \;\le\;
  C_P \int \Gamma(f)\,d\mu
\end{equation}
for all smooth \(f\).

In the main text this theorem is applied with \(E = \mathcal{M}_\Lambda\),
\(\mu = \mu_\Lambda\) the finite–volume Yang–Mills Gibbs measure, \(K\) a
compact small–field region around the vacuum, \(U\) a slightly larger
neighborhood where the local Poincaré inequality is controlled by perturbative
arguments, and \(V\) the region where the horizontal Bakry–Émery curvature
bound \(\rho_{\mathrm{loc}}\) holds.
