...
\date{Revision 5 --- 30 August 2026}
\maketitle
...
is conditional, that condition is stated separately.
...
vacuum route vanishes. In the charge-even sector, however, this proposition
does \emph{not} dispose of the non-adjacent vacuum route. Taken over all pairs
it would be a rank-one operator supported at $k=0$, not the twelve-neighbour
adjacency multiplied by $\ell_N$. The check above establishes the all-rank
shared-link value $\ell_N$; the reorganisation or cancellation of the
charge-even all-pairs term remains a separate requirement.
\end{remark}
...
One earlier physical identification remains conditional. That the complete
linked charge-even Bloch symbol is the unsigned incidence
...
The separate program \texttt{audit\_radius2\_attachment.py} verifies all
twelve files in the radius-two manifest, rejects unsafe or pickle-bearing NPZ
members, and independently recomputes $34$ matrix, spectrum, gap, onset and
scope identities from the serialized result. The manifest-pinned
\texttt{verify\_two\_cube\_cutoff\_free\_radius2\_finite\_u\_release.py}
then binds the two sealed parity-sector parent triplets and reconstructs the
three nested spectra, direct incremental operators and stable weak-$u$ shifts.
It returned
\texttt{DETACHED\_RADIUS\_TWO\_RELEASE\_RECOMPUTED} with zero maximum
reconstruction error. This is a replay relative to supplied hash-pinned
parents, not an external contraction or a proof that the declared frontier is
complete.
