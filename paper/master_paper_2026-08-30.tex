...
The orbit basis makes that pattern sharper than a list, because it gives
``momentum-independent'' an exact meaning. The on-site orbit's carrier
projection is exactly $\sigma e_1$, so $\varepsilon = T/q = \sigma$ is constant
over the zone and carries no $A$, $B$, $C$ or $D$; conversely a correction
recorded as a pure scalar shift \emph{is} an on-site-orbit shift and nothing
else. Read that way the exceptional record is uniform --- at $SU(6)$ the
determinant sector shifts the on-site orbit and stops there, at $SU(5)$ there is
no such sector to shift, at $SU(4)$ the offsets vanish --- and $N=3$ is the
single rank at which it leaves that orbit.
\chk{the on-site orbit IS the momentum-independent channel}

Where it goes instead is then forced, and the consequence is exact. Both sides
of the dispute agree $\alpha=5/12$, so $\Delta A_3=0$; with $A=-\nu-4u$ that
gives $\Delta\nu=-4\Delta u$. The primitive cube-completion channel is
link-balanced --- one $U$ and one $U^\dagger$ on every link, so $\nu_\ell=0$ ---
hence $\epsilon$-blind, and that channel is the normal orbit, so $\Delta\nu=0$
and therefore $\Delta u=0$; and $\Delta u_2=0$ because $u_2=2u$ holds exactly in
both recorded kernels. Since
$C_{\mathrm{shp}}=-\tfrac{5}{96}-u-\tfrac12(\rho+\pi)$, the sector has exactly
one place left to act. With $\Delta\beta_3=25/64$,
\[
  \Delta(\rho+\pi)\;=\;-\tfrac{25}{512},
  \qquad \Delta u=\Delta u_2=\Delta\nu=\Delta\sigma=0 .
\]
A direct balanced contraction at $N=3$ --- which this repository does not hold
--- must therefore return $\rho+\pi$ shifted from the historical
$-12778990891087/1595026186790400$ by exactly that amount. The falsifier is
correspondingly sharp: any such contraction differing from the historical
amplitudes anywhere but in $\rho+\pi$, or shifting $\rho+\pi$ by anything else,
refutes the reading. It prefers neither side of the fourth-order dispute, and it
does not use the forbidden substitution --- only $\Delta\beta_3$, which is
recorded independently.
\chk{CORRECTED PREDICTION: the eps-sector at N=3 is Delta(rho + pi~) = -25/512; u is NOT constrained}

\paragraph{Correction (2026-09-01).} The step $\Delta\nu=0\Rightarrow\Delta u=0$ above
is withdrawn. The normal orbit is $\nu=\tilde\nu-4u$ with $\tilde\nu=-5/48$ the
primitive cube completion and $-4u$ the diagonal shadow of the two-hop sector
$u\,S_\square^2$; eps-blindness of the primitive channel fixes $\tilde\nu$, which is
the same statement as $\Delta A=0$, and constrains $u$ not at all --- the two
recorded kernels both have $\tilde\nu=-5/48$ exactly with $u$ differing by a factor
$4.1327437$. With $\tilde\pi=\pi+2u$ the identity is $C=-5/96-(\rho+\tilde\pi)/2$, so
the prediction that survives is $\Delta(\rho+\tilde\pi)=-25/512$ with $\tilde\nu$
unchanged and $u$, $u_2$, $\tilde\sigma$ free. The label above names the corrected
check; the prose of this section is kept as written on 30 August. See ADR 0019.
...
\input{research_notes/G18_FIXED_SPACING_CARRIER_BRIDGE_INSERT}
...
\input{research_notes/G19_CONTINUUM_BRIDGE_INSERT}
