...
dispute is two shared-link amplitudes. Three amplitudes have been recomputed
from outside both pipelines by cluster expansion: the two-hop weight and the
in-plane and normal shared-link amplitudes agree with the historical kernel
exactly, the cold rival is wrong on all three, and the rotation amplitude
...
axial datum can decide it. The rational and polynomial identities the paper
rests on are collected as ninety-five Lean~4 theorems, proof-checked with no
omitted proof.
...
physical sector is conditional, that condition is stated separately. Every
rational or polynomial identity displayed below is also proof-checked: a
statement carrying a \leanref{name} tag is proved in Lean~4 from explicit
definitions, and Appendix~\ref{app:lean} lists all ninety-five such theorems.
The tag certifies the arithmetic of the statement it sits beside and nothing
about the physics that produced it.
...
\item The two recorded fourth-order kernels are one Hodge polynomial plus one
  ...
  amplitudes; three amplitudes recomputed from outside both pipelines
...
$8N^2-5(N^2-1)=3N^2+5$ and $6N^2-5(N^2-1)=N^2+5$
\leanref{zero\_nality\_clears\_window}; the enumeration of the $SU(3)$ one-link
numerators below $18$ \leanref{su3\_numerators\_below\_eighteen}; the semigroup
...
\end{equation}
an identity of rational functions \leanref{hopping\_deficit\_numerator}, and
the right side is positive and the left strictly decreasing for every real
...
\]
with $\mathcal N(k)$ the unsigned incidence of Section~\ref{sec:even} and
...
\end{equation}
so that $d_{2,N,L}=d^-_{2,N}$ in Corollary~\ref{cor:assembly}, the flat scalar
is $d^-_{2,N}-4t_N$, and in the charge-even sector the $A_1^{++}$ level at
...
$-\tfrac{481}{612}$ before and $-\tfrac{11}{306}$ after the $-\tfrac34$
bubble, and $0$ on a link-disjoint pair, from the primitives --- the last of
...
is reached by exactly two monomials, $(\epsilon,\epsilon')=(+,+)$ and $(-,-)$
when $ss'=1$ and the other two when $ss'=-1$, each with coefficient
$\epsilon\epsilon'/2=\tfrac12$; the first puts $F\otimes F$ on the shared link
and the second $\bar F\otimes\bar F$, that is $\rho$ and $\bar\rho$, with the
same weight $d_\rho/N^2$ because Haar measure is conjugation invariant. The
mixed channels are self-conjugate, and their two monomials add with the sign
$\eta_\rho=-1$. Charge conjugation commutes with $H_E$, with $V$ and with any
...
Proposition~\ref{prop:vacuum} is cancelled pair by pair, and the adjacent
element is $\ell_N$ whatever the orientations. At order $u^3$ the
identification remains a hypothesis, because the third-order charge-even
...
charge-even band is the wider one at every rank, unconditionally, and that
...
Laplacians (Section~\ref{sec:hodge-form}). It reports three of the kernel's
amplitudes recomputed from outside both pipelines, which decides the
...
\subsection{Three amplitudes computed from outside both pipelines}\label{sec:outside}

The lattice fourth-order matrix element between two plaquettes is a finite
sum of connected cluster cumulants, and the smallest clusters can be evaluated
...
\item \textbf{The rotation amplitude.} The same assembly on the
  ...
  \[
   ...
   =+0.0405972,
  \]
  against the historical $+0.0082309$ and the cold $-0.0879716$: neither.
\end{enumerate}
\end{reported}
...
\leanref{C\_shp\_from\_orbits\_cluster}. What the three outside
computations decide is standing, not the coefficient. The cold pipeline is
wrong on every amplitude checked --- $u$, $\pi$, $\rho$ --- and until its
...
right on $u$, $\pi$ and $\nu$ and wrong on $\rho$, so its $C_{\mathrm{shp}}$
is not the fourth-order coefficient either. The assembled $\rho$ rests on the
...
exactly $-55/13872$, so the $\epsilon$-sector of $\rho+\tilde\pi$ is
$-55/6936$ \leanref{eps\_sector\_refutes\_U5}. The continuation shift $25/64$
...
says what the two vectors are: the diagonal shadow of $S_\square^2$, so that
in orbit amplitudes $D=-6u+3u_2$ with $u_2=2u$ and $B=(+1,+1,-2)\cdot x$
vanish identically \leanref{tier\_collapse\_orbits}. What that leaves for a
...
span. The third value \eqref{eq:third-side} is a case in point: with
$W_4=\alpha+\beta$ and $\beta=8A+16C_{\mathrm{shp}}$
\leanref{width\_eq\_alpha\_add\_beta}, \leanref{beta\_from\_A\_and\_C}, it
carries the smallest bandwidth of the three, and the exclusion radius is
stated for whichever record is in hand. The statement is non-vacuous only for $u<0.133$ at $\theta=1/2$,
...
Two-hop, in-plane and normal fourth-order amplitudes
 & Reproduced from outside & Cluster expansion from the engine's primitives; two implementations \\
...
This edition is pinned in the verifier at commit
\nolinkurl{304dbcd15d2deda16dbfce576c52035ef4e7cdde}, where the registry holds $292$ checks in $28$ suites,
all passing, and the Lean core holds $95$ theorems with no omitted proof and
no axiom beyond the three standard ones. Every displayed statement carries, in
...
coefficient; three of its six amplitudes have been recomputed from outside
both pipelines, refuting the cold record on all three and the historical
...
\item \texttt{hopping\_deficit\_numerator} --- The \texttt{N$^-$$^3$} cancellation leaves a positive deficit, denominators cleared:
...
\item \texttt{even\_cubic\_at\_sixteen} --- The band ceiling, in the form that carries the bound: at \texttt{$\mu$ = 16} the cubic is \texttt{16(16-p)$^2$ - 4abc}.
\item \texttt{even\_cubic\_derivative\_factors} --- Above \texttt{p} the cubic is strictly increasing, because its derivative factors as \texttt{(3$\mu$ - p)($\mu$ - p)}.
...
\item \texttt{extraction\_A} --- The four checkpoint-extraction formulas invert the cubic-invariant ansatz.
...
\item \texttt{delta\_P} --- The checkpoint delta at \texttt{P = ($\pi$, $\pi$/2, 0)}:
...
\item \texttt{evenHop\_two} --- The charge-even hop never vanishes where the charge-odd one does:
...
\item \texttt{zero\_nality\_clears\_window} --- The one-form flux exclusion at \texttt{L = 4}: a once-winding fundamental loop sits at \texttt{2C\_F} for every \texttt{N}, while a zero-\texttt{N}-ality loop costs \texttt{2C\_2(R) $\ge$ 2N}, which clears the window \texttt{5C\_F/2 = 5(N$^2$ $-$ 1)/(4N)}; denominators cleared:
...
\item \texttt{su3\_numerator\_large} --- Any label above \texttt{4} already exceeds \texttt{18}:
...
\item \texttt{riesz\_separations} --- The two separations of the Riesz island, from the neighbours \texttt{14} and \texttt{17} of the shell \texttt{16} in units of \texttt{1/6}, each interval dilated by \texttt{1  $\lambda$}.
...
\item \texttt{kinematic\_constants} --- The local interaction bound \texttt{27|u|}: the \texttt{3/2} rescaling, three plaquettes per cell, and \texttt{ +  $\le$ 6}; and the rescaled SU(3) local gap \texttt{(3/4) C\_F = 1}.
...
\item \texttt{even\_cubic\_pos\_above\_sixteen} --- Above \texttt{$\mu$ = 16} the cubic is strictly positive when each \texttt{a\_i $\in$ [0, 4]}, so \texttt{$\lambda$ $\le$ 12}:
\item \texttt{two\_cube\_spectra} --- \texttt{B = 6}: the four cross-cell pairs split \texttt{D  c} about \texttt{$-$2317/612} by \texttt{c = 5/612}, the end caps sit at \texttt{$-$2295/612 = $-$15/4}; \texttt{B = 4}: about \texttt{$-$7/4} by \texttt{c = $-$1/12}.
...
\item \texttt{eps\_sector\_refutes\_U5} --- The $\epsilon$-sector of \texttt{$\rho$ + $\pi$} measured by the $\epsilon$-blind assembly, \texttt{2 $\times$ ($-$55/13872) = $-$55/6936}, is not U5's \texttt{$-$25/512}.
\end{itemize}
\endgroup
