...
def momentum_path(points: list[tuple[float, float, float]], per_segment: int = 180):
    ...
    for left, right in zip(points[:-1], points[1:]):
        ...
        pieces.append(np.asarray(left)[None, :] * (1 - t[:, None]) + np.asarray(right)[None, :] * t[:, None])
...
def second_order_figure():
    points = [(0, 0, 0), (np.pi, 0, 0), (np.pi, np.pi, 0),
    ...
    for i, (pi, prod) in enumerate(zip(p, product)):
    ...
    ax.plot(flat + np.zeros_like(q), color="#b33a2e", lw=2.7,
            label=r"signed carrier $11/306$")
    ax.plot(odd_partner, color="#b33a2e", lw=1.8, ls="--",
            label="signed dispersive doublet")
    for branch in unsigned.T:
        ax.plot(branch, color="#2569a8", lw=1.7, ls=":")
    ax.plot([], [], color="#2569a8", lw=1.7, ls=":",
            label="unsigned comparison")
    ...
    ax.annotate("flat for every momentum", xy=(ticks[2], flat),
                xytext=(ticks[2] - 95, flat + 0.085),
                arrowprops={"arrowstyle": "->", "color": "#b33a2e"},
                color="#b33a2e", fontsize=9)
...
def historical_fourth_order_figure():
    ...
    ax.scatter([ticks[0], ticks[3]], [qold, qold + alpha + beta],
               color=["#147a68", "#a84432"], zorder=4)
    style_axis(ax, ticks, [r"$\Gamma$", "$X$", "$M$", "$R$", r"$\Gamma$"])
    ...
    ax.annotate(r"$\Gamma$: scalar anchor", xy=(ticks[0], qold),
                xytext=(ticks[0] + 45, qold + 0.04), fontsize=9)
    ax.annotate(r"$R$: $\Delta W_4=\alpha+\beta$", xy=(ticks[3], qold + alpha + beta),
                xytext=(ticks[3] - 145, qold + alpha + beta - 0.055),
                arrowprops={"arrowstyle": "->", "color": "#a84432"},
                color="#a84432", fontsize=9)
...
def cellular_carrier_figure():
    fig, (ax_cube, ax_chain) = plt.subplots(
        1, 2, figsize=(9.4, 4.25), gridspec_kw={"width_ratios": [0.9, 1.35]},
    ...
    for vertices, color in faces:
        ax_cube.add_patch(Polygon(vertices, closed=True, facecolor=color,
                                  edgecolor="#243447", lw=1.5))
    for u, v in [(a, a2), (a2, b2), (a2, d2), (b2, c2), (d2, c2)]:
        ax_cube.plot([u[0], v[0]], [u[1], v[1]], color="#59636e",
                     lw=1.1, ls="--" if np.array_equal(u, a) else "-")
    ...
    ax_cube.text(1.82, 0.98, r"$+[x+e_1;23]$", color="#216533", ha="center",
                 rotation=81, fontsize=9.5)
    ax_cube.text(1.1, -0.28, "one oriented cube boundary", ha="center",
    ...
    for y0, y1, label in [(0.86, 0.73, r"$\partial_3$"),
                          (0.64, 0.51, r"$\partial_2$")]:
        ax_chain.add_patch(FancyArrowPatch((0.5, y0), (0.5, y1),
                                           arrowstyle="-|>", mutation_scale=13,
                                           lw=1.4, color="#243447"))
        ax_chain.text(0.54, (y0 + y1) / 2, label, va="center", fontsize=12)
    ax_chain.text(0.68, 0.57, r"$\partial_2\partial_3=0$", fontsize=12,
                  color="#9b2f26", va="center")

    boxes = [
        (0.03, 0.17, 0.42, 0.18, "cube boundaries",
         r"$\dim\operatorname{im}\partial_3=L^3-1$", "#edf4fa"),
        (0.55, 0.17, 0.42, 0.18, "wrapping classes",
         r"$\dim H_2(T^3)=3$", "#f8f1e7"),
    ]
    for x, y, w, h, title, formula, color in boxes:
        ax_chain.add_patch(FancyBboxPatch((x, y), w, h,
                                         boxstyle="round,pad=0.018",
                                         facecolor=color, edgecolor="#6a737d", lw=1.0))
        ax_chain.text(x + w / 2, y + 0.125, title, ha="center", fontsize=9.5)
        ax_chain.text(x + w / 2, y + 0.055, formula, ha="center", fontsize=10.5)
    ax_chain.text(0.5, 0.055,
                  r"$\ker\partial_2=\operatorname{im}\partial_3\oplus H_2,$"
                  r"  $\dim\ker\partial_2=L^3+2$",
                  ha="center", fontsize=11.5, fontweight="bold")
    ax_chain.set_title("Cellular origin of the carrier", fontsize=11.5,
