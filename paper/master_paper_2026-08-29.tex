...
genuine face-sharing two-cube $SU(3)$ space of $1{,}590{,}462$ states, and the
...
$SU(3)$ third- and fourth-order statements remain conditional on supplied
...
\item A supplied two-cube delivery reports six link-irrep channel
  ...
  that space is the same truncation as the published link cutoff of
  item~\ref{item:cutoff}. Conditional on the delivered diagonals, the symmetry
  analysis of Proposition~\ref{prop:orbit} applies to them: the orbit structure
  itself is exact and takes no delivered input.
  The $1{,}590{,}462$-state build was not reproduced here.
...
\begin{proof}
The diagonal and within-plaquette terms are scalars in face-orbital space, and
they are scalars for a reason worth naming, because it fails elsewhere in this
paper: translations act transitively on the faces of each orientation and the
cubic point group permutes the three orientations, so the faces are a
\emph{single} orbit and any invariant diagonal is a multiple of the identity.
On an open cluster that is false, and Proposition~\ref{prop:orbit} is what
replaces it there.
...
giving the charge-even width $16|\ell_3|=88/153$.
...
builder, graph fit, branching rule or validator, with the payload hashes frozen
before any comparison --- and this repository re-thresholds those gates rather
...
weighted equally --- which is measured here on the $1{,}590{,}462$-state space
rather than assumed.
...
sextet needs exactly $6$ and the adjoint $17/3$, so both survive $B=6$ and
neither survives $B=4$.
...
routes, so $-1/12$ is its projected hopping and $14/153$ is the counterterm
...
$1{,}590{,}462$-state construction was not re-run here: its arithmetic is
...
spectrum has two levels, so the charge-even rest frame can. The $A_1^{++}$
...
contributes $8/3+u+u^2/2+7u^3/32$.
...
coefficient --- it is the balanced $\epsilon$-free continuation of the
...
than by redistribution, which is a constraint on \emph{any} candidate,
including both recorded ones. Neither record is promoted here and neither is withdrawn: the interval of
...
\subsection{A near-$\Gamma$ statement that survives the dispute}
...
$k=0$ and at the zone corner --- and $\tau(u)\ge t_3u^2$ for $u>0$, the
...
float --- and the larger bounds both, so the exclusion radius can be stated
now. The statement is non-vacuous only for $u<0.133$ at $\theta=1/2$,
...
--- the NPB 205 (1982) erratum row and Smit's Table 1 scalar column, which
credits them --- agree rational for rational through $u^8$. That is agreement
...
exactly $-96$, a fourth correction already in print by 1983 that no erratum
records. This is a Euclidean record; no Hamiltonian comparison is drawn from
...
in 1988 that at $\beta=5.9$ a bare local operator carries only a couple of
percent of the physical glueball state, and gave the scaling $a^5$ for a local
dimension-four operator --- which is why that percentage needs its coupling
quoted alongside it. That makes the smeared basis structural rather than a convenience:
better statistics cannot recover a power law. It is the published, independent
form of the obstruction this program records as its own largest debt.
...
Two premises earlier in the paper are arguments in prose rather than results,
and both are load-bearing. That the range of $P_-$ is exactly the charge-odd
one-plaquette span for $L\ge3$ is argued in Section~\ref{sec:carrier} from the
link count of a reduced fundamental loop. That the four fusion channels
exhaust the second-order shared-link routes is assumed in
Theorem~\ref{thm:allrank}; Theorem~\ref{thm:weingarten} fixes the weight of
each channel and does not establish that the list is complete. Neither has a
check.
...
twenty-three checks in nine claim groups: the rational coefficient ledger; the
...
$B=6$ and $B=4$ spectra, and Reported result~\ref{rep:census} channel by
channel; the charge-even cubic of Theorem~\ref{thm:evencubic} with $p+q=12$ and
the four high-symmetry spectra; the checkpoint algebra of
\eqref{eq:checkpoints} together with the vanishing of $e_2$ on every axial cut;
...
\texttt{5ca91a92ac3b15b4a9ef5138c408f08947b16ffa} of the accompanying
...
\texttt{git checkout 5ca91a9}, \texttt{make verify} gives the $T_1$ and $T_2$
...
exact-rational and polynomial algebra only: no perturbative derivation, no
operator theory, and nothing that bears on $C_{\mathrm{shp}}$ appears there,
because none of that reduces to rational arithmetic.
...
In the same convention, with $\lambda\in\{12,-4\}$ and \eqref{eq:assembly}:
