...
\item For every $N\ge3$ the complete momentum dependence at order $u^2$ is
  $t_Nu^2BB\dg$ (Theorem~\ref{thm:allrank}), on the one premise
  Section~\ref{sec:scope} names and no check covers: that one-shared-link
  processes exhaust the second-order off-diagonal intermediates.
...
span. The statement is non-vacuous only for $u<0.133$ at $\theta=1/2$,
...
Three premises earlier in the paper are arguments in prose rather than results,
and all three are load-bearing. None has a check.

That the range of $P_-$ is exactly the charge-odd one-plaquette span for
$L\ge3$ is argued in Section~\ref{sec:carrier} from the link count of a reduced
fundamental loop.

That the four fusion channels exhaust the second-order shared-link routes is
assumed in Theorem~\ref{thm:allrank}. Three of its ingredients \emph{are}
settled: the two tensor-product decompositions \eqref{eq:fusion} are complete
by representation theory, two distinct faces of this lattice share at most one
link, and the vacuum-mediated route vanishes in this sector
(Proposition~\ref{prop:vacuum}). What is not settled by any of them is that
one-shared-link processes exhaust the second-order \emph{off-diagonal}
intermediates. The gap is in the process enumeration, not in the channel list,
and Theorem~\ref{thm:weingarten} is silent about it.
