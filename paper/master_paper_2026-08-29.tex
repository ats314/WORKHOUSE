...
\title{\bfseries Homological flat bands and channel-resolved two-cube hopping
in strong-coupling $SU(N)$ Hamiltonian lattice gauge theory\\[2mm]
\large With machine-checked provenance}
...
The conversion those labels check is explicit, and the object it converts is
used again in Section~\ref{sec:even}, so it is written out here. At $N=3$ the
within-plaquette one-plaquette series of the two sectors are
...
\end{equation}
in the corpus's own printed coefficients. Then $4\Delta_-(3u/2)$ and
$4\Delta_+(3u/2)$ are the charge-odd and charge-even towers in canonical $u$,
and the erratum is exactly the claim that no further rescaling stands between
the two coordinates. The letter $\Delta$ is reused elsewhere with other
subscripts --- $\Delta_s$ for $s\in\{X,M,P,R\}$ are the fourth-order checkpoint
differences of Section~\ref{sec:fourth}, $\Delta_C$ the gap between the two
recorded off-axis coefficients, and $\Delta_L$ the finite-volume incidence separation of
Section~\ref{sec:scope}. Only the sector subscript $\pm$ means this series.
...
trivial electric centre flux around each torus cycle. Write $\Pi_{F,-}$ for the
full $E_{F,N}$ spectral projector there and $P_-$ for the span of the
elementary charge-odd one-plaquette states \eqref{eq:states}. We take as a
\emph{hypothesis} that these coincide for $L\ge3$,
\begin{equation}\label{eq:shell}
  P_-=\Pi_{F,-}\qquad(L\ge3),
\end{equation}
and it is a hypothesis rather than a result: the supporting argument is a
loop-length count --- a reduced contractible fundamental loop at that energy
has four distinct links and so is an elementary plaquette, and the sector
qualification excludes the accidentally degenerate straight wrapping loop at
$L=4$ --- but no check covers it, and Section~\ref{sec:scope} lists it among
the premises carried in prose. Everything local to a plaquette pair is
independent of it; what needs it is the assembly, which is why
Conditional corollary~\ref{cor:assembly} names it again. With $Q=1_--P_-$ the
reduced resolvent crosses no omitted $E_{F,N}$ eigendirection, and degenerate
...
Assume in addition
\emph{(i)} the retained-shell hypothesis \eqref{eq:shell}, and
\emph{(ii)} that after the linked, vacuum-subtracted fold of \eqref{eq:heff}
one-shared-link processes exhaust the second-order \emph{off-diagonal}
intermediates on $P_-$ --- equivalently, that no connected second-order history
between non-adjacent plaquettes contributes a momentum-dependent term there.
Then
\[
...
This repository re-thresholds those gates rather
...
tower term is the certified coupling conversion $4\Delta_\pm(3u/2)$ of
\eqref{eq:bridge}, so \eqref{eq:assembly} takes no unregistered input. The twelve is a count on this
...
$L\ge3$ --- the retained-shell hypothesis \eqref{eq:shell} --- is argued in
Section~\ref{sec:sector} from the link count of a reduced fundamental loop and
carried as a hypothesis there, not established.

That one-shared-link processes exhaust the second-order \emph{off-diagonal}
intermediates is hypothesis \emph{(ii)} of
Conditional corollary~\ref{cor:assembly}; Theorem~\ref{thm:allrank}, being a
statement about one matrix element, does not need it. Three neighbouring
