...
This is a single-file, local/Colab pure-gauge Wilson-lattice validation
experiment.  It is intentionally conservative: smoke and pilot profiles are
diagnostics, while large production profiles require an explicit opt-in.  It
contains three logically separate layers:
  ...
  C. A published-data replay gate.
     ...
     This checks transcription and this simplified uncorrelated regression;
     it is never labelled as independent Monte Carlo evidence.

The exact operator bridge is

    Im Tr exp(iX) = -Tr(X^3)/6 + Tr(X^5)/120 + O(X^7)

for traceless X. The simulation also reports an exploratory one-cosh amplitude
ratio for the raw plaquette source. It is not a normalized spectral residue or
an equal-time ground-state fraction. Operator identity is exact; spectral
overlap remains a separate, falsifiable question.

Quick Colab / local CPU check:

    %run NB_FLUX_T1PM_engine_repaired.py --profile smoke

Published continuum replay only:

    %run NB_FLUX_T1PM_engine_repaired.py --profile replay

Pilot and continuum profiles remain in the engine for audit/prototyping, but
the delivered validation workflow does not recommend launching them as physics
runs. Large profiles require the explicit --allow-large-run acknowledgement.

The script strips only the notebook-injected `-f kernel.json` pair and rejects
all other unknown arguments.
It does not download physics data or mount drives.  NumPy/SciPy are sufficient
for CPU execution. GPU execution uses an already-installed CuPy/CUDA runtime.
The code never installs packages during a physics run. AMD/ROCm devices are
not supported by this CuPy backend.
"""
...
ENGINE_VERSION = "NB-FLUX repaired 2026-08-22"
RESULT_SCHEMA = "nb-flux-t1pm-result-v2"
...
def stable_sha256(value) -> str:
    encoded = json.dumps(json_safe(value), sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")
    ...
    def __init__(self, prefer_gpu: bool, seed: int, install_cupy: bool = False):
        ...
        if prefer_gpu:
            try:
                ...
                if install_cupy:
                    raise RuntimeError(
                        "--install-cupy is disabled in the repaired engine: install a CuPy wheel "
                        "before the run, restart the kernel, and rerun. Original GPU error: "
                        + self.gpu_error
                    ) from exc
...
def rank_mod_prime(matrix: NDArray[np.int64], prime: int) -> int:
    """Return the exact rank of an integer matrix after reduction to F_prime."""
    a = np.asarray(matrix, dtype=np.int64).copy() % int(prime)
    nrow, ncol = a.shape
    rank = 0
    for col in range(ncol):
        ...
        inv = pow(int(a[rank, col]), -1, int(prime))
        a[rank] = (a[rank] * inv) % prime
        rows = np.flatnonzero(a[:, col])
        rows = rows[rows != rank]
        if rows.size:
            factors = a[rows, col].copy()
            a[rows] = (a[rows] - factors[:, None] * a[rank][None, :]) % prime
        rank += 1
        if rank == nrow:
            break
    return int(rank)
...
def transform_plaquette_chains(
    chains: NDArray[np.int64], L: int, rotation: NDArray[np.int64]
) -> NDArray[np.int64]:
    """Apply a signed lattice rotation to actual oriented plaquette cochains."""
    sites = list(itertools.product(range(L), repeat=3))
    planes = ((0, 1), (0, 2), (1, 2))
    pindex: Dict[Tuple[Tuple[int, ...], int, int], int] = {}
    for x in sites:
        for mu, nu in planes:
            pindex[(x, mu, nu)] = len(pindex)
    out = np.zeros_like(chains)
    eye = np.eye(3, dtype=np.int64)
    for x in sites:
        rx = rotation @ np.asarray(x, dtype=np.int64)
        for mu, nu in planes:
            dmu = rotation @ eye[:, mu]
            dnu = rotation @ eye[:, nu]
            p = int(np.argmax(np.abs(dmu)))
            q = int(np.argmax(np.abs(dnu)))
            smu, snu = int(dmu[p]), int(dnu[q])
            a, b = sorted((p, q))
            order_sign = 1 if p < q else -1
            orientation = smu * snu * order_sign
            base = rx.copy()
            if smu < 0:
                base += dmu
            if snu < 0:
                base += dnu
            target_x = tuple(int(v % L) for v in base)
            source = pindex[(x, mu, nu)]
            target = pindex[(target_x, a, b)]
            out[target] += orientation * chains[source]
    return out
...
def run_symmetry_topology_certificate() -> None:
    ...
    ranks_B = tuple(rank_mod_prime(B, p) for p in primes)
    ranks_C = tuple(rank_mod_prime(C, p) for p in primes)
    rank_B = ranks_B[0]
    rank_C = ranks_C[0]
    ...
    gate(
        "torus incidence ranks (exact modular certificate)",
        len(set(ranks_B)) == 1
        and len(set(ranks_C)) == 1
        and rank_B == expected_rank_B
        and rank_C == expected_rank_C,
        f"rank(d2)={ranks_B}, rank(d3)={ranks_C} over primes {primes}",
    )
    gate(
    ...
    # Rotate the actual plaquette cochains, including basepoint shifts and
    # orientation signs, rather than reconstructing R from R.
    carrier_rep_ok = True
    for R in rotations:
        carrier_rep_ok &= np.array_equal(transform_plaquette_chains(H, Cx.L, R), H @ R)
    parity_ok = np.array_equal(
        transform_plaquette_chains(H, Cx.L, -np.eye(3, dtype=np.int64)), H
    )
    gate(
        "zero-momentum harmonic carrier representation",
        carrier_rep_ok and parity_ok and H.shape[1] == 3 and b2 == 3,
        "actual cochain permutation gives P_R H=H R for all 24 rotations and P H=H; ImTr is C-odd",
    )
...
class EnsembleConfig:
    ...
    def validate(self) -> None:
        ...
        if self.fit_tmax >= self.Nt // 2:
            raise ValueError("fit_tmax must be strictly below Nt/2")
...
class SU3WilsonLattice:
    ...
    def reunitarize_matrix(self, M):
        """Bi-unitarily equivariant polar projection to SU(3).

        Ordered column Gram--Schmidt is not covariant under independent gauge
        rotations at the two ends of a link. The polar factor U V^dagger is;
        the scalar determinant correction then places it in SU(3).
        """
        xp = self.xp
        U, _, Vh = xp.linalg.svd(M)
        polar = U @ Vh
        det = xp.linalg.det(polar)
        phase = xp.exp((-1j / 3.0) * xp.angle(det))
        projected = phase[..., None, None] * polar
        return projected.astype(self.dtype, copy=False)

    def audit_projection_covariance(self) -> Tuple[float, float, float]:
        """Test the actual projection under M -> G M H^dagger for G,H in SU(3)."""
        h1 = np.array(
            [[0.31, 0.17 + 0.11j, -0.09j], [0.17 - 0.11j, -0.22, 0.13], [0.09j, 0.13, -0.09]],
            dtype=np.complex128,
        )
        h2 = np.array(
            [[-0.18, 0.07j, 0.16], [-0.07j, 0.27, -0.12 + 0.05j], [0.16, -0.12 - 0.05j, -0.09]],
            dtype=np.complex128,
        )
        h1 -= np.eye(3) * np.trace(h1) / 3.0
        h2 -= np.eye(3) * np.trace(h2) / 3.0
        G = linalg.expm(1j * h1)
        H = linalg.expm(1j * h2)
        rng = np.random.default_rng(4815162342)
        M = rng.normal(size=(16, 3, 3)) + 1j * rng.normal(size=(16, 3, 3))
        xp = self.xp
        Mx, Gx, Hx = xp.asarray(M, dtype=self.dtype), xp.asarray(G, dtype=self.dtype), xp.asarray(H, dtype=self.dtype)
        lhs = self.reunitarize_matrix(Gx @ Mx @ self.dagger(Hx))
        rhs = Gx @ self.reunitarize_matrix(Mx) @ self.dagger(Hx)
        cov = self.B.scalar(xp.max(xp.sqrt(xp.sum(xp.abs(lhs - rhs) ** 2, axis=(-2, -1)))))
        unit = self.B.scalar(
            xp.max(xp.sqrt(xp.sum(xp.abs(self.dagger(lhs) @ lhs - xp.eye(3, dtype=self.dtype)) ** 2, axis=(-2, -1))))
        )
        det = self.B.scalar(xp.max(xp.abs(xp.linalg.det(lhs) - 1.0)))
        return cov, unit, det
    ...
    def gauge_transform_spatial(self, spatial_links, gauge_field):
        out = spatial_links.copy()
        for i in range(3):
            axis = i + 1
            out[..., i, :, :] = (
                gauge_field
                @ spatial_links[..., i, :, :]
                @ self.dagger(self.shift(gauge_field, axis, 1))
            )
        return out

    def audit_ape_gauge_covariance(self) -> float:
        """Random-local-gauge test of the complete one-step APE map."""
        rng = np.random.default_rng(8675309)
        raw = rng.normal(size=self.shape + (3, 3)) + 1j * rng.normal(size=self.shape + (3, 3))
        gauge = self.reunitarize_matrix(self.xp.asarray(raw, dtype=self.dtype))
        spatial = self.U[..., 1:4, :, :].copy()
        transformed = self.gauge_transform_spatial(spatial, gauge)
        lhs = self.ape_step(transformed)
        rhs = self.gauge_transform_spatial(self.ape_step(spatial), gauge)
        err = self.xp.max(self.xp.sqrt(self.xp.sum(self.xp.abs(lhs - rhs) ** 2, axis=(-2, -1))))
        return self.B.scalar(err)
    ...
    def spatial_polyakov_operators(self, spatial_links) -> NDArray[np.complex128]:
        ...
        for i in range(3):
            ...
            # Sum the L^2 distinct transverse loops, average the L cyclically
            # repeated winding starts, then divide by sqrt(L^2)=L so the
            # zero-transverse-momentum operator has volume-stable norm.
...
def integrated_autocorrelation_time(x: NDArray[np.float64]) -> float:
    ...
    if not np.isfinite(x).all():
        raise ValueError("autocorrelation series contains non-finite values")
...
def gevp_vector(C: NDArray[np.float64], basis: VariationalBasis, t0: int, td: int) -> NDArray[np.float64]:
    ...
    # move. Reject a materially indefinite metric rather than manufacturing a
    # large vector by clipping it to a tiny positive number.
    eb, Qb = np.linalg.eigh(B)
    emax = float(np.max(eb))
    floor = max(1e-10 * emax, 1e-12)
    if not np.isfinite(eb).all() or emax <= 0 or float(np.min(eb)) <= floor:
        raise RuntimeError("bootstrap GEVP metric is non-positive or numerically singular")
    Binvhalf = Qb @ np.diag(eb ** -0.5) @ Qb.T
    Aw = Binvhalf @ A @ Binvhalf
    vals, vecs = np.linalg.eigh(0.5 * (Aw + Aw.T))
    z = Binvhalf @ vecs[:, np.argmax(vals)]
    v = W @ z
    norm2 = float(v @ C[t0] @ v)
    if not math.isfinite(norm2) or norm2 <= 1e-20:
        raise RuntimeError("GEVP vector has a non-positive C(t0) norm")
...
def fit_periodic_cosh(g: NDArray[np.float64], Nt: int, tmin: int, tmax: int, sigma: Optional[NDArray[np.float64]] = None) -> Tuple[float, float, float, int]:
    ...
    if len(t) < 3:
        raise RuntimeError("a two-parameter cosh fit needs at least three time slices")
    if sigma is None:
        s = np.ones_like(y)
    else:
    ...
    if not result.success:
        raise RuntimeError(f"cosh minimization failed: {result.message}")
    m = float(result.x)
    f = np.exp(-m * t) + np.exp(-m * (Nt - t))
    w = 1.0 / (s * s)
    A = float(np.sum(w * f * y) / np.sum(w * f * f))
...
class ChannelResult:
    ...
    bootstrap_succeeded: int
    bootstrap_success_fraction: float
    fit_hit_bound: bool
    projected_correlator: List[float]
    effective_mass: List[float]
    raw_amplitude_ratio: Optional[float] = None
    raw_amplitude_ratio_error: Optional[float] = None
...
def analyze_channel(
    ...
    basis = build_whitener(C[0], max_rank=min(10, C.shape[-1]))
    td = 1
    ...
    for _ in range(n_boot):
        ...
        try:
            ...
            continue
        if np.isfinite(mb) and 2e-4 < mb < 5.95:
            boot_g.append(gb)
            boot_m.append(mb)
            boot_amp.append(Ab)
            if raw_index is not None:
                ...
                # The fitted amplitude multiplies both forward and backward
                # exponentials; at t=0 the model contribution is
                # Ar*(1+exp(-m Nt)). This remains only an exploratory ratio.
                boot_frac.append(Ar * (1.0 + math.exp(-mb * Nt)) / max(float(raw[0]), 1e-30))
    if len(boot_m) < max(30, n_boot // 2):
    ...
    return ChannelResult(
        ...
        n_blocks=nblock,
        bootstrap_attempted=n_boot,
        bootstrap_succeeded=len(boot_m),
        bootstrap_success_fraction=float(len(boot_m) / n_boot),
        fit_hit_bound=bool(mass <= 2e-4 or mass >= 5.95),
        projected_correlator=[float(x) for x in g],
        effective_mass=[float(x) for x in meff],
        raw_amplitude_ratio=frac,
        raw_amplitude_ratio_error=frac_err,
    )
...
def run_published_replay() -> ContinuumFit:
    ...
    delta = fit.intercept - benchmark
    gate(
        "published continuum replay",
        abs(delta) < 0.10,
        f"replay={fit.intercept:.4f}({fit.intercept_error:.4f}), paper={benchmark:.4f}({benchmark_err:.4f}), "
        f"same-source delta={delta:+.4f} (regression tolerance 0.10; not an independent pull)",
    )
    return fit
...
def run_ensemble(cfg: EnsembleConfig) -> EnsembleResult:
    ...
    if cfg.prefer_gpu and not backend.is_gpu and backend.gpu_error:
        print(f"  [INFO] CUDA backend unavailable; using CPU ({backend.gpu_error})")
    if cfg.prefer_gpu and not backend.is_gpu and cfg.L >= 14:
        raise RuntimeError(
            "production lattice requested but no CUDA/CuPy backend is available; install and "
            "validate a compatible CuPy wheel before launching the run, restart the kernel, "
            "or pass --no-gpu intentionally"
        )
    print(
    ...
    proj_cov, proj_unit, proj_det = lat.audit_projection_covariance()
    projection_tol = 2e-5 if lat.dtype == backend.xp.complex64 else 2e-11
    gate(
        "APE SU(3) projection gauge covariance",
        proj_cov < projection_tol and proj_unit < projection_tol and proj_det < projection_tol,
        f"covariance={proj_cov:.3e}, unitarity={proj_unit:.3e}, det={proj_det:.3e}",
    )
    p_hist, a_hist = thermalize(lat)
    thermalization_gates(lat, p_hist, a_hist)
    total_before = lat.plaquette_trace_sum()
    lat.overrelaxation_sweep(audit=True)
    total_after = lat.plaquette_trace_sum()
    or_global_density = abs(total_after - total_before) / float(6 * np.prod(lat.shape))
    gate(
        "global over-relaxation action invariance",
        or_global_density < 2e-5,
        f"absolute trace-sum drift per plaquette={or_global_density:.3e}",
    )
    # Audit the staple identity on a post-warm-up, nontrivial field. Testing it
    # only on the cold identity field can miss orientation/shift mistakes.
    stencil_error = lat.audit_local_action_stencil()
    gate(
        "Wilson action/staple identity on post-warm-up field",
        stencil_error < 5e-5,
        f"one-link total/local trace mismatch={stencil_error:.3e}",
    )
    if int(np.prod(lat.shape)) <= 10000:
        ape_cov = lat.audit_ape_gauge_covariance()
        gate(
            "full APE-step local gauge covariance on post-warm-up field",
            ape_cov < 5.0 * projection_tol,
            f"max transformed-link Frobenius error={ape_cov:.3e}",
        )
    else:
        print("  [INFO] full-field APE covariance audit skipped on large volume; polar projection audit passed")
    ...
    if t1_res.raw_amplitude_ratio is not None:
        print(
            f"  raw ImTr(plaquette) exploratory fitted amplitude ratio="
            f"{t1_res.raw_amplitude_ratio:.6f} +/- {t1_res.raw_amplitude_ratio_error:.6f}"
        )
        print("  (diagnostic only: this is not a normalized spectral residue)")
    print(
    ...
    t1_fit_ok = (
        np.isfinite(t1_res.mass)
        and not t1_res.fit_hit_bound
        and t1_res.dof > 0
        and np.isfinite(t1_res.p_value)
        and t1_res.p_value > 0.01
        and t1_res.bootstrap_success_fraction >= 0.5
    )
    tor_fit_ok = (
        np.isfinite(tor_res.mass)
        and not tor_res.fit_hit_bound
        and tor_res.dof > 0
        and np.isfinite(tor_res.p_value)
        and tor_res.p_value > 0.01
        and tor_res.bootstrap_success_fraction >= 0.5
    )
    gate(
        "T1 exploratory mass fit",
        t1_fit_ok,
        f"aM={t1_res.mass:.4f}, dof={t1_res.dof}, p={t1_res.p_value:.3f}, "
        f"bootstrap={t1_res.bootstrap_succeeded}/{t1_res.bootstrap_attempted}",
        hard=physical_run,
    )
    gate(
        "torelon exploratory fit",
        tor_fit_ok,
        f"aE={tor_res.mass:.4f}, dof={tor_res.dof}, p={tor_res.p_value:.3f}, "
        f"bootstrap={tor_res.bootstrap_succeeded}/{tor_res.bootstrap_attempted}",
        hard=physical_run,
    )
    ...
    if cfg.published_mass is not None:
        pub_err = float(cfg.published_mass_error or 0.0)
        pull = (t1_res.mass - cfg.published_mass) / max(math.hypot(t1_res.mass_error, pub_err), 1e-12)
    ...
    if cfg.published_asqrt_sigma is not None:
        pub_err = float(cfg.published_asqrt_sigma_error or 0.0)
        pull = (asqrt - cfg.published_asqrt_sigma) / max(math.hypot(asqrt_err, pub_err), 1e-12)
    ...
    if t1_res.raw_amplitude_ratio is not None and t1_res.raw_amplitude_ratio_error is not None:
        print(
            "  [INFO] no pass/fail gate is assigned to the raw fitted amplitude ratio; "
            "a residue requires a normalized spectral-projector or controlled multi-state analysis"
        )
...
def load_ensemble_jsons(paths: Sequence[str]) -> List[EnsembleResult]:
    ...
    for path in paths:
        ...
        if payload.get("schema") != RESULT_SCHEMA:
            raise ValueError(f"{path}: unsupported or missing result schema")
        if not payload.get("overall_passed", False):
            raise ValueError(f"{path}: saved run did not pass all hard gates")
        source_hashes.add(payload.get("engine_sha256"))
    ...
    if len(source_hashes) != 1 or None in source_hashes:
        raise ValueError(f"input files were produced by different or unidentified engine sources: {source_hashes}")
    return out
...
def parse_args(argv: Optional[Sequence[str]] = None):
    ...
    parser.add_argument(
        "--profile",
        choices=("smoke", "pilot", "production", "continuum", "combine", "replay"),
    ...
    parser.add_argument(
        "--install-cupy",
        action="store_true",
        help="compatibility flag; repaired engine refuses in-run installation and asks for a kernel restart",
    )
    parser.add_argument(
        "--allow-large-run",
        action="store_true",
        help="required acknowledgement for production/continuum profiles",
    )
    ...
    i = 0
    while i < len(raw):
        if raw[i] == "-f" and i + 1 < len(raw) and str(raw[i + 1]).lower().endswith(".json"):
            i += 2
            continue
        cleaned.append(raw[i])
        i += 1
    # Misspelled scientific options now fail loudly; only the standard kernel
    # connection-file pair is stripped.
...
def final_summary(results: Sequence[EnsembleResult], replay: ContinuumFit, elapsed: float, new_fit: Optional[ContinuumFit] = None) -> None:
    ...
    print("\n  Claim ledger:")
    print("    * Executably certified on the tested L=4 periodic complex: H2 is the three-dimensional")
    print("      zero-momentum plaquette-plane carrier; cube boundaries lie in B2, are A1^{--},")
    print("      and telescope at k=0.")
    ...
    physical_results = []
    for r in results:
        ncfg = int(r.config.get("n_cfg", 0))
        pub_scale = r.config.get("published_asqrt_sigma")
        large_enough = pub_scale is not None and float(r.config["L"]) * float(pub_scale) >= 3.0
        if ncfg >= 100 and large_enough:
            physical_results.append(r)
    if hard_failures and results:
        print("    * This run is inconclusive: at least one hard gate failed; no new physics claim is made.")
    elif physical_results:
        print("    * Exploratory ensemble output from this run: the reported lattice mass, string scale,")
        print("      exploratory raw-operator amplitude ratio, and weak-field expansion diagnostics.")
    elif results:
    ...
    if new_fit is not None and not hard_failures:
    ...
    elif new_fit is not None:
        print("    * A continuum fit was computed but is rejected because a hard gate failed.")
    print("    * Not proven: equality of the one-plaquette Hamiltonian gap and the physical glueball mass.")
    print("    * Not identified here: continuum spin. The raw T1 source mixes leading J=1 and J=3")
    print("      tensor components; A2/T2 partners and spin-resolved sources are absent.")
    print("    * APE step counts are not a fixed-physical-radius continuum source trajectory.")
    if hard_failures:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.profile in {"production", "continuum"} and not args.allow_large_run:
        raise RuntimeError(
            "Large production profiles are disabled by default. First pass the repaired smoke/pilot "
            "workflow and review its diagnostics; then rerun with --allow-large-run explicitly."
        )
    print(f"Spatial SU(3) T1^{{+-}} cubic-Casimir bridge test ({ENGINE_VERSION})")
    ...
    if args.json:
        ...
        payload = {
            ...
            "overall_passed": not hard_failures,
        ...
        payload["analysis_config_sha256"] = stable_sha256(
            {"profile": args.profile, "configs": [asdict(r)["config"] for r in results]}
        )
        certificate_payload = json.loads(
            json.dumps(json_safe(payload), sort_keys=True, allow_nan=False)
        )
        if isinstance(certificate_payload.get("ensemble"), dict):
            certificate_payload["ensemble"].pop("wall_seconds", None)
        for row in certificate_payload.get("ensembles", []):
            if isinstance(row, dict):
                row.pop("wall_seconds", None)
        payload["deterministic_payload_sha256"] = stable_sha256(certificate_payload)
        tmp_json = args.json + ".tmp"
        with open(tmp_json, "w", encoding="utf-8") as f:
            json.dump(json_safe(payload), f, indent=2, allow_nan=False)
        os.replace(tmp_json, args.json)
