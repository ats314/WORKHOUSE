...
**Date:** 2026-08-28  
**Status:** exact through second order in the charge-odd one-plaquette sector; the \(B=4=T_1\) cube benchmark is reconstructed and passed; the \(B=6\) full-matrix benchmark remains open  
**Main result:** the channel-complete finite-\(N\) Schur complement supplies an exact three-dimensional spectral calculation beyond the simulated \(T_1\) cutoff, while an author-data-derived \(B=4\) cube reconstruction independently validates the predicted \(1+3+2\) truncated shell.
...
Balaji et al. estimate that a direct \(T_2\) plaquette construction in \(d=3\) requires at least \(10^{20}\) magnetic matrix elements. Even their softer \(B\) truncation reaches only selected states involving all two-index irreps at roughly \(10^8\) matrix elements. The package scope described in the paper supports circuit generation only in \(d=3/2\) and \(d=2\), and lists \(d\ge3\) and open boundary conditions as future work. The later public development history now contains a \(d=3,B=4\) local master table, but still does not publish a native open-cube global ED matrix or charge-resolved spectrum. The paper's resource boundaries are stated in [arXiv:2509.25865v3](https://arxiv.org/html/2509.25865v3).
...
## 10. Decisive \(B=4=T_1\) full-Hamiltonian test: passed

The authors' original global sparse matrix and charge-resolved spectrum are not public. Their public \(d=3,B=4\) master plaquette-coefficient table nevertheless determines the exact gauge-invariant physical block of the six-face open cube. An independent assembly gives:
...
The only nontrivial boundary step was tested separately. A direct open-boundary `pyclebsch` generator and the trivially restricted periodic table agree for all \(6\times81=486\) directed cube-face coefficients, with zero missing or extra entries and maximum coefficient error zero. A broader \(B=3\) signed-direction convention audit gives 17,367 exact matches out of 17,367. Thus the open-cube restriction is tested coefficient by coefficient rather than inferred from the table filename.
...
the exact charge-odd one-face shell is
...
After subtracting the singleton, this is
...
which is exactly the predicted \(1+3+2\) shell and the reversed \(T_1\) ordering. The full second-order matrix identity is
...
with maximum entrywise error \(4.27\times10^{-11}\). No spectral coefficient or multiplicity was fit.

The cube action identifies these subspaces independently of their eigenvalue degeneracies:
...
As an end-to-end normalization and assembly check, the reconstructed 243-state block reproduces all 61 vector points of the paper's published exact cube ground-state curve over \(0.8\le g\le2.0\), with maximum absolute discrepancy \(5.59\times10^{-8}\) and RMS discrepancy \(1.88\times10^{-8}\), at the coordinate precision of the published vector figure.

This upgrades the \(B=4=T_1\) shell from a prediction to a validated author-data-derived reconstruction. It is not an independent-group replication: the paper, `ymcirc`, and `pyclebsch` share an author/code lineage, and the global cube assembly is ours rather than an author-supplied matrix.

The remaining decisive cutoff-completion test is now only the \(B=6\) cube with all four shared-link channels. Its acceptance limit in the Balaji normalization is
...
which has not yet been checked against a full \(B=6\) cube matrix. That test would validate the predicted sign reversal from the observed \(T_1\) ordering to the channel-complete ordering.
...
- `reconstruct_balaji_open_cube.py` reconstructs the exact 243-state physical block from the pinned public Balaji/`ymcirc` coefficient table, projects charge, evaluates the complete Schur complement, performs finite-\(u\) diagonalizations, and checks the published ground curve.
- `balaji_open_cube_B4_T1_hamiltonian.npz` stores the reconstructed matrix family and bases.
- `balaji_open_cube_B4_T1_certificate.json` stores source hashes, matrix diagnostics, shell coefficients, finite-\(u\) results, and the 61-point published-curve comparison.
- `balaji_open_cube_padding_audit_certificate.json` records the independent direct-open-boundary comparison of all 486 cube-face coefficients.
- `BALAJI_OPEN_CUBE_CHARGE_ODD_SHELL_VALIDATION_2026-08-28.md` gives the complete validation derivation and claim boundary.
...
> The exact finite-rank SU(3) plaquette matrix elements admit a charge-odd, second-order Schur complement equal to \((5/612)u^2B^\dagger B\). This supplies a channel-complete three-dimensional finite-volume spectrum using four local fusion channels and sparse cubical incidence. An exact 243-state open-cube reconstruction from Balaji et al.'s public \(B=4=T_1\) master coefficients reproduces their published ground-state curve and independently recovers the predicted \(1+3+2\) charge-odd shell, including the negative \(-1/12\) kinetic coefficient and reversed \(T_1\) ordering. The missing \(\mathbf6\) and \(\mathbf8\) channels analytically reverse that coefficient to \(+5/612\), but the corresponding full \(B=6\) cube diagonalization remains open. The construction bypasses the full \(d=3,T_2\) transition census at this order; it is not a full finite-coupling or continuum calculation.

That is more than a specialist coefficient identity: it is a proved diagnostic and a changed calculation capability. The first decisive numerical benchmark, the published \(B=4=T_1\) open cube, now passes. The stronger channel-complete benchmark remains the \(B=6\) cube.
