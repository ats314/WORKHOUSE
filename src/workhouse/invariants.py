...
def _():
    ...
    #
    # The interesting number in this graph is not its node or edge count --
    # both grow every session, and both were quoted here as fixed figures
    # until 2026-08-28, by which time each was wrong. It is the count of nodes
    # with no edge at all, which the detail line below computes rather than
    # asserts. A stranded node is invisible to `workhouse why`, so it is
    # evidence nobody can traverse to, and a graph that quietly accumulates
    # them looks complete while going hollow.
    #
...
def _():
    ...
    excluded = absent["relation"] is None and not absent["exhausted"] and absent["norm_bound"]
    return agree and norm(lll) == (3, -5, -1) and bool(excluded), (
        ...
        f"of norm below {absent['norm_bound']:.6g} exists at 16 digits"
    )
