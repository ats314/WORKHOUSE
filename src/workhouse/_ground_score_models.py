...
def controls():
    ...
    require(
        all(sp.simplify(beta - t) > 0 for t in generalized_eigenvalues), "operator-norm Fisher cap"
    )
    gvalues = [(sp.ones(2, 1).T * s)[0] for s in score_values]
    variance = sum(g * g for g in gvalues) / 4
    correlation = (
        sum((g * s for g, s in zip(gvalues, score_values, strict=True)), sp.zeros(2, 1)) / 4
    )
    ...
    def average(expression, variable):
        return sp.simplify(
            sp.integrate(sp.expand_trig(expression), (variable, -sp.pi, sp.pi)) / (2 * sp.pi)
        )
    ...
    require(
        sp.simplify(only_divergence - correlation_score) != 0,
        "omitted density connection is rejected",
    )
    require(
        sp.simplify(only_density_term - correlation_score) != 0,
        "omitted fiber divergence is rejected",
    )
    literal_f = sp.sin(x)
    ...
    zero(
        form_matrix.charpoly().as_expr().subs(form_matrix.charpoly().gen, delta),
        "exact sharp energy eigenvalue",
    )
    ...
    require(
        sp.simplify((form_matrix - wrong_delta * sp.eye(2)).det()) < 0,
    ...
    require(
        floor * normalized / (floor + normalized) <= delta <= normalized,
        "independent Schur sandwich",
    )
    ...
    return {
        ...
        "noncommuting_fisher": {
            "A": str(a),
            "I": str(fisher),
            "operator_eigenvalues": list(map(str, generalized_eigenvalues)),
            "beta": str(beta),
            "eta": str(eta),
            "w": str(weight),
            "congruence_slack": str(slack),
            "variance_g": str(variance),
            "coarse_form": str(coarse_form),
            "cross_form": str(cross),
            "cross_squared": str(cross**2),
            "upper_bound": str(eta * coarse_form * weight * variance),
        },
        "nonzero_connection_torus": {
            "conditional_density_relative_to_uniform_Haar": str(density),
            "b": str(b),
            "score": str(sp.factor(score)),
            "exact_cross_form": str(cross_direct),
            "score_correlation": str(correlation_score),
            "only_divergence": str(only_divergence),
            "only_density_connection": str(only_density_term),
        ...
        "sharp_gap_model": {
            "matrix": str(form_matrix),
            "alpha": str(alpha),
            "f0": str(floor),
            "eta": str(eta),
            "delta": str(delta),
            "exact_eigenvector": str(nullvector),
            "normalized_schur_energy": str(normalized),
            "wrong_fisher_factor_rejected": True,
        },
        "weighted_ratio": {
            "ratio": str(ratio),
            "inverse_sqrt_u_majorant": str(majorant / t),
            "margin": str(margin),
        },
        "scope": (
            "Exact noncommuting finite Fisher model, compact torus connection identity, two-"
            "state sharp gap and scalar weighted-ratio controls. No uniform Wilson score, "
            "actual interacting-volume fast form or marginal-gap input is certified."
        ),
    }
...
# Original SHA256: 18d965bec7edbb123b35c10354f0cb3839f40246528ead27acc3941f2ad6f770
