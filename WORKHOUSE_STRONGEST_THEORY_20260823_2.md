...
**REVISION 2** (post adversarial verification, 24 agents): four claims below are corrected
downward. See §13 for the full list. Headline: **R2 is entirely prior art** in a theory
document I failed to open, and **R3's hypothesis is far tighter than Revision 1 stated**.
...
**Prior art — R2 IS PRIOR ART, and Revision 1 was wrong to claim otherwise.**
`theory/GLUEBALL_DETAILED_FORMULA_2026-08-20_v3.1.md` — a current, SHA256SUMS-pinned theory
document — states all of it, and I never opened the file:

- **:175** — *"The `k=0` fiber is a homology problem, not a quotient by `q(0)`."*
- **:203–219** — `ℋ₂ := Z₂ ∩ ker ∂₃† ≅ H₂(C_•)`, `Z₂ = B₂ ⊕ ℋ₂`, `dim B₂ = L³−1`,
  `dim ℋ₂ = 3`, and *"The first nonflat adjacency level is `4sin²(π/L)`, so finite-volume
  isolation closes as `L⁻²`."*
- **:230** — *"the harmonic plane triplet is assigned **axial `T₁`, parity `+`, and `C=−`**
  from the imaginary-trace source. The topology of the split is exact; the
  representation-to-interpolating-operator bridge is a separate analytic interpretation and
  should not be mislabeled as a measured physical overlap."*

That is R2's identification and its representation assignment, verbatim, with its own caveat
already attached. The residue is thin: only the observation that §5.2's scalar-Γ-block
*assumption* is a Schur consequence of an assignment the corpus already prints. That is a
tier remark, not a result.

**And the J = 1 step needs a hypothesis I stated too loosely.** A rank-3 *cubic* `T₁` is
compatible with `J = 1, 3, 4, 5, 6, 7, 8` — `T₁` occurs once in `J = 4` as well. `J = 1`
follows only for a **complete SO(3) multiplet**, where `dim = 2J+1 = 3` forces it. Document A
states this correctly ("under restored `SO(3)`"); Revision 1 did not. The `O → SO(3)`
subduction table is genuinely absent from the corpus and would be worth registering.
...
Only *(a)* and *(d)* use `α`; *(b)* and *(c)* use only `β ≥ 0`.

**The `β ≥ 0` hypothesis is much tighter than Revision 1 implied.** In `C_shp` coordinates,
`β = 2α + 16C_shp ≥ 0 ⟺ C_shp ≥ −α/8 = −5/96 = −0.0520833333`:

```
historical  C_shp = −0.0480863832   margin  0.0039969502   ( 8.31% of |C_shp|)
v10a.26     C_shp = −0.0202133289   margin  0.0318700044   (157.67% of |C_shp|)
```

The historical branch clears the threshold by **0.0040**. Saying "both branches have `β > 0`"
is true and misleading: R3 is one branch away from failing, and **G3 is the run that decides
it**. That makes the falsifier live in a much stronger sense than Revision 1 conveyed.

**The equality case needs `b ≥ 3`.** At `b = 2`, `n = 1` and `n = b−1 = 1` coincide, so there
are **three** axial aliases, not six (verified: argmins `{(1,0,0),(0,1,0),(0,0,1)}`, min `5/12`).
The *value* is unaffected at every `b`.
...
**Prior art — R3's novelty is smaller than Revision 1 claimed.** The reformulation is exact
and deflating:

```
g(L) = α·sin²(π/L) = 4·A_shp·sin²(π/L) = A_shp × [ 4 sin²(π/L) ]
```

and `4sin²(π/L)` is **the corpus's own "first nonflat adjacency level"**, which
`DETAILED_FORMULA:219` already pairs with the conclusion *"finite-volume isolation closes as
`L⁻²`."* So the corpus asserts the `L⁻²` closure and names the very function. What R3 adds is
the **proof** that the fourth-order coefficient in front of it is exactly `A_shp`, and that
this is `β`-free — i.e. a proof and a C2-independence statement, not a new phenomenon.
The claim-catalogue searches ("alias", "fiber", "isolation gap") still return nothing, but
that reflects the catalogue, not the corpus.
...
2. ~~**`IsLittleO.tendsto_div_nhds_zero` needs `g` eventually non-zero.**~~ **I was wrong.**
   It is unconditional in Mathlib, so `valid_gap` going unused in `error_term_tends_to_zero`
   is correct, not a dangling dependency. Real defects in its place: the import path
   `Mathlib.Analysis.Asymptotics.Asymptotics` **does not exist** at the repo-pinned Mathlib;
   the instruction to append the block mid-file guarantees a syntax error (`import` after
   declarations); and `tendsto_of_tendsto_of_tendsto_of_le_of_le` takes the **Pi order**
   `g ≤ f`, not `∀ x, g x ≤ f x`. The `Tendsto.const_mul` / `.pow` / `.const_sub` / `zero_pow`
   argument orders I doubted are all **correct**.
