...
def single_link_row_sum(mass: float, amplitude: float, cutoff: int = 500) -> float:
    ...
    return math.fsum(
        (12 if r == 1 else 12 * r * r) * amplitude * math.exp(-mass * r) for r in range(1, cutoff)
    )
...
def phase8_replay(archived: dict) -> dict:
    ...
    for row in archived["part3_block_tensorization"]["scans"]:
        ...
        checks.append(
            dict(
                name=f"phase8_kappa_block_M{row['M']}_L{row['L']}",
                replay=value,
    ...
    for half in (10, 20):
        ...
        four[half] = counts
    four_d = {
        f"kappa_block_4d_M{mass}": block_row_sum(four[10 if mass > 1 else 20], mass, amplitude)
        for mass in (2.1939, 1.5515, 0.8000)
    }
    four_d["kappa_single_4d_z32r3"] = math.fsum(
        (24 if r == 1 else 32 * r**3) * amplitude * math.exp(-2.1939 * r) for r in range(1, 500)
    )
...
def phase9_replay(archived: dict) -> dict:
    ...
    vectors = [
        [elements[i][n] * math.exp(-energies[n] * times[i]) for n in range(4)] for i in range(6)
    ]
    gram_small = [
    ...
    checks = [
        dict(
            name=f"phase9_os2_gram_invariant_{label}",
    ...
    sequence = [
        0.85**2 * math.exp(-2.1939 * t) + 0.10**2 * math.exp(-2.9250 * t) for t in range(1, 11)
    ]
    for index, target in enumerate(archived["os4_clustering"]["S2_connected"]):
        error = abs(sequence[index] - target)
        checks.append(
            dict(
                name=f"phase9_os4_S2_t{index + 1}",
    ...
    return dict(
        ...
        not_replayed=("Part 3 beta = 4 moments are Monte-Carlo statistics with no stored seed."),
    )
