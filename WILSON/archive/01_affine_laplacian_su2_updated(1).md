...
This is strikingly *rigid*. In 4D you might notice \(12=D(D-1)\), but the analytic explanation is simpler: \(12=4\times 3\) (four links per plaquette \(	imes\) the SU(2) Laplacian eigenvalue on the fundamental trace component under the project’s normalization).
...
4. Show the resulting coefficient is exactly \(12=4\times 3\), independent of the lattice dimension \(D\) as long as a plaquette is a 4-link square and \(B_{\rm avg}\) is the plaquette average.
...
See `05_affine_laplacian_law_analytic_proof.md` for a full derivation.
...
2. Test whether the coefficient stays pinned at \(12\) for other lattice dimensions \(D\) (e.g., 2D, 3D); analytically it should, with the same Laplacian normalization.
